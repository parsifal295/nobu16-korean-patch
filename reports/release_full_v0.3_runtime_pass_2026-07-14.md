# 전체 MSGUI / Font-v4 v0.3 런타임 검증

날짜: 2026-07-14 (KST)

## 판정

간체중문(SC) 파일 경로의 전체 `msgui` 0–5099 레시피와 Font-v4 조합은 실제 게임에서 부팅, 한국어 표시, 대표 화면 글리프·잘림 확인, 정상 종료, 순정 복원을 통과했다. 성 이름 세로 표시는 사용자 결정에 따라 알려진 제한으로 수용하며 `castle_name_horizontal=false`로 기록했다.
...
- 대표 장문은 읽을 수 있었고 치명적인 잘림은 보이지 않았다.
...
- manifest: `version=0.3`, `release_eligible=true`, `runtime_validation=passed`
- 독립 공개 감사: 통과, 패키지 파일 17개

배포본은 레시피·델타와 OFL 글꼴에서 프로젝트가 생성한 글리프 픽셀만 포함하며 완성 상용 게임 리소스를 포함하지 않는다.
