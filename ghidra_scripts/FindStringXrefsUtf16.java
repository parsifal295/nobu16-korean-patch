...
public class FindStringXrefsUtf16 extends GhidraScript {
    @Override
    public void run() throws Exception {
        String[] args = getScriptArgs();
        if (args.length == 0) {
            throw new IllegalArgumentException("UTF-16LE strings required");
        }
        Memory memory = currentProgram.getMemory();
        for (String needle : args) {
            byte[] bytes = needle.getBytes(StandardCharsets.UTF_16LE);
