...
PUBLIC_MANIFEST = ROOT / "data" / "public" / "steam_jp_117_v0111_release.v1.json"
...
release = load_module(
    "steam_jp_v0111_release", "tools/build_steam_jp_v0111_release.py"
)
previous_release = load_module(
    "steam_jp_v0110_release_for_v0111_test",
    "tools/build_steam_jp_v0110_release.py",
)


class SteamJpV0111ReleaseTests(unittest.TestCase):
    def make_fixture(
        self, root: Path
    ...
    def test_version_and_release_names_are_v0111(self) -> None:
        self.assertEqual(release.VERSION, "v0.11.1")
        self.assertIn("v0.11.1", release.ZIP_NAME)
        self.assertIn("v0.11.1", release.MANIFEST_NAME)
        self.assertTrue(release.SCHEMA.endswith("v0.11.1"))
    ...
    def test_public_manifest_matches_the_release_builder_contract(self) -> None:
        ...
        self.assertEqual(
            manifest["release_zip"],
            {
                ...
                "size": 374_143_647,
                "sha256": "DC3A3DC0792F665C6D7B000F7F846FBBCAE11FC3F3588A7419C8786D5BF10536",
            },
        )
        self.assertTrue(manifest["checks"]["batch_game_root_trailing_backslash_fixed"])
        self.assertTrue(manifest["checks"]["powershell_utf8_bom_pinned"])
        self.assertTrue(manifest["qa"]["batch_apply_restore_smoke"])
        self.assertTrue(manifest["qa"]["powershell_korean_output_readable"])

    def test_the_game_and_static_exe_pins_are_unchanged_from_v0110(self) -> None:
        self.assertEqual(len(release.GAME_TARGETS), 15)
        self.assertEqual(release.GAME_TARGETS, previous_release.GAME_TARGETS)
        self.assertEqual(release.STATIC_EXE_PATCH, previous_release.STATIC_EXE_PATCH)
