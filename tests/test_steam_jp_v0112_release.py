...
PUBLIC_MANIFEST = ROOT / "data" / "public" / "steam_jp_117_v0112_release.v1.json"
...
release = load_module(
    "steam_jp_v0112_release", "tools/build_steam_jp_v0112_release.py"
)
previous_release = load_module(
    "steam_jp_v0111_release_for_v0112_test",
    "tools/build_steam_jp_v0111_release.py",
)


class SteamJpV0112ReleaseTests(unittest.TestCase):
    def make_fixture(
        self, root: Path
    ...
    def test_version_and_release_names_are_v0112(self) -> None:
        self.assertEqual(release.VERSION, "v0.11.2")
        self.assertIn("v0.11.2", release.ZIP_NAME)
        self.assertIn("v0.11.2", release.MANIFEST_NAME)
        self.assertTrue(release.SCHEMA.endswith("v0.11.2"))
    ...
    def test_public_manifest_matches_the_release_builder_contract(self) -> None:
        ...
        self.assertEqual(
            manifest["release_zip"],
            {
                ...
                "size": 374_140_796,
                "sha256": "C87D4E9C148D1C07108CA8272FD8E331055A96505D088C8AF78D754A48ED6D7D",
            },
        )
        self.assertTrue(all(manifest["checks"].values()))

    def test_wave5_game_profile_and_static_exe_pins_are_explicit(self) -> None:
        self.assertEqual(len(release.GAME_TARGETS), 15)
        self.assertEqual(set(release.GAME_TARGETS), set(previous_release.GAME_TARGETS))
        changed_game_resources = {
            path
        ...
        self.assertEqual(release.SUPPORT_TARGETS, previous_release.SUPPORT_TARGETS)
