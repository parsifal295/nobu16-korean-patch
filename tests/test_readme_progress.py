...
class ReadmeProgressTests(unittest.TestCase):
    ...
    def test_readme_documents_the_v0120_dynamic_map_labels(self) -> None:
        ...
        self.assertIn("### 정적 EXE 마스터 설치기", readme)
        self.assertIn("데이터 기반 레지스트리", readme)
        self.assertIn("`001` | 무장 편집기 한글 성명 검증", readme)
        self.assertIn("`002` | 가공 히메 한글 작명 검증", readme)
        self.assertIn("`003` | 한국어 상단 헤더 가로 배치", readme)
        self.assertIn("`004` | 지도 성·지명 가로쓰기와 동적 배경 라벨", readme)
        self.assertIn("`Applied` 또는 `Pending`", readme)
        self.assertIn("EXE를 다시 쓰지 않습니다", readme)
        self.assertIn("변조된 페이로드", readme)
        self.assertIn("RESTORE_ORIGINAL_NOBU16PK_EXE.bat", readme)
