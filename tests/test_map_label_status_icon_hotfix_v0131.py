...
class MapLabelStatusIconHotfixV0131Tests(unittest.TestCase):
    ...
    def test_pool_pairs_and_dynamic_width_source_are_exact(self) -> None:
        self.assertEqual(builder.POOL_HEADER_BASE_VA, 0x142252694)
        self.assertEqual(builder.LABEL_POOL_1_VA, 0x1422626CC)
        self.assertEqual(builder.SUPPLY_POOL_2_VA, 0x142272704)
        self.assertEqual(builder.LABEL_POOL_3_VA, 0x14228273C)
        self.assertEqual(builder.SUPPLY_POOL_4_VA, 0x142292774)
        self.assertEqual(builder.LABEL_POOL_PAIR_DELTA, 2 * builder.POOL_STRIDE)
    ...
    def test_wrapper_forwards_stack_args_and_updates_only_target_fields(self) -> None:
        ...
        self.assertLessEqual(len(injected), 428)
        self.assertEqual(labels["status_and_supply_wrapper"], code_va)
        ...
        # Dynamic rendered width, the single status slot, active-base guard,
        # and the paired supply parent X assignment must all be present.
        ...
        self.assertIn(bytes.fromhex("49 81 C0 38 00 01 00"), injected)
        self.assertIn(bytes.fromhex("F3 0F 11 40 08"), injected)

        # Pool records are matched by key type 2 and exact key pointer.
        self.assertGreaterEqual(injected.count(bytes.fromhex("41 83 7A 08 02")), 2)
        self.assertIn(bytes.fromhex("49 8B 52 10"), injected)
        self.assertIn(bytes.fromhex("49 39 52 10"), injected)
