...
def load_release_module():
    path = ROOT / "tools" / "build_steam_jp_v0110_release.py"
...
class StaticOfficerEditorInstallerTests(unittest.TestCase):
    ...
    def test_both_launchers_target_the_static_installer(self) -> None:
        apply = (ROOT / "release_payload" / "v0.11.0" / "APPLY_STATIC_OFFICER_EDITOR_FIX.bat").read_text(
            encoding="utf-8"
        )
        restore = (ROOT / "release_payload" / "v0.11.0" / "RESTORE_ORIGINAL_NOBU16PK_EXE.bat").read_text(
            encoding="utf-8"
        )
