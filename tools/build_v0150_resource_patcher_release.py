#!/usr/bin/env python3
"""Build the compact v0.15.0 Steam JP direct unified-patcher release ZIP.
...
rebuilds ten text resources from a frozen operation ledger, applies reviewed
image deltas, and translates whichever supported DLC records are installed.
"""
...
VERSION: Final = "v0.15.0"
ZIP_NAME: Final = "NOBU16_PK_Korean_Patch_Steam_1.1.7_v0.15.0.zip"
MANIFEST_NAME: Final = "release_manifest.v0.15.0.json"
LEDGER_MEMBER: Final = "patches/v0.15.0-direct-resource-patch.json.gz"
PATCHER_MEMBER: Final = "NOBU16_KR_RESOURCE_PATCHER.exe"
PROFILE_MEMBER: Final = "v0.15.0-resource-profile.json"
ZIP_TIMESTAMP: Final = (2026, 7, 22, 16, 0, 0)
SCHEMA: Final = "nobu16.kr.steam-jp-direct-resource-patcher.v0.15.0"
...
PATCHER_SOURCE: Final = PROJECT_ROOT / "tools" / "v0150_resource_patcher.py"
...
PATCHER = load_module("v0150_resource_patcher_release", PATCHER_SOURCE)
...
def read_binary_delta_payload(binary_patch_root: Path) -> dict[str, bytes]:
    """Read the exact bounded BSDIFF40 payload set for direct mode."""
...
def manifest_for(members: Mapping[str, bytes], ledger: Mapping[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "distribution": {
            "kind": "pristine-steam-jp-1.1.7-to-v0.15.0-direct-unified-patcher",
        ...
        "resource_patcher": {
            ...
            "optional_dlc_resource_count": len(PATCHER.OPTIONAL_RESOURCE_PATHS),
            "optional_dlc_resources": sorted(PATCHER.OPTIONAL_RESOURCE_PATHS),
            "full_preflight_resource_count": len(PATCHER.RESOURCE_PATHS),
            "changed_resources": list(PATCHER.RESOURCE_PATHS),
            "backup_directory": "KR_PATCH_BACKUP/v0.15.0-direct-patcher",
        ...
        "static_installer": {
            ...
            "castle_name_layout_profiles": {
                "1": {
                    "name": "Vertical",
                    "patch_ids": ["001", "002", "003", "007", "009", "010"],
                    "excluded_horizontal_patch_ids": ["004", "005", "006", "008"],
                    "output_sha256": "3964E160B789982E1E197F77CBA3F592AFF0144F063E7AFCAC4DCEB4C6C99CB4",
                },
                "2": {
                    "name": "Horizontal",
                    "patch_ids": [f"{index:03d}" for index in range(1, 11)],
                    "excluded_horizontal_patch_ids": [],
                    "output_sha256": "C1E9123539506055C1ACB96A15A446C43952AED607DEA2C9646F690813FA53D5",
                },
            },
        },
        "unified_patcher": {
            ...
            "interactive_castle_name_layout_choice": True,
            "castle_name_layout_choice_values": {"1": "Vertical", "2": "Horizontal"},
            "missing_dlc_policy": "skip; rerun after installing DLC to patch newly present files",
        },
