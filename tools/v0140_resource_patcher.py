#!/usr/bin/env python3
"""Build and apply the v0.14.0 Steam JP direct resource patch.

The public patch ledger rebuilds all text resources from pristine Steam JP
1.1.7 inputs and applies bounded BSDIFF40 deltas for the five large font and
image archives. It never distributes a complete game resource, verifies every
...
LEDGER_FILENAME = "v0.14.0-direct-resource-patch.json.gz"
...
def ensure_backups(game_root: Path, ledger: Mapping[str, Any]) -> None:
    for resource in ensure_list(ledger.get("resources"), "resources"):
        item = ensure_mapping(resource, "resource")
        relative, predecessor, _target = parse_resource_specs(item)
...
def restore_patch(
    ...
    *,
    ...
    if not retained_prechecked:
        assert_retained_resources(game_root, ledger)
    state = installed_state(game_root, ledger)
    if set(state.values()) == {"predecessor"}:
        return {"result": "already_predecessor", "writes_performed": False, "state": state}
    if set(state.values()) != {"target"}:
        raise PatcherError(f"restore requires a complete target vector: {state}")
    require_stopped_game()
    paths = sorted(state)
    ...
    try:
        ...
        final = installed_state(game_root, ledger)
        if set(final.values()) != {"predecessor"}:
        ...
        if rollback_errors or set(final.values()) != {"target"}:
            raise PatcherError(
...
def parse_binary_patch_descriptor(value: object, relative: str) -> dict[str, object]:
    ...
    if (
        ...
        or int(spec["size"]) * MAX_BINARY_PATCH_FRACTION_DENOMINATOR
...
def build_ledger(
    ...
    """Create the frozen direct ledger from pristine inputs and v0.14 targets."""
...
def build_candidates(
    game_root: Path, ledger: Mapping[str, Any], *, patch_root: Path | None = None
) -> dict[str, bytes]:
...
def backup_root(game_root: Path) -> Path:
    return game_root / "KR_PATCH_BACKUP" / "v0.14.0-direct-patcher"
...
def apply_patch(
    ...
    *,
    ...
    if values == {"target"}:
        return {"result": "already_target", "writes_performed": False, "state": state}
    if values != {"predecessor"}:
        raise PatcherError(f"apply requires a complete pristine Steam JP vector: {state}")
    require_stopped_game()
    candidates = build_candidates(game_root, ledger, patch_root=patch_root)
    ensure_backups(game_root, ledger)
    paths = sorted(candidates)
    write_state(game_root, ledger_hash, "applying", paths)
    replaced: list[str] = []
        ...
        if args.preflight:
            ...
            if values == {"target"}:
                ...
                for resource in ensure_list(ledger.get("resources"), "resources"):
                    ...
                    if item.get("kind") == "binary_bsdiff40":
                        load_binary_patch(patch_root, item, relative)
                result = "ready"
            else:
                raise PatcherError(
                    "preflight requires a complete pristine Steam JP vector or "
                    f"the complete v0.14.0 target vector: {state}"
                )
            print_json(
                {
                    **report,
