...
$probeRoot = Join-Path $workRoot 'tmp\castle_hangul_orientation_probe'
$backupRoot = Join-Path $workRoot 'backups\castle_hangul_orientation_probe'
...
$targets = @(
    [ordered]@{
        ...
        probe_hash = '2C674DCF1F99EB5CC88820B1E78B67DEC2FCACCA637685367FCAADEAD98E99B2'
        probe = Join-Path $probeRoot 'msgdata_probe.bin'
    ...
    [ordered]@{
        ...
        probe_hash = '3BC57379D9AF95E83A77C96C1EE2D104AAF4A8BEA1733EA33FC3D1BCF056D1A9'
        probe = Join-Path $workRoot 'workstreams\msgui_full\font_v4\build\private\candidate\res_lang.SC.font-v4.bin'
...
function Write-Journal([string]$Phase, [int]$Completed) {
    $record = [ordered]@{
        ...
        purpose = 'castle-hangul-orientation-runtime-probe'
...
function Invoke-AtomicReplace(
    ...
    $stage = $Target + '.n16kr.castle.new'
    $swap = $Target + '.n16kr.castle.swap.bak'
...
function Show-Status {
    [pscustomobject]@{
        ...
        launches_game = $false
        test_castle_id = 9168
        test_text = [string]::Concat([char]0xC624, [char]0xB2E4, [char]0xC131)
...
try {
    ...
    if ($Action -eq 'Restore') {
        ...
        for ($index = $targets.Count - 1; $index -ge 0; $index--) {
            ...
            Write-Journal 'restoring' ($targets.Count - $index)
        }
    ...
    try {
        ...
        Write-Journal 'stock-after-rollback' $completed
        throw
    }
