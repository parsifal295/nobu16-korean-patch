#!/usr/bin/env python3
"""Build and apply the v0.15.0 Steam JP direct resource patch.
...
LEDGER_FILENAME = "v0.15.0-direct-resource-patch.json.gz"
...
PROFILE_FILENAME = "v0.15.0-resource-profile.json"
...
def _load_release_profile() -> Mapping[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"cannot load the v0.15.0 resource profile: {path}") from exc
    if not isinstance(value, Mapping):
        raise RuntimeError("v0.15.0 resource profile is not an object")
    expected_keys = {
        "schema",
        "version",
        "text_resource_paths",
    ...
    if set(value) != expected_keys:
        raise RuntimeError("v0.15.0 resource profile keys differ")
    if value.get("schema") != "nobu16.kr.resource-profile.v0.15.0" or value.get("version") != VERSION:
        raise RuntimeError("v0.15.0 resource profile identity differs")
    return value
...
def _profile_pin_map(value: object, paths: Sequence[str], label: str) -> dict[str, tuple[int, str]]:
    if not isinstance(value, Mapping) or set(value) != set(paths):
        raise RuntimeError(f"v0.15.0 {label} profile paths differ")
    result: dict[str, tuple[int, str]] = {}
    for relative in paths:
        spec = value[relative]
        if not isinstance(spec, Mapping) or set(spec) != {"size", "sha256"}:
            raise RuntimeError(f"v0.15.0 {label} profile is malformed: {relative}")
        size = spec.get("size")
        ...
        try:
            ...
            raise RuntimeError(f"v0.15.0 {label} profile hash is malformed: {relative}") from exc
...
if len(set(RESOURCE_PATHS)) != len(RESOURCE_PATHS):
    raise RuntimeError("v0.15.0 resource profile contains duplicate paths")
...
if not OPTIONAL_RESOURCE_PATHS.issubset(BINARY_RESOURCE_PATHS):
    raise RuntimeError("v0.15.0 optional-resource paths differ")
...
if set(RESOURCE_KINDS) != set(RESOURCE_PATHS):
    raise RuntimeError("v0.15.0 resource-kind paths differ")
EXPECTED_OPERATION_COUNTS = {
...
if set(EXPECTED_OPERATION_COUNTS) != set(TEXT_RESOURCE_PATHS):
    raise RuntimeError("v0.15.0 text operation-count paths differ")
...
def backup_root(game_root: Path) -> Path:
    return game_root / "KR_PATCH_BACKUP" / "v0.15.0-direct-patcher"
