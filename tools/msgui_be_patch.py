...
def is_ascii_printable_code(code: int) -> bool:
    return code in (0x09, 0x0A, 0x0D) or (0x20 <= code <= 0x7E)


def decode_utf16be_z(data: bytes, offset: int, max_bytes: int | None = None) -> tuple[str, int, bool]:
    ...
    while pos + 1 < end:
        ...
        if not is_ascii_printable_code(code):
...
def quality_ok(text: str, min_chars: int) -> bool:
    ...
    return has_alpha or has_placeholder


def extract_entries(data: bytes, min_chars: int, start: int, end: int) -> list[Entry]:
    hits: list[Entry] = []
    ...
    while i + 1 < end:
        text, used, terminated = decode_utf16be_z(data, i, max_bytes=end - i)
        if terminated and used >= 4 and quality_ok(text, min_chars):
            hits.append(
                Entry(
                    id=0,
                    offset=i,
...
def load_csv_entries(path: Path) -> list[Entry]:
    out: list[Entry] = []
...
def validate_entries(entries: list[Entry]) -> tuple[list[str], list[str], int]:
    errors: list[str] = []
    warnings: list[str] = []
    targets = 0
    ...
    for e in entries:
        ...
        if len(enc) > e.allocated_bytes:
            errors.append(
                f"id={e.id} offset=0x{e.offset:X}: encoded {len(enc)} > allocated {e.allocated_bytes}"
            )

        ph_src = placeholder_list(e.original_en)
        ph_dst = placeholder_list(t)
        if ph_src != ph_dst:
            warnings.append(
                f"id={e.id} offset=0x{e.offset:X}: placeholder mismatch src={ph_src} dst={ph_dst}"
            )
...
def inject(
    ...
    for e in entries:
        ...
        if strict_original:
            cur, used, terminated = decode_utf16be_z(data, e.offset, max_bytes=e.allocated_bytes)
            if not terminated:
                raise ValueError(f"id={e.id} offset=0x{e.offset:X}: no terminator inside allocated bytes")
            if cur != e.original_en:
                raise ValueError(
                    f"id={e.id} offset=0x{e.offset:X}: original mismatch current={cur!r} csv={e.original_en!r}"
                )
            if used > e.allocated_bytes:
                raise ValueError(f"id={e.id} offset=0x{e.offset:X}: used {used} > allocated {e.allocated_bytes}")
...
def cmd_extract(args: argparse.Namespace) -> int:
    ...
    entries = extract_entries(data, args.min_chars, start, end)
...
def cmd_validate(args: argparse.Namespace) -> int:
    entries = load_csv_entries(Path(args.input_csv))
    errors, warnings, targets = validate_entries(entries)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="msgui.bin UTF-16BE extractor/injector MVP")
    sp = p.add_subparsers(dest="cmd", required=True)

    p_ex = sp.add_parser("extract", help="Extract UTF-16BE null-terminated candidates")
    p_ex.add_argument("--input", required=True)
    p_ex.add_argument("--out-csv", required=True)
    p_ex.add_argument("--out-summary", required=True)
    p_ex.add_argument("--min-chars", type=int, default=2)
    p_ex.add_argument("--start", type=int, default=0)
    p_ex.add_argument("--end", type=int)
    p_ex.set_defaults(func=cmd_extract)

    p_va = sp.add_parser("validate", help="Validate translation CSV")
    p_va.add_argument("--input-csv", required=True)
    p_va.set_defaults(func=cmd_validate)

    p_in = sp.add_parser("inject", help="Inject translated text into a copied msgui.bin")
