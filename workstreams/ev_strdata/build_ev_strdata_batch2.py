#!/usr/bin/env python3
"""Build source-free MSG/SC/ev_strdata officer-name batch2 artifacts."""
...
BATCH_ID = "ev-strdata-officer-names-0150-0349-v0.2"
OVERLAY_NAME = "ev_strdata_ko_officer_names_0150_0349.v0.2.json"
...
def build_once(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    expected_ids = list(range(SCOPE_START, SCOPE_END + 1))
    if ids != expected_ids:
    ...
    evidence = {
        "schema": "nobu16.kr.ev-strdata-alignment-evidence.v2",
        "batch_id": BATCH_ID,
        "resource": "ev_strdata",
        "scope": {
    ...
    review = {
        "schema": "nobu16.kr.ev-strdata-review-index.v2",
        "batch_id": BATCH_ID,
    ...
    validation = {
        "schema": "nobu16.kr.ev-strdata-generation-validation.v2",
        "batch_id": BATCH_ID,
        "passed": True,
        "scope": {
            "start_id": SCOPE_START,
...
def build_reproducibly(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nobu16-evstr2-a-") as first_tmp:
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr2-b-") as second_tmp:
