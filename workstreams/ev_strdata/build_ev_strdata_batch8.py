#!/usr/bin/env python3
"""Build source-free MSG/SC/ev_strdata officer-name batch8 artifacts."""
...
BATCH_ID = "ev-strdata-officer-names-1350-1549-v0.8"
OVERLAY_NAME = "ev_strdata_ko_officer_names_1350_1549.v0.8.json"
...
# No source spelling differs from the pinned officer-name seed in this batch.
# Keep the independent branch explicit so future batches cannot silently reuse a
# translation when an SC hash differs.
...
def configured_engine() -> Iterator[None]:
    """Run the reviewed v0.4 mechanics with only v0.8 batch values changed."""
...
def upgrade_v08_metadata(out_root: Path) -> None:
    ...
    evidence["schema"] = "nobu16.kr.ev-strdata-alignment-evidence.v8"
    ...
    review["schema"] = "nobu16.kr.ev-strdata-review-index.v8"
    ...
    validation["schema"] = "nobu16.kr.ev-strdata-generation-validation.v8"
    safety = validation["safety"]
    safety.pop("existing_v01_v02_v03_artifacts_modified", None)
    safety["existing_v01_v02_v03_v04_v05_v06_v07_artifacts_modified"] = False
    ...
    if any(
        ...
        raise shared.EvStrDataError(
            "v0.8 public artifact contains source script text or an embedded NUL"
        )
...
def build_reproducibly(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nobu16-evstr8-a-") as first_tmp:
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr8-b-") as second_tmp:
