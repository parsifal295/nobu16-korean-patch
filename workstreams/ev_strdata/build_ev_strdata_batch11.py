#!/usr/bin/env python3
"""Build source-free MSG/SC/ev_strdata officer-name batch11 artifacts."""
...
BATCH_ID = "ev-strdata-officer-names-1950-2149-v0.11"
OVERLAY_NAME = "ev_strdata_ko_officer_names_1950_2149.v0.11.json"
...
def configured_engine() -> Iterator[None]:
    """Run the reviewed v0.4 mechanics with only v0.11 batch values changed."""
...
def upgrade_v011_metadata(out_root: Path) -> None:
    ...
    evidence["schema"] = "nobu16.kr.ev-strdata-alignment-evidence.v11"
    ...
    review["schema"] = "nobu16.kr.ev-strdata-review-index.v11"
    ...
    validation["schema"] = "nobu16.kr.ev-strdata-generation-validation.v11"
    ...
    if any(
        ...
        raise shared.EvStrDataError(
            "v0.11 public artifact contains source script text or an embedded NUL"
        )
...
def build_once(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    upgrade_v011_metadata(out_root)
    return {
...
def build_reproducibly(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nobu16-evstr11-a-") as first_tmp:
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr11-b-") as second_tmp:
