#!/usr/bin/env python3
"""Build source-free MSG/SC/ev_strdata officer-name batch7 artifacts."""
...
BATCH_ID = "ev-strdata-officer-names-1150-1349-v0.7"
OVERLAY_NAME = "ev_strdata_ko_officer_names_1150_1349.v0.7.json"
...
def configured_engine() -> Iterator[None]:
    """Run the reviewed v0.4 mechanics with only v0.7 batch values changed."""
...
def upgrade_v07_metadata(out_root: Path) -> None:
    ...
    evidence["schema"] = "nobu16.kr.ev-strdata-alignment-evidence.v7"
    ...
    review["schema"] = "nobu16.kr.ev-strdata-review-index.v7"
    ...
    validation["schema"] = "nobu16.kr.ev-strdata-generation-validation.v7"
    safety = validation["safety"]
    safety.pop("existing_v01_v02_v03_artifacts_modified", None)
    safety["existing_v01_v02_v03_v04_v05_v06_artifacts_modified"] = False
    ...
    if any(
        ...
        raise shared.EvStrDataError(
            "v0.7 public artifact contains source script text or an embedded NUL"
        )
...
def build_reproducibly(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nobu16-evstr7-a-") as first_tmp:
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr7-b-") as second_tmp:
