#!/usr/bin/env python3
"""Build source-free MSG/SC/ev_strdata officer-name batch12 artifacts."""
...
BATCH_ID = "ev-strdata-officer-names-2150-2206-v0.12"
OVERLAY_NAME = "ev_strdata_ko_officer_names_2150_2206.v0.12.json"
...
def configured_engine() -> Iterator[None]:
    """Run the reviewed v0.4 mechanics with only v0.12 batch values changed."""
...
def upgrade_v012_metadata(out_root: Path) -> None:
    ...
    evidence["schema"] = "nobu16.kr.ev-strdata-alignment-evidence.v12"
    ...
    review["schema"] = "nobu16.kr.ev-strdata-review-index.v12"
    ...
    validation["schema"] = "nobu16.kr.ev-strdata-generation-validation.v12"
    ...
    if any(
        ...
        raise shared.EvStrDataError(
            "v0.12 public artifact contains source script text or an embedded NUL"
        )
...
def build_once(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    upgrade_v012_metadata(out_root)
    return {
...
def build_reproducibly(game_root: Path, out_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="nobu16-evstr12-a-") as first_tmp:
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr12-b-") as second_tmp:
