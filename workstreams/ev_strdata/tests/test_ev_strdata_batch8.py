...
import build_ev_strdata_batch8 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "23FA0714F7066682536F618205217C50011D5D316C89706F33175DAF223C1E59"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "0D853A54D9AB00AE138F8F6C05A4C07C9920272BC5EE96268781068400A58EA6"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "BD94436E55C1AA2B40B9DD4131588855171DC05E5C061C2F0F4C01F3F8755F87"
    ),
    batch.VALIDATION_NAME: (
        "790E157B3537043590FD626F0A259D70A6A3E1943B4ED0D0B4C80D831EDF9AE2"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch8Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_1350_through_1549(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(1350, 1550)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(1550, self.evidence["scope"]["next_start_id"])
        self.assertEqual(1550, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v08_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_validation_records_invariants_determinism_and_safety(self) -> None:
        ...
        for key in (
            ...
            "executable_modified",
            "registry_modified",
            "existing_v01_v02_v03_v04_v05_v06_v07_artifacts_modified",
        ):
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr8-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(1550, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
