...
import build_ev_strdata_batch5 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "3715610AC5B0C78988878BE80171B43722F64255EE9B686337EE5A75FE948544"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "808CBE9D218FF5D785C247303C3FEB8AA684E04C6ACF8B87BC833E3B3293E6B2"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "F03D877E23658721FAEE08B32AE479CFDF0BAAEA360B5A7EBBF309AF037694FE"
    ),
    batch.VALIDATION_NAME: (
        "9A07E26D905715FB1B21F30C90F9F6A88AF46B476368A0AADC8D12B5C86B00B0"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch5Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_750_through_949(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(750, 950)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(950, self.evidence["scope"]["next_start_id"])
        self.assertEqual(950, self.validation["scope"]["next_start_id"])
    ...
    def test_alignment_and_hash_matched_reuse_are_complete(self) -> None:
    ...
    def test_public_v05_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr5-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(950, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
