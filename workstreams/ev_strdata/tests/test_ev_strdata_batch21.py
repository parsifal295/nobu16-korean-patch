...
import build_ev_strdata_batch21 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "445DDB4E6D9904C911E87DB204B06F6E895B77A5BC352ADFE1FF91C23D1BEAF9"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "5D15C7BB281D512DC2F9FDC92FCDFE0B556BCE69C9723188BE7595766E9EDD58"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "08CBE3DF0E8965DC97AD05D08D68F72A54887DD9A030609BF495E23ED5C6B687"
    ),
    batch.VALIDATION_NAME: (
        "3ADCFE7F59A34D9124CC7FB1EBFED48B3663384D16914EDB763C8920BAC9C0B3"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch21Tests(unittest.TestCase):
    @classmethod
    ...
    def test_overlay_contains_exact_178_historical_event_entries(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(4032, 4210)), ids)
        self.assertEqual(178, self.overlay["entry_count"])
        by_id = {int(entry["id"]): entry["ko"] for entry in self.overlay["entries"]}
        self.assertIn("\uac04\ud1a0 \uac04\ub808\uc774", by_id[4032])
        self.assertIn("\ubb34\ub77c\uce74\ubbf8 \uc694\uc2dc\ud0a4\uc694", by_id[4056])
        self.assertIn("\uc2dc\ub098\ub178 \ubd81\ubd80", by_id[4088])
        self.assertIn("\uc624\ub2e4 \ub178\ubd80\ub098\uac00", by_id[4105])
        self.assertIn("\ub370\ub8e8\ud1a0\ub77c", by_id[4128])
        self.assertIn("\uc544\uc0ac\ucfe0\ub77c \uc18c\ud14c\ud0a4", by_id[4139])
        self.assertIn("\uc544\ub9c8\uace0 \uac00\ubb38", by_id[4161])
        self.assertIn("\uc2a4\uc5d0 \ud558\ub8e8\uce74\ud0c0", by_id[4180])

    def test_scope_has_no_non_display_exclusions(self) -> None:
        self.assertEqual(set(range(4032, 4210)), set(batch.TRANSLATIONS))
    ...
    def test_review_marks_layout_and_selected_historical_terms(self) -> None:
        self.assertEqual(178, len(self.review["entries"]))
        self.assertEqual(23, self.review["terminology_review_count"])
        flagged = set()
    ...
    def test_overlay_is_source_free_common_message_contract(self) -> None:
        ...
        self.assertEqual(178, len(entries))
    ...
    def test_alignment_and_translation_hash_sets_are_pinned(self) -> None:
        ...
        self.assertEqual(534, alignment["translated_reference_hash_count"])
        self.assertEqual(178, alignment["translated_ids_nonempty_in_all_references"])
    ...
    def test_v021_generated_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_exactly_seven_new_batch_files_are_source_free(self) -> None:
        paths = (
            WORKSTREAM_ROOT / "build_ev_strdata_batch21.py",
            WORKSTREAM_ROOT / "BATCH21_V0.21_README_KO.md",
            ...
            WORKSTREAM_ROOT / "tests" / "test_ev_strdata_batch21.py",
        )
    ...
    def test_validation_records_determinism_invariants_and_file_only_safety(self) -> None:
        ...
        self.assertEqual(178, self.validation["replacement_invariants"]["checked"])
        ...
        self.assertEqual(178, self.validation["offline_binary_build"]["operations"])
        self.assertEqual(178, self.validation["offline_binary_build"]["overlay_entries"])
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        for language in shared.LANGUAGES:
            self.assertEqual(
                ...
                common.text_hash(loaded[language]["table"].texts[4210]),
            )
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr21-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(178, result["entry_count"])
            self.assertEqual(178, result["inspected_count"])
            self.assertEqual(0, result["deferred_count"])
            self.assertEqual(4210, result["next_display_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
