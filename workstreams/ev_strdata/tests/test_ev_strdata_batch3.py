...
import build_ev_strdata_batch3 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "12CFC136A5018DA4FDD5763189D05B1E41EAC2AAB7EA56383BDB493B2EDC5651"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "A41AC6FD31B3977E449AF973CEB46A9E21A0C6E89D6F4C460A1B383211BE3F34"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "3B65BF12C412860D21A89459EFA85AEC6F391F1BE53B4A4A10257A463FFCFA09"
    ),
    batch.VALIDATION_NAME: (
        "14AD368FDD059EC5EFD5DE26128B709058AF85A71132D776EEA1C4A003EFD48F"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch3Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_350_through_549(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(350, 550)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(550, self.evidence["scope"]["next_start_id"])
        self.assertEqual(550, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v03_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr3-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(550, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
