...
import build_ev_strdata_batch9 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "A80887B06240CCB3B2A0EBD28C40A3ECDC194C9B05B9E090FFCF93E1BF9D9A4C"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "065B9D0DCE3E341180556FA6B21186C3184E3CDE65099D1BEE1D068BC85F10BE"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "46941FD894058D4E47F0363C1F68BCA90F65A58C8F96332FA3BCDD38BA7C56A0"
    ),
    batch.VALIDATION_NAME: (
        "3E68ABA39079AD11E6E1B203EFD876F910019C9E32D9DA73BAF1E765A9D82A2B"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch9Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_1550_through_1749(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(1550, 1750)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(1750, self.evidence["scope"]["next_start_id"])
        self.assertEqual(1750, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v09_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_validation_records_invariants_determinism_and_safety(self) -> None:
        ...
        for key in (
            ...
            "executable_modified",
            "registry_modified",
            "existing_v01_v02_v03_v04_v05_v06_v07_v08_artifacts_modified",
        ):
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr9-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(1750, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
