...
import build_ev_strdata_batch10 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "E25247986B42AA0D4E57034531FF9167A9AD7F581AE1E0A2F0380EB720594AA7"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "C7C2EBF4D19AA3BE376FCEF7ED1A0845DB7DB2F0A0BE5D741764357A82ED5C91"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "DFAFE7A6CC9D960DAAE4ECC62FC1D714A68EC31E971B4EF085189CD97469B6F9"
    ),
    batch.VALIDATION_NAME: (
        "DB4AC362C04AED0926D3DE31032A85623FB64C57047873437F68A0FEDBBB48A7"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch10Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_1750_through_1949(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(1750, 1950)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(1950, self.evidence["scope"]["next_start_id"])
        self.assertEqual(1950, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v010_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr10-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(1950, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
