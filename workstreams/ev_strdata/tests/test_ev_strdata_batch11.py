...
import build_ev_strdata_batch11 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "E3D6F0BC4D570C7DD13596893E840C8F7B5FA97DA99A755BA2E370D9B9F5F4FC"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "E803DD9290889E385EB0047C4AB6196C0C88C9C57C94B755008DD2A6B7896CCC"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "2C0B56399381420388944BDA2EC9BC6F7C9492FA71092F9230A71D1FA3D3AAEB"
    ),
    batch.VALIDATION_NAME: (
        "AF7F0669720A264A055602036E57D1D7CD8EB7A04440833623279D517A0EBFFA"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch11Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_1950_through_2149(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(1950, 2150)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(2150, self.evidence["scope"]["next_start_id"])
        self.assertEqual(2150, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v011_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_validation_records_invariants_determinism_and_safety(self) -> None:
        ...
        for key in (
            ...
            "executable_modified",
            "registry_modified",
            "existing_v01_v02_v03_v04_v05_v06_v07_v08_v09_v010_artifacts_modified",
        ):
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr11-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(2150, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
