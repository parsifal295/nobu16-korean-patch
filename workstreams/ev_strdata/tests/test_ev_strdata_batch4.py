...
import build_ev_strdata_batch4 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "A424DC8D096E9650BF7D01D96D2F55AAA252D0B3A4F6CB572CE707EB8C063857"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "7C5ACF8EE85EB750400C2E3227F342200AABFA053D737E0079A9F3DC87B62686"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "86696478352CB259135A2C64C0311097FD835DC2BA3C24953EE8DA79CD53AE1D"
    ),
    batch.VALIDATION_NAME: (
        "BF3C02E8BEB44F98BA55A127D07BD3D028359A71C862D50C252A513E350C332D"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch4Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_ids_550_through_749(self) -> None:
        ids = [int(entry["id"]) for entry in self.overlay["entries"]]
        self.assertEqual(list(range(550, 750)), ids)
        self.assertEqual(200, self.overlay["entry_count"])
        self.assertEqual(750, self.evidence["scope"]["next_start_id"])
        self.assertEqual(750, self.validation["scope"]["next_start_id"])
    ...
    def test_public_v04_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr4-test-") as temporary:
            result = batch.build_reproducibly(GAME_ROOT, Path(temporary))
            self.assertEqual(200, result["entry_count"])
            self.assertEqual(750, result["next_start_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
