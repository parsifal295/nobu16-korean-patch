...
import build_ev_strdata_batch19 as batch  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "79BD5681444089F6EDD63935D3875702A8FC616479317C86B5B9C72336E06918"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "E5C8FBD5B736B4682E47C7088EDEFC800CB91CDE124A0CD75D9895AD1EDB47F2"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "70688D09634932283606A4A2F55E8140AECA8FFA0B39A9F50C2DC5E5440B8497"
    ),
    batch.VALIDATION_NAME: (
        "D2CE8065733C6C621C82CC47B60DEDAC7B5D0FF5E6A4B220F9BE8C4078340CFB"
    ),
}
STOCK_AVAILABLE = all(
...
class EvStrDataBatch19Tests(unittest.TestCase):
    @classmethod
    ...
    def test_overlay_contains_exact_178_historical_event_entries(self) -> None:
        ...
        self.assertEqual(list(range(3662, 3840)), ids)
        self.assertEqual(178, self.overlay["entry_count"])
        by_id = {int(entry["id"]): entry["ko"] for entry in self.overlay["entries"]}
        self.assertIn("모리 다카모토", by_id[3662])
        self.assertIn("가와고에 야전", by_id[3706])
        self.assertIn("깃포시", by_id[3707])
        self.assertIn("무로마치 막부 제13대 쇼군", by_id[3715])
        self.assertIn("기쓰노", by_id[3761])
        self.assertIn("나가오", by_id[3767])
        self.assertIn("라이키리마루", by_id[3815])
        self.assertIn("미요시", by_id[3820])

    def test_scope_has_no_non_display_exclusions(self) -> None:
        self.assertEqual(set(range(3662, 3840)), set(batch.TRANSLATIONS))
        ...
        self.assertEqual(3840, self.evidence["scope"]["next_display_id"])
    ...
    def test_repeated_sc_source_uses_one_korean_translation(self) -> None:
        ...
        self.assertEqual([[3794, 3837]], policy["repeated_source_id_groups"])
        self.assertEqual(0, policy["failures"])
        by_id = {int(entry["id"]): entry["ko"] for entry in self.overlay["entries"]}
        self.assertEqual(by_id[3794], by_id[3837])
    ...
    def test_related_msgev_reuse_and_sc_adjustments_are_recorded(self) -> None:
        ...
        self.assertEqual(3, related["sc_structure_adjusted_entry_count"])
        self.assertEqual(
            [3689, 3694, 3767],
            related["ev_strdata_sc_color_structure_differs_at_ids"],
        )
    ...
    def test_v019_generated_files_are_pinned_and_source_free(self) -> None:
    ...
    def test_exactly_seven_new_batch_files_are_source_free(self) -> None:
        paths = (
            WORKSTREAM_ROOT / "build_ev_strdata_batch19.py",
            WORKSTREAM_ROOT / "BATCH19_V0.19_README_KO.md",
            ...
            WORKSTREAM_ROOT / "tests" / "test_ev_strdata_batch19.py",
        )
    ...
    def test_validation_records_determinism_invariants_and_file_only_safety(self) -> None:
        ...
        self.assertEqual(177, self.validation["offline_binary_build"]["operations"])
    ...
    def test_stock_replay_is_deterministic_and_install_is_unchanged(self) -> None:
        ...
        for language in shared.LANGUAGES:
            self.assertEqual(
                ...
                common.text_hash(loaded[language]["table"].texts[3840]),
            )
        with tempfile.TemporaryDirectory(prefix="nobu16-evstr19-test-") as temporary:
            ...
            self.assertEqual(3840, result["next_display_id"])
            self.assertEqual(
                EXPECTED_HASHES,
                {
