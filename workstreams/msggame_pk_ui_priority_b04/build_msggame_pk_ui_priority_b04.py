#!/usr/bin/env python3
"""Build source-free PK msggame UI-priority batch B04."""
...
BATCH_ID = "msggame_pk_ui_priority_b04_250.v1"
RESOURCE = "MSG_PK/SC/msggame.bin"
OVERLAY_NAME = "msggame_ko_pk_ui_priority_b04_250.v1.json"
EVIDENCE_NAME = "msggame_pk_ui_priority_b04_evidence.v1.json"
REVIEW_NAME = "msggame_pk_ui_priority_b04_review.v1.json"
VALIDATION_NAME = "msggame_pk_ui_priority_b04_validation.v1.json"
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    allowed_blocks = {6, 7, 8, 9, 12, 13, 14, 15, 17}
    if any(coordinate[0] not in allowed_blocks for coordinate in TRANSLATIONS):
        raise BatchError("B04 translation escaped the reviewed UI/system-message blocks")

    source_paths = {
        "SC": args.pk_sc.resolve(),
        "JP": args.pk_jp.resolve(),
        "EN": args.pk_en.resolve(),
        "TC": args.pk_tc.resolve(),
    }
    ...
    for coordinate in sorted(TRANSLATIONS):
        ...
        for label in ("SC", "JP", "EN", "TC"):
            ...
            if literal is not None:
                ...
                official_widths[label] = []
                official_hashes[label] = None
        if record_counts["SC"] < 1:
            raise BatchError(f"selected coordinate lacks SC record context: {coordinate}")
        ...
        width_limit = max(max(widths) for widths in official_widths.values() if widths) + 12
    ...
    overlay = {
        ...
        "translation_provenance": {
            ...
            "kind": "agent_ui_priority_help_prompt_and_system_message_translation",
            "runtime_reviewed": False,
            "source_text_embedded": False,
        },
        "selection_policy": {
            "priority": "block14_help_then_tutorial_confirmation_and_battle_system_ui",
    ...
    evidence_entries = [
        {
            ...
            "record_id": item["coordinate"][1],
            "literal_id": item["coordinate"][2],
            "review_method": "pk_sc_jp_en_tc_ui_context_review",
            ...
            "pk_tc_utf16le_sha256": item["official_hashes"]["TC"],
            ...
            "static_help_or_independent_ui_literal": True,
        }
        for item in selected
    ]
    evidence = {
        ...
        "source_free": True,
        "scope": {
            "selected_blocks": sorted({coordinate[0] for coordinate in TRANSLATIONS}),
            "selected_count": len(selected),
            "review_exclusion_count": len(EXCLUSIONS),
            "selection": "help_tutorial_confirmation_battle_status_and_management_ui",
        },
        "target_catalog": {
        ...
        "overlap_checks": {
            ...
            "outside_reviewed_ui_blocks_count": 0,
        },
    ...
    review = {
        ...
        "batch_id": BATCH_ID,
        "quality_state": "agent_ui_priority_help_and_complete_system_messages_translated",
        "selected_count": len(selected),
    ...
    progress_payload = read_json(args.progress.resolve())
    progress_item = next(
        item for item in progress_payload["resources"] if item.get("path") == RESOURCE
    )
    full_overlay_coordinates: set[tuple[int, int, int]] = set()
    full_entries: list[dict[str, Any]] = []
    for pattern in progress_item["overlay_globs"]:
        if pattern == SELF_RELATIVE:
            continue
        matches = sorted(REPO_ROOT.glob(pattern))
        if len(matches) != 1:
            raise BatchError(f"full candidate pattern {pattern!r} did not resolve uniquely")
        payload = read_json(matches[0])
        coordinates = overlay_coordinates(matches[0])
        if coordinates & full_overlay_coordinates:
            raise BatchError("registered overlays overlap during full candidate reconstruction")
        full_overlay_coordinates.update(coordinates)
        full_entries.extend(payload["entries"])
    full_entries.extend(overlay["entries"])
    combined_overlay = dict(overlay)
    combined_overlay["overlay_id"] = BATCH_ID + ".full-candidate"
    combined_overlay["entries"] = full_entries
    combined_overlay["entry_count"] = len(full_entries)
    full_candidate, full_manifest = apply_overlay_blob(stock["packed"], combined_overlay)
    full_overlay_coordinates.update(selected_set)
    full_literals = literal_map(parse_packed_msggame(full_candidate).archive)
    if set(full_literals) != set(stock["literals"]):
        raise BatchError("full candidate changed literal coordinates")
    for coordinate, source_literal in stock["literals"].items():
        if coordinate not in full_overlay_coordinates and full_literals[coordinate].text != source_literal.text:
            raise BatchError(f"full candidate changed non-overlay coordinate {coordinate}")
    for item in selected:
        if full_literals[item["coordinate"]].text != item["replacement"]:
            raise BatchError(f"full candidate lost B04 replacement at {item['coordinate']}")
    ...
    validation = {
        ...
        "proofs": {
            "static_ui_literal_with_pk_multilingual_record_context": True,
        ...
        "full_candidate_validation": {
            "registered_predecessor_entry_count": len(full_overlay_coordinates - selected_set),
            "b04_entry_count": full_manifest["entry_count"],
            "target_packed_size": len(full_candidate),
            "target_packed_sha256": sha256(full_candidate),
            "literal_coordinates_preserved": True,
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--pk-tc", type=Path, default=DEFAULT_PK_TC)
