...
import build_msggame_pk_ui_priority_b03 as builder  # noqa: E402
import build_full_pk_message_candidate as integration  # noqa: E402


class MsgGamePkUiPriorityB03Tests(unittest.TestCase):
    ...
    def test_scope_and_disjointness(self) -> None:
        self.assertEqual(len(builder.TRANSLATIONS), 220)
        self.assertTrue(all(coordinate[0] == 14 for coordinate in builder.TRANSLATIONS))
    ...
    def test_overlay_passes_full_candidate_parser(self) -> None:
        ...
        parsed = integration.parse_msggame_overlay(payload, "b03")
        self.assertEqual(len(parsed["entries"]), 220)
        self.assertEqual(
            set(payload),
            {
