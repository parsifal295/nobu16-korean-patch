#!/usr/bin/env python3
"""Build the source-free Steam JP msggame wave07 j02 overlay."""
...
from legacy_seed import LEGACY_COORDINATE_KO  # noqa: E402
from manual_records import MANUAL_RECORDS  # noqa: E402
from nobu16_lz4 import decompress_wrapper  # noqa: E402
from review_records import CANONICAL_REFERENCE_KO, REVIEWED_RECORDS  # noqa: E402


BATCH_ID = "j02"
BLOCK_ID = 17
ENTRY_COUNT = 969
COORDINATES_SHA256 = "09C5226BE0F1FCF5D4A8B7AD984F5865F3927E23B988ABC10A9681524FE53E48"
PRIVATE_CONTEXT = (
    REPO_ROOT
    / "tmp"
...
EXPECTED_COMBINED = 25_180
EXPECTED_REMAINING = 3_092
...
def apply_record_map(
    ...
    private_by_record: dict[int, list[dict[str, Any]]],
    records: dict[int, tuple[str, ...]],
    origin: str,
) -> None:
    for record_id, korean_literals in sorted(records.items()):
        assigned = sorted(
            private_by_record.get(record_id, []),
            key=lambda entry: entry["coordinate"][2],
        )
        if len(assigned) != len(korean_literals):
            raise BuildError(
                f"{origin} record {record_id} literal count mismatch: "
...
def assemble(
    ...
    for entry in private_entries:
        by_record[entry["coordinate"][1]].append(entry)

    translations: dict[tuple[int, int, int], str] = {}
    origins: dict[tuple[int, int, int], str] = {}
    for (record_id, literal_id), korean in sorted(LEGACY_COORDINATE_KO.items()):
        coordinate = (BLOCK_ID, record_id, literal_id)
        if coordinate not in batch_coordinates:
            raise BuildError(f"legacy seed escaped j02: {coordinate}")
        translations[coordinate] = korean
        origins[coordinate] = "legacy_exact_coordinate_context_reviewed"

    apply_record_map(
        translations, origins, by_record, MANUAL_RECORDS, "new_manual_context_translation"
    )
    apply_record_map(
        translations, origins, by_record, REVIEWED_RECORDS, "segmentation_context_review"
    )

    canonical_groups: dict[str, list[tuple[int, int, int]]] = defaultdict(list)
    for coordinate, entry in by_coordinate.items():
        canonical_groups[text_hash(entry["jp"])].append(coordinate)
    for (record_id, literal_id), korean in sorted(CANONICAL_REFERENCE_KO.items()):
        reference = (BLOCK_ID, record_id, literal_id)
        entry = by_coordinate.get(reference)
        if entry is None:
            raise BuildError(f"canonical reference is outside j02: {reference}")
        for coordinate in canonical_groups[text_hash(entry["jp"])]:
            translations[coordinate] = korean
            origins[coordinate] = "repeated_source_canonical_review"

    # A repeated JP literal must have one Korean rendering.  Prefer explicit
    # canonical review, then current-boundary review, then new manual work, and
    # only then the legacy seed.  Equal-priority disagreements are rejected.
    origin_priority = {
        "legacy_exact_coordinate_context_reviewed": 1,
        "new_manual_context_translation": 2,
        "segmentation_context_review": 3,
        "repeated_source_canonical_review": 4,
    }
    for coordinates in canonical_groups.values():
        populated = [coordinate for coordinate in coordinates if coordinate in translations]
        if not populated:
            continue
        best_priority = max(origin_priority[origins[coordinate]] for coordinate in populated)
        preferred = {
            translations[coordinate]
            for coordinate in populated
            if origin_priority[origins[coordinate]] == best_priority
        }
        if len(preferred) != 1:
            raise BuildError(
                f"equal-priority repeated-source disagreement at {populated[:3]}"
            )
        canonical_korean = preferred.pop()
        for coordinate in coordinates:
            if coordinate in translations and translations[coordinate] != canonical_korean:
                translations[coordinate] = canonical_korean
                origins[coordinate] = "repeated_source_priority_review"
    ...
    if conflicts:
        raise BuildError(f"repeated JP source translation conflict: {len(conflicts)}")
    ...
    stats = {
        ...
        "repeated_source_group_count": sum(
            len(coordinates) > 1 for coordinates in canonical_groups.values()
        ),
    }
...
def make_overlay(
    ...
    return {
        ...
        "overlay_id": "msggame_ko_pk_jp_native_wave07_j02_969",
        ...
        "translation_provenance": {
            "batch_id": BATCH_ID,
            "kind": "human_context_translation_with_reviewed_source_free_seed",
...
def command_build(args: argparse.Namespace) -> int:
    private_entries, _private_map = load_private(args.private_context)
    translations, origins, stats = assemble(private_entries)
    ...
    with tempfile.TemporaryDirectory(prefix="wave07_j02_ab_", dir=REPO_ROOT / "tmp"):
    ...
    review = {
        ...
        "reviewed_resegmented_record_count": len(REVIEWED_RECORDS),
        "canonical_reference_count": len(CANONICAL_REFERENCE_KO),
        "legacy_seed_coordinate_count": len(LEGACY_COORDINATE_KO),
...
def command_verify(args: argparse.Namespace) -> int:
    private_entries, _private_map = load_private(args.private_context)
    translations, _origins, stats = assemble(private_entries)
