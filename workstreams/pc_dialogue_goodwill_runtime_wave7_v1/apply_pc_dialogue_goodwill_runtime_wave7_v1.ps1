...
# Wave 7 accepts only its workspace candidate root or a tightly scoped
# per-user staging root.  The latter exists solely because the Steam-write
# execution context cannot read the workspace ACL; every input and output is
# still verified by SHA-256 before a file is replaced.
$repoRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$tmpRoot = Join-Path $repoRoot 'tmp\pc_dialogue_goodwill_runtime_wave7_v1'
$stagingRoot = Join-Path $env:TEMP 'nobu16_wave7_apply_stage'
$profilePaths = @(
...
$changedPaths = @('MSG/JP/msggame.bin', 'MSG_PK/JP/msggame.bin')
$inputHashes = [ordered]@{
...
$targetHashes['MSG/JP/msggame.bin'] = '83C4DF9326DB1487707FDABE9CF2A00380144D14D3AC4A4FCD02513C8E3C279E'
$targetHashes['MSG_PK/JP/msggame.bin'] = '31950B8213AC80C9BCB866163EE7B4B655440ADF863DED21186273E3F8A34BDB'
$manifestSchema = 'nobu16.kr.pc-dialogue-current-static-repairs-wave7.v1'
$transactionId = 'pc-dialogue-current-static-repairs-wave7-v1'
$applySchema = 'nobu16.kr.pc-dialogue-current-static-repairs-wave7-apply.v1'
...
function Assert-Under-OneOf([string]$Path, [string[]]$Roots, [string]$Label) {
    ...
    Fail "$Label must remain below an approved Wave 7 staging root: $fullPath"
}
...
function Assert-HashMap($Actual, $Expected, [string]$Label) {
    foreach ($relative in $profilePaths) {
        $actualValue = (Get-MapValue $Actual $relative "$Label actual hash map").ToUpperInvariant()
        $expectedValue = (Get-MapValue $Expected $relative "$Label expected hash map").ToUpperInvariant()
        if ($actualValue -ne $expectedValue) { Fail "$Label hash mismatch at ${relative}: actual $actualValue, expected $expectedValue" }
    }
}
...
function Get-ProfileState([string]$Root, [string]$Label) {
    ...
    Fail "$Label is neither the approved input nor Wave 7 target profile"
}
...
function Write-State([string]$Path, $State) { $State | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $Path -Encoding UTF8 }
...
if ($Operation -eq 'RESTORE') {
    if (-not (Test-Path -LiteralPath $BackupRoot -PathType Container)) { Fail 'restore backup is absent' }
    $statePath = Join-Path $BackupRoot 'state.json'
    $originalRoot = Join-Path $BackupRoot 'originals'
    if ((-not (Test-Path -LiteralPath $statePath -PathType Leaf)) -or (-not (Test-Path -LiteralPath $originalRoot -PathType Container))) { Fail 'restore requires an applied Wave 7 backup state and originals' }
    $state = Get-Content -LiteralPath $statePath -Raw -Encoding UTF8 | ConvertFrom-Json
    if (($state.schema -ne $applySchema) -or ($state.status -ne 'applied')) { Fail 'restore state is not an applied Wave 7 transaction' }
    foreach ($relative in $changedPaths) {
        if ((Get-Sha256 (Join-Path $originalRoot ($relative -replace '/', '\'))) -ne $inputHashes[$relative]) { Fail "restore original hash mismatch: $relative" }
    }
    $liveState = Get-ProfileState $SteamRoot 'Steam profile before restore'
    if ($liveState.name -eq 'input') { [PSCustomObject]@{ status='PASS'; result='already_restored'; writes_performed=$false } | ConvertTo-Json -Compress; return }
    if ($DryRun) { [PSCustomObject]@{ status='PASS'; result='restore_dry_run'; writes_performed=$false } | ConvertTo-Json -Compress; return }
    Assert-GameStopped
    foreach ($relative in $changedPaths) {
        Replace-From (Join-Path $originalRoot ($relative -replace '/', '\')) (Join-Path $SteamRoot ($relative -replace '/', '\')) $inputHashes[$relative] "restore $relative"
    }
...
if (($manifest.schema -ne $manifestSchema) -or ($manifest.transaction_id -ne $transactionId)) { Fail 'candidate manifest is not the pinned Wave 7 transaction' }
if (@(Compare-Object -ReferenceObject $changedPaths -DifferenceObject @($manifest.changed_paths)).Count -ne 0) { Fail 'candidate manifest changed paths differ' }
...
if ($liveState.name -eq 'target') { [PSCustomObject]@{ status='PASS'; result='already_target'; writes_performed=$false } | ConvertTo-Json -Compress; return }
...
$originalRoot = Join-Path $BackupRoot 'originals'
$statePath = Join-Path $BackupRoot 'state.json'
$state = [ordered]@{ schema=$applySchema; status='backed_up'; timestamp_utc=[DateTime]::UtcNow.ToString('o'); steam_root=$SteamRoot; candidate_root=$CandidateRoot; manifest_path=$ManifestPath; changed_paths=$changedPaths; input_sha256=$inputHashes; output_sha256=$targetHashes; written_paths=@() }
...
try {
    foreach ($relative in $changedPaths) {
        ...
        if ((Get-Sha256 $original) -ne $inputHashes[$relative]) { Fail "saved backup hash mismatch: $relative" }
    }
    Write-State $statePath $state
    foreach ($relative in $changedPaths) {
        $written.Add($relative); $state.written_paths=@($written); Write-State $statePath $state
        Replace-From (Join-Path $CandidateRoot ($relative -replace '/', '\')) (Join-Path $SteamRoot ($relative -replace '/', '\')) $targetHashes[$relative] "apply $relative"
    }
    if ((Get-ProfileState $SteamRoot 'Steam profile after apply').name -ne 'target') { Fail 'post-apply profile is not the pinned Wave 7 target' }
