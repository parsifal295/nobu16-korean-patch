#!/usr/bin/env python3
"""Build a deterministic, corpus-driven SC G1N font-v6 component.
...
DEFAULT_DEMAND_FILES = (
    SCRIPT_DIR / "corpus" / "msgui_0000_5099" / "glyph_demand.json",
    SCRIPT_DIR / "corpus" / "msgev_officer_names_0000_2399" / "glyph_demand.json",
    SCRIPT_DIR / "corpus" / "msgev_dialogue_3202_3229" / "glyph_demand.json",
    SCRIPT_DIR / "corpus" / "msgdata_castle_names_9151_9542" / "glyph_demand.json",
)
EXPECTED_DEMAND_SOURCES = {
    "KR_PATCH_WORK/workstreams/font_v6/corpus/msgdata_castle_names_9151_9542/glyph_demand.json": {
        "sha256": "C7DDF8106E6634FEB50940B8646D27A64A91EADD67546B5F1C60D31A6B76B79B",
        "character_count": 88,
        "hangul_syllable_count": 88,
    },
    "KR_PATCH_WORK/workstreams/font_v6/corpus/msgev_dialogue_3202_3229/glyph_demand.json": {
        "sha256": "71921713787E57A0BCEB429AB11764379CEABBDC78A82BC52EB2DCC10DC07570",
        "character_count": 285,
        "hangul_syllable_count": 270,
    },
    "KR_PATCH_WORK/workstreams/font_v6/corpus/msgev_officer_names_0000_2399/glyph_demand.json": {
        "sha256": "A4AB899D785F461AB9CE09174FB503373012D4EC8E8ACBE8CCBF08BDA8BE8754",
        "character_count": 125,
        "hangul_syllable_count": 125,
    },
    "KR_PATCH_WORK/workstreams/font_v6/corpus/msgui_0000_5099/glyph_demand.json": {
        "sha256": "7DBF97C2AC889F2FB33856A1A8096A1DB091C4D25DB411E73E95E5D0FB7E0D16",
        "character_count": 625,
        "hangul_syllable_count": 524,
    },
}
EXPECTED_UNION = {
    "source_count": 4,
    "source_non_whitespace_character_count": 720,
    "source_non_whitespace_codepoints_sha256": "BE2064ECAAA736CACA215AF8BD8623CF5E8E057CB6180D754779870CC815DDE8",
    "excluded_font_token_count": 20,
    "excluded_font_codepoints_sha256": "7D1FB6ABD2B901679DBE8DCB24B9B865BE555A67686611ACB7B23551A4E6D483",
    "excluded_font_tokens_sha256": "9BCD0B18E34536D90B032B09A8E597B5693FC45304DAF4F921166043FA3EF226",
    "character_count": 700,
    "hangul_syllable_count": 599,
    "non_hangul_character_count": 101,
    "union_codepoints_sha256": "64DBBA110E35B359709008E07D8A584B445B9EFD952EB54FB122D9A73EBBFFA9",
    "hangul_codepoints_sha256": "AC38EE12A7C19033281E76ABB4FB0E7CF9466EE6D0C7AFAB00F1FDAA57FEF4EF",
    "raster_codepoint_count": 638,
    "raster_codepoints_sha256": "3E60696B9F4F9A1BFDCBD66171C4D064847F80C63D50E29A87FF0B4B9C335672",
}
EXPECTED_APPEND_CONTRACT = {
    (6, 0): (600, "26CFE8A693AA88688F55C15A971900372EE48AEBD51325CFFFE9CC57820E1905"),
    (6, 1): (638, "3E60696B9F4F9A1BFDCBD66171C4D064847F80C63D50E29A87FF0B4B9C335672"),
    (7, 0): (600, "26CFE8A693AA88688F55C15A971900372EE48AEBD51325CFFFE9CC57820E1905"),
    (7, 1): (638, "3E60696B9F4F9A1BFDCBD66171C4D064847F80C63D50E29A87FF0B4B9C335672"),
}
EXPECTED_FULL_RASTER_PAYLOAD_SIZES = {6: 1469952, 7: 653312}
EXPECTED_PUBLIC_PIXEL_PAYLOAD_SIZES = {6: 1426176, 7: 633856}
EXPECTED_PUBLIC_METRICS_ROW_COUNT = 2476
EXPECTED_V5_ADDED_HANGUL = "갓갔깊깼꼭꼼꾀끼낳뇌눴닛닥닷돗딸땅떨뚫렀먹밤빗빛섬슨식쌓쓸암였잃잦젯졌죽쥔짝째척켜튿틈푸혀"
EXPECTED_V5_ADDED_HANGUL_SHA256 = "39CA8C717F71F1F949699B3EDFDDA3D07DEC82AB8C67F311FE13F8D0CA4711A6"
...
PUBLIC_PAYLOAD_ARTIFACTS = (
    ("payload/glyph_pixels_entry_6.pixels", "generated OFL Noto glyph pixels"),
    ("payload/glyph_pixels_entry_7.pixels", "generated OFL Noto glyph pixels"),
    ("licenses/OFL-NotoSansKR.txt", "SIL Open Font License text"),
    ("licenses/OFL-NotoSerifKR.txt", "SIL Open Font License text"),
    ("metrics/glyphs.jsonl", "generated OFL Noto glyph metrics"),
)
...
BASE = load_module("nobu16_font_v6_file_only_base", TOOLS_DIR / "build_file_only_font_recipe.py")
...
def public_payload_inventory(public_root: Path) -> list[dict[str, Any]]:
    """Pin the exact five non-recipe artifacts in the source-free public tree."""
    expected = {relative for relative, _origin in PUBLIC_PAYLOAD_ARTIFACTS}
    actual = {
    ...
    if actual != expected:
        raise ValueError(
            f"public non-recipe artifact set mismatch: {sorted(actual ^ expected)}"
        )

    inventory: list[dict[str, Any]] = []
    for relative, origin in PUBLIC_PAYLOAD_ARTIFACTS:
        ...
        if row["size"] != path.stat().st_size or row["sha256"] != sha256_file(path):
            raise ValueError(f"public payload inventory pin drifted while reading: {relative}")
        inventory.append(row)

    if len(inventory) != 5 or {row["path"] for row in inventory} != expected:
        raise ValueError("public payload inventory must cover exactly five unique artifacts")
    return inventory
...
def demand_union(paths: Iterable[Path]) -> dict[str, Any]:
    ...
    if len(resolved_paths) != len(set(resolved_paths)):
        raise ValueError("duplicate glyph_demand.json input path")
    unique_paths = sorted(resolved_paths, key=lambda item: item.as_posix())
...
def validate_expected_union(union: dict[str, Any], demand_paths: Iterable[Path]) -> None:
    """Fail closed unless this is the pinned four-source font-v6 corpus."""
    sources = union["sources"]
    if len(sources) != EXPECTED_UNION["source_count"]:
        raise ValueError(
            f"font-v6 requires exactly {EXPECTED_UNION['source_count']} demand sources"
        )
    actual_sources = {row["path"]: row for row in sources}
    if len(actual_sources) != len(sources) or set(actual_sources) != set(EXPECTED_DEMAND_SOURCES):
        raise ValueError(
            "font-v6 demand source set mismatch: "
            f"{sorted(set(actual_sources) ^ set(EXPECTED_DEMAND_SOURCES))}"
        )
    for path, expected in EXPECTED_DEMAND_SOURCES.items():
        actual = actual_sources[path]
        for key in ("sha256", "character_count", "hangul_syllable_count"):
            if actual.get(key) != expected[key]:
                raise ValueError(
                    f"font-v6 demand source pin mismatch: {path} {key} "
                    f"expected={expected[key]} actual={actual.get(key)}"
                )
    ...
    for key, actual in actual_union.items():
        if actual != EXPECTED_UNION[key]:
            raise ValueError(
                f"font-v6 pinned union mismatch: {key} "
                f"expected={EXPECTED_UNION[key]} actual={actual}"
            )

    validated = [validate_demand(Path(path).resolve()) for path in demand_paths]
    v5_hangul = {
        codepoint
        for demand in validated
        if demand["path"] in {
            "KR_PATCH_WORK/workstreams/font_v6/corpus/msgui_0000_5099/glyph_demand.json",
            "KR_PATCH_WORK/workstreams/font_v6/corpus/msgev_officer_names_0000_2399/glyph_demand.json",
        }
        for codepoint in demand["hangul_syllable_codepoints"]
    }
    added_hangul = sorted(set(union["hangul"]) - v5_hangul)
    added_text = "".join(chr(codepoint) for codepoint in added_hangul)
    added_lines = "".join(canonical_cp(codepoint) + "\n" for codepoint in added_hangul).encode("ascii")
    if (
        added_text != EXPECTED_V5_ADDED_HANGUL
        or sha256_bytes(added_lines) != EXPECTED_V5_ADDED_HANGUL_SHA256
    ):
        raise ValueError("font-v6 delta from immutable font-v5 is not the pinned 45-Hangul set")
...
def raster_request(codepoints: Iterable[int]) -> dict[str, Any]:
    return {
        "schema": "nobu16.kr.font-v6-raster-request.v1",
...
def run_rasterizer(powershell: Path, request_path: Path, output_dir: Path) -> dict[str, Any]:
    helper = SCRIPT_DIR / "rasterize_font_v6.ps1"
    command = [
    ...
    if result.get("schema") != "nobu16.kr.font-v6-raster-result.v1":
...
def validate_raster_result(result: dict[str, Any], root: Path, expected_codepoints: list[int]) -> dict[int, bytes]:
    ...
    for item in result.get("payloads", []):
        ...
        if (
            expected_count == EXPECTED_UNION["raster_codepoint_count"]
            and expected_size != EXPECTED_FULL_RASTER_PAYLOAD_SIZES[entry]
        ):
            raise ValueError(f"entry {entry} font-v6 full raster payload size contract drifted")
...
def build_entry(
    ...
    recipe = {
        ...
        "pixel_payload": {
            "file": f"payload/glyph_pixels_entry_{entry}.pixels",
    ...
    rebuilt = BASE.build_g1n_from_recipe(
        stock, recipe, compact_pixel_bytes, f"font-v6 entry {entry} clean recipe"
    )
...
def replay_public_recipe_v2(
    ...
    try:
        ...
        raise ValueError(f"cannot read font-v6 recipe: {exc}") from exc
    if recipe.get("schema") != "nobu16.file-only-g1n-tail-recipe.v2":
        raise ValueError("font-v6 replay requires the per-table v2 recipe schema")
    if recipe.get("file_only") is not True or recipe.get("runtime_patch_features") != []:
        raise ValueError("font-v6 recipe is not file-only")
    language_recipe = recipe.get("languages", {}).get("SC")
    if not isinstance(language_recipe, dict):
        raise ValueError("font-v6 recipe is missing SC")
    ...
    if len(stock_blob) != int(stock_info["size"]) or sha256_bytes(stock_blob) != stock_info["sha256"]:
        raise ValueError("font-v6 replay stock archive gate failed")
    stock_archive = BASE.LZ4.parse_link(stock_blob)
    if BASE.LZ4.rebuild_link(stock_archive) != stock_blob:
        raise ValueError("font-v6 replay stock LINK identity failed")
    ...
    for entry in (6, 7):
        ...
        if len(pixels) != int(payload_info["size"]) or sha256_bytes(pixels) != payload_info["sha256"]:
            raise ValueError(f"entry {entry}: font-v6 public pixel payload gate failed")
        ...
        rebuilt = BASE.build_g1n_from_recipe(stock_g1n, entry_recipe, pixels, f"font-v6 replay entry {entry}")
        replacements[entry] = rebuilt
        atomic_write(output_dir / f"SC_{entry}.font-v6.recipe.g1n", rebuilt)
    ...
    if len(rebuilt_archive) != int(language_recipe["target_archive"]["size"]):
        raise ValueError("font-v6 replay target archive size mismatch")
    if sha256_bytes(rebuilt_archive) != target_hash:
        raise ValueError("font-v6 replay target archive hash mismatch")
    parsed = BASE.LZ4.parse_link(rebuilt_archive)
    if BASE.LZ4.rebuild_link(parsed) != rebuilt_archive:
        raise ValueError("font-v6 replay target LINK identity failed")
    for index, stock_entry in enumerate(stock_archive.entries):
        if index in (6, 7):
            if BASE.extract_raw_entry(parsed, index, f"SC replay target entry {index}") != replacements[index]:
                raise ValueError(f"font-v6 replay entry {index} re-extraction mismatch")
        elif parsed.entries[index].data != stock_entry.data:
            raise ValueError(f"font-v6 replay untouched LINK entry {index} changed")
    archive_path = output_dir / "res_lang.SC.font-v6.recipe.bin"
    atomic_write(archive_path, rebuilt_archive)
    report = {
        "schema": "nobu16.kr.font-v6-recipe-replay.v1",
...
def build(args: argparse.Namespace) -> int:
    if args.demand_file:
        ...
        demand_paths = [path.resolve() for path in DEFAULT_DEMAND_FILES]
    union = demand_union(demand_paths)
    validate_expected_union(union, demand_paths)
    ...
    if (
        len(raster_codepoints) != EXPECTED_UNION["raster_codepoint_count"]
        or sha256_bytes(raster_lines) != EXPECTED_UNION["raster_codepoints_sha256"]
    ):
        raise ValueError("font-v6 stock preflight produced an unexpected raster union")
    for entry in (6, 7):
        for table in (0, 1):
            codepoints = append_plan[entry][table]
            expected_count, expected_sha256 = EXPECTED_APPEND_CONTRACT[(entry, table)]
            actual_sha256 = sha256_bytes(
                "".join(f"{canonical_cp(cp)}\n" for cp in codepoints).encode("ascii")
            )
            if len(codepoints) != expected_count or actual_sha256 != expected_sha256:
                raise ValueError(
                    f"font-v6 entry {entry} table {table} append contract drifted"
                )
    ...
    for entry in (6, 7):
        ...
        atomic_write(private_root / "candidate" / f"SC_{entry}.font-v6.g1n", candidate)
        if len(public_pixels) != EXPECTED_PUBLIC_PIXEL_PAYLOAD_SIZES[entry]:
            raise ValueError(f"entry {entry}: font-v6 public pixel payload size drifted")
        atomic_write(public_payload / f"glyph_pixels_entry_{entry}.pixels", public_pixels)
    ...
    if BASE.LZ4.rebuild_link(parsed_candidate) != candidate_archive:
        raise ValueError("font-v6 candidate LINK identity roundtrip failed")
    stock_archive = BASE.LZ4.parse_link(stock_blob)
    for index, stock_entry in enumerate(stock_archive.entries):
        if index in (6, 7):
    ...
    private_archive_path = private_root / "candidate" / "res_lang.SC.font-v6.bin"
    ...
    metrics_blob = metric_blob(full_result, selected_by_profile=metric_selection)
    if metrics_blob.count(b"\n") != EXPECTED_PUBLIC_METRICS_ROW_COUNT:
        raise ValueError("font-v6 public metrics row count drifted")
    atomic_write(public_metrics / "glyphs.jsonl", metrics_blob)
    inventory = public_payload_inventory(public_root)
    ...
    recipe = {
        ...
        "release_blocker": "font-v6 candidate requires in-game SC runtime render/exit validation before release eligibility",
        "corpus": {
            "schema": "nobu16.kr.font-v6-corpus-union.v1",
            "sources": union["sources"],
            "source_non_whitespace_character_count": union["source_codepoint_count"],
            "source_non_whitespace_codepoints_sha256": union["source_union_sha256"],
        ...
        "languages": {
            "SC": {
                ...
                "target_archive": {
                    "relative_path": "private/candidate/res_lang.SC.font-v6.bin",
    ...
    if replay_archive != candidate_archive:
        raise ValueError("public font-v6 recipe replay is not byte-identical to the private candidate")

    stock_hash_after = sha256_file(stock_path)
    if stock_hash_after != stock_hash_before:
        raise ValueError("stock archive changed during read-only font-v6 build")
    validation = {
        "schema": "nobu16.kr.font-v6-validation.v1",
        "passed": True,
        "demand_contract": {
            "schema_valid": True,
        ...
        "public_payload_inventory": {
            "non_recipe_artifact_count": len(inventory),
    ...
    manifest = {
        "schema": "nobu16.kr.font-v6-build-manifest.v1",
        "target_language": "SC",
        "file_only": True,
        "corpus": {
            "sources": union["sources"],
        ...
        "inputs": {
            ...
            "rasterizer": {"path": relative_project_path(SCRIPT_DIR / "rasterize_font_v6.ps1"), "sha256": sha256_file(SCRIPT_DIR / "rasterize_font_v6.ps1")},
        ...
        "outputs": {
            ...
            "private_candidate_archive": {
                "path": "private/candidate/res_lang.SC.font-v6.bin",
                "sha256": sha256_bytes(candidate_archive),
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--demand-file",
        action="append",
        help="Pinned demand input; repeat exactly four times. Default uses the four versioned project demands.",
    )
    result.add_argument("--p3-regression-demand", default=str(DEFAULT_P3_REGRESSION_DEMAND))
    result.add_argument(
        "--stock-archive",
        required=True,
        help="Explicit pristine SC res_lang.bin/backup input; no installed-file fallback is allowed.",
    )
