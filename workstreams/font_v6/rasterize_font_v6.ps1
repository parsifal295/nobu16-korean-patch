...
function New-RasterGlyph([int]$Codepoint, [object]$Profile) {
    ...
    if ($Codepoint -lt 0 -or $Codepoint -gt 0xFFFF) {
        throw ('G1N font-v6 supports BMP codepoints only: U+{0:X}' -f $Codepoint)
    }
...
if ($requestData.schema -ne 'nobu16.kr.font-v6-raster-request.v1') {
...
try {
    ...
    $result = [ordered]@{
        schema = 'nobu16.kr.font-v6-raster-result.v1'
