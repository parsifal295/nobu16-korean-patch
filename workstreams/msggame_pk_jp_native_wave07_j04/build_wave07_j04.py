#!/usr/bin/env python3
"""Build and verify the source-free Steam JP msggame wave07 J04 overlay."""
...
BATCH_ID = "j04"
COORDINATE_COUNT = 680
UNIQUE_SOURCE_HASH_COUNT = 538
...
OVERLAY_PATH = ROOT / "public" / "msggame_ko_pk_jp_native_wave07_j04_680.v1.json"
    ...
    for batch in value.get("batches", []):
        ...
        if batch["batch_id"] == BATCH_ID:
            ...
            if batch["coordinates_sha256"] != COORDINATES_SHA256:
                raise BuildError("partition J04 coordinate hash mismatch")
        else:
            if not others.isdisjoint(current):
                raise BuildError("non-J04 partition batches overlap")
            others.update(current)
    if selected is None or len(selected) != COORDINATE_COUNT:
        raise BuildError("partition J04 is absent or incomplete")
    if not selected.isdisjoint(others):
        raise BuildError("partition J04 overlaps another batch")
...
def build_overlay(entries: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if private_coordinates != selected or not private_coordinates.isdisjoint(others):
        raise BuildError("private coordinates do not exactly match disjoint partition J04")
    ...
    overlay = {
        ...
        "overlay_id": "msggame_ko_pk_jp_native_wave07_j04_680",
        "resource": RESOURCE,
        "base_language": "JP",
