#!/usr/bin/env python3
"""Build the third native PK ``msgev`` translation batch.

The batch takes the next 350 safe semantic PK/SC target rows left after all
registered predecessors.  It explicitly preserves internal keys, speaker
keys, dynamic substitutions, and punctuation-only rows, audits Switch v1.3
before using the pinned official SC/JP/TC/EN alignment, and publishes only
source hashes plus Korean replacements.
"""
...
sys.path[:0] = [str(BATCH02_ROOT), str(BATCH01_ROOT), str(TOOLS_ROOT)]

import build_msgev_pk_native_batch02 as previous  # noqa: E402


_TRANSLATIONS_SPEC = importlib.util.spec_from_file_location(
    "_nobu16_msgev_pk_native_batch03_translations",
    WORKSTREAM_ROOT / "translations.py",
)
if _TRANSLATIONS_SPEC is None or _TRANSLATIONS_SPEC.loader is None:
    raise RuntimeError("cannot load batch03 translation table")
...
BATCH_ID = "msgev-pk-native-batch03-350.v1"
RESOURCE = "MSG_PK/SC/msgev.bin"
OVERLAY_NAME = "msgev_ko_pk_native_batch03_350.v1.json"
EVIDENCE_NAME = "msgev_pk_native_batch03_alignment.v1.json"
REVIEW_NAME = "msgev_pk_native_batch03_review.v1.json"
VALIDATION_NAME = "validation.v1.json"
SELF_OVERLAY_PATH = f"workstreams/msgev_pk_native_batch03/public/{OVERLAY_NAME}"
...
EXPECTED_PREDECESSOR_PATH_COUNT = 35
EXPECTED_PREDECESSOR_PATHS_SHA256 = (
    "FA7872DDBFD664D7ED9E7663874A95D364A031D15C7E6DB22EC850D1E9A51F1F"
)
EXPECTED_PREDECESSOR_CLAIM_COUNT = 13_178
EXPECTED_PREDECESSOR_CLAIM_IDS_SHA256 = (
    "7CAA0A25E8D509AB464E2D3ACD27EEC30A08ECAE0BD5F42FE7CA9B6C049EFF3C"
)
EXPECTED_PREDECESSOR_TARGET_COUNT = 11_580
EXPECTED_PREDECESSOR_TARGET_IDS_SHA256 = (
    "24FDB28A644878B0CDFE373FBBE40D67372ECCEADC0E433C61C829CF6FF89FD7"
)
...
EXPECTED_PRE_BATCH_GAP_COUNT = 1_326
EXPECTED_PRE_BATCH_GAP_IDS_SHA256 = (
    "A218F0957FA2C3BE77AE6BDED4E1D81B8432C424BC6D7D0A121663C9CB29F3D4"
)
EXPECTED_SELECTED_IDS_SHA256 = (
    "759AA9934807AFE074081A40BC98BE03D9958DCE1EFA9C71CEA0505A9523777B"
)
EXPECTED_CANDIDATE_COUNT = 486
EXPECTED_CANDIDATE_IDS_SHA256 = (
    "A58F3D231DE2C2B46425329A0CF4F8715E6F4A49F9A62ED810EF456C61C0F500"
)
EXPECTED_EXCLUDED_COUNT = 136
EXPECTED_EXCLUDED_IDS_SHA256 = (
    "165DAD8FA1636112837D350F3DC02E0C88557083251F9E0207FFA1D0DF74A26F"
)
EXPECTED_NEW_EXCLUDED_IDS_SHA256 = (
    "BD9344103CBE5857FC4B3CAD9CA73514A6C23B03EB66BF8AF9BB718497AADB08"
)
...
NEW_DYNAMIC_TOKEN_PUNCTUATION_IDS: list[int] = []
NEW_PUNCTUATION_ONLY_IDS = [10_105, 10_258]
...
def audit_progress_registration(
    ...
    The first 35 non-self paths are the pinned predecessor set.  Later paths
...
def derive_candidate_partition(
    ...
    if len(SELECTED_IDS) != 350 or hash_json(SELECTED_IDS) != EXPECTED_SELECTED_IDS_SHA256:
    ...
    if SWITCH_SEMANTIC_REUSE_IDS or len(PK_NATIVE_IDS) != 350:
    ...
    if lexical_ids[:350] != SELECTED_IDS:
        raise NativeBatchError("selection is no longer the first 350 safe semantic gap rows")
...
def build_once(
    ...
    evidence = {
        "schema": "nobu16.kr.msgev-pk-native-batch03-evidence.v1",
        "batch_id": BATCH_ID,
        "resource": RESOURCE,
        "scope": scope,
        ...
        "translation_policy": {
            "selection": "next_350_safe_semantic_gap_rows_after_batch02",
    ...
    review = {
        "schema": "nobu16.kr.msgev-pk-native-batch03-review.v1",
        "batch_id": BATCH_ID,
        "resource": RESOURCE,
    ...
    validation = {
        "schema": "nobu16.kr.msgev-pk-native-batch03-validation.v1",
        "batch_id": BATCH_ID,
        "passed": True,
        "scope": scope,
        "target_catalog": target_public,
...
def build_reproducibly(
    ...
    with tempfile.TemporaryDirectory(
        prefix="nobu16-msgev-native-b03-a-", dir=tmp_root
    ) as first_dir, tempfile.TemporaryDirectory(
        prefix="nobu16-msgev-native-b03-b-", dir=tmp_root
    ) as second_dir:
        first = build_once(
