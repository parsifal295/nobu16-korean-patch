...
import build_msgev_pk_native_batch03 as batch  # noqa: E402


class MsgevPkNativeBatch03Tests(unittest.TestCase):
    @classmethod
    ...
    def test_selection_is_the_next_350_safe_semantic_gap_rows(self) -> None:
        ...
        self.assertEqual(350, len(batch.SELECTED_IDS))
        self.assertEqual(batch.SELECTED_IDS, lexical[:350])
        self.assertEqual(
    ...
    def test_candidate_selected_and_excluded_partitions_are_exact(self) -> None:
        ...
        self.assertEqual(486, len(batch.CANDIDATE_IDS))
        self.assertEqual(350, self.evidence["scope"]["translated_count"])
        self.assertEqual(136, self.evidence["scope"]["excluded_count"])
        self.assertEqual(0, self.evidence["scope"]["switch_semantic_reuse_count"])
        self.assertEqual(350, self.evidence["scope"]["pk_native_translation_count"])
    ...
    def test_reproducible_build_matches_checked_in_artifacts(self) -> None:
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b03-test-", dir=REPO_ROOT / "tmp"
        ) as directory:
            ...
            self.assertEqual(486, result["candidate_count"])
            self.assertEqual(350, result["entry_count"])
            self.assertEqual(136, result["excluded_count"])
    ...
    def test_pre_and_post_exact_self_registration_builds_are_byte_identical(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b03-registration-", dir=REPO_ROOT / "tmp"
        ) as directory:
            root = Path(directory)
    ...
    def test_valid_successor_registration_does_not_change_predecessor_selection(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b03-successor-", dir=REPO_ROOT / "tmp"
        ) as directory:
            root = Path(directory)
            ...
            successor["entries"] = [
                {
                    ...
                    "source_sc_utf16le_sha256": batch.common.text_hash(
                        self.sources["SC"]["table"].texts[10_292]
                    ),
                    "ko": "후속 등록 검증",
                }
            ]
