...
거부한다. 진행률 파일 자체와 98개 입력 파일의 path·SHA·schema·항목 수를 합친
...
| `msgstf` | 1 / 8 |
| `msggame` | 30 / 10,572 |
| 공유 `MSG/SC/strdata.bin` | 3 / 24,631 |

합계 98개 overlay의 76,692행(좌표 중복을 포함한 글꼴 수요 입력)에서 1,401개
codepoint(한글 음절 1,236개)를 수집한다. PC 순정 SC에 이미 있는 비한글 glyph는
보존하고, 실제 래스터 추가 수요는 1,318개다. entry 6/7 table 0에는 1,241개,
table 1에는 1,318개를 append한다. 독립 A/B verifier는 네 G1N table 모두가 이
전체 수요를 누락 없이 매핑하는지도 확인한다. 정확한 pin·hash·고지는
[manifest.v1.json](manifest.v1.json)에 있다.

## 로컬 적용 상태

2026-07-15에 최신 1,318개 래스터 수요를 `48px=EB / 32px=B`로 독립 A/B
빌드했고, 두 폰트 후보가 SHA-256
`799AFB588A4C944028E3DED89214F4E0D485B8A204DC8940585EBE7B354E50B4`로
일치함을 확인했다. 이 갱신 후보는 아직 설치하지 않았으며 실제 화면 검수를 기다린다.

현재 설치 archive에는 앞선 1,306개 수요 후보가 들어 있다. 비폰트 LINK entry
