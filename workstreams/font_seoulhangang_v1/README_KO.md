...
거부한다. 진행률 파일 자체와 113개 입력 파일의 path·SHA·schema·항목 수를 합친
...
| `msgstf` | 1 / 8 |
| `msggame` | 31 / 10,822 |
| 공유 `MSG/SC/strdata.bin` | 4 / 24,658 |

합계 113개 overlay의 81,748행(좌표 중복을 포함한 글꼴 수요 입력)에서 1,418개
codepoint(한글 음절 1,246개)를 수집한다. PC 순정 SC에 이미 있는 비한글 glyph는
보존하고, 실제 래스터 추가 수요는 1,335개다. entry 6/7 table 0에는 1,251개,
table 1에는 1,335개를 append한다. 독립 A/B verifier는 네 G1N table 모두가 이
전체 수요를 누락 없이 매핑하는지도 확인한다. 정확한 pin·hash·고지는
[manifest.v1.json](manifest.v1.json)에 있다.

## 로컬 적용 상태

2026-07-15에 최신 1,335개 래스터 수요를 `48px=EB / 32px=B`로 독립 A/B
빌드했고, 두 폰트 후보가 SHA-256
`BF64F73A75AA6342781E9ACE5E0132D3E1491ECADD76BBA4D9259D9772300D8D`로
일치함을 확인했다. 이 갱신 후보는 아직 설치하지 않았으며 실제 화면 검수를 기다린다.

현재 설치 archive와 v0.4.1 배포본에는 앞선 1,318개 수요 후보가 들어 있다. 비폰트 LINK entry
40개를 그대로 보존하고 entry 6/7만 합성한 설치본 SHA-256은
`706563F2BFB3D8BD63B8859366E69066E0FEEFE1A989A14B967BD4649152E271`이다.
제목 이미지가 든 outer LINK entry 3은 적용 전후 SHA와 바이트가 동일하다.

현재 설치는 `pk-translation-wave02-49607-20260715-v1` 파일 전용 트랜잭션의
검증된 backup으로 복원할 수 있다. 최신 1,335개 후보는 다음 PR 병합 후 메시지
리소스와 함께 적용하며, 실제 화면에서 굵기·클리핑·프로필 역할을 확인하는 런타임
QA는 아직 남아 있다.
