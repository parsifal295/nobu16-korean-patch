...
거부한다. 진행률 파일 자체와 106개 입력 파일의 path·SHA·schema·항목 수를 합친
...
합계 106개 overlay의 78,648행(좌표 중복을 포함한 글꼴 수요 입력)에서 1,415개
codepoint(한글 음절 1,243개)를 수집한다. PC 순정 SC에 이미 있는 비한글 glyph는
보존하고, 실제 래스터 추가 수요는 1,332개다. entry 6/7 table 0에는 1,248개,
table 1에는 1,332개를 append한다. 독립 A/B verifier는 네 G1N table 모두가 이
전체 수요를 누락 없이 매핑하는지도 확인한다. 정확한 pin·hash·고지는
[manifest.v1.json](manifest.v1.json)에 있다.

## 로컬 적용 상태

2026-07-15에 최신 1,332개 래스터 수요를 `48px=EB / 32px=B`로 독립 A/B
빌드했고, 두 폰트 후보가 SHA-256
`B4AFBD0750BC3F9CAC02B3A3F943282F947AAB3E6328225BBD016438D4D88BDF`로
...
검증된 backup으로 복원할 수 있다. 최신 1,332개 후보는 다음 PR 병합 후 메시지
