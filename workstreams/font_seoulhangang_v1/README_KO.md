...
기본 corpus는 `translation_progress.v0.1.json`에 등록된 **PK 실행 경로 7개 메시지
리소스의 모든 source-free 공개 overlay**를 SHA로 고정한다. 진행률 파일 자체와 91개
입력 파일의 path·SHA·schema·항목 수를 합친 catalog hash가 모두 맞아야 한다.

| PK 리소스 | overlay 파일 / 항목 수 |
| --- | ---: |
| `msgui` | 1 / 4,037 |
| `msgev` | 35 / 13,178 |
| `msgdata` | 10 / 21,152 |
...
합계 91개 overlay의 51,066행(좌표 중복을 포함한 글꼴 수요 입력)에서 1,376개
codepoint(한글 음절 1,219개)를 수집한다. PC 순정 SC에 이미 있는 비한글 glyph는
보존하고, 실제 래스터 추가 수요는 1,293개다. entry 6/7 table 0에는 1,224개,
table 1에는 1,293개를 append한다. 독립 A/B verifier는 네 G1N table 모두가 이
