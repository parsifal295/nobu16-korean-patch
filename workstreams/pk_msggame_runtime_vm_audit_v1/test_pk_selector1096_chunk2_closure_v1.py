#!/usr/bin/env python3
"""Tests for the independent selector-1096 chunk-2 closure layer."""
...
BUILDER_PATH = (
    SCRIPT.parent / "build_pk_selector1096_chunk2_closure_v1.py"
)
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector1096_chunk2_closure_test_builder_v1",
)
...
class Selector1096Chunk2ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_runtime_partitions(self) -> None:
        ...
        self.assertEqual(len(analysis["promotion_roots"]), 33)
        self.assertEqual(len(analysis["promotion_coordinates"]), 67)
        self.assertEqual(len(analysis["rejected_roots"]), 8)
        self.assertEqual(len(analysis["rejected_pending_roots"]), 6)
        self.assertEqual(
            len(analysis["rejected_pending_coordinates"]), 12
        )
        self.assertFalse(
    ...
    def test_all_343_accepted_assemblies_and_candidate_are_bound(
        self,
    ) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
            len(analysis["accepted_assembly_manifest"]), 343
        )
        self.assertEqual(
    ...
    def test_actions_promote_67_renew_203_and_override_47(self) -> None:
        actions = Counter(
            str(row["action"]) for row in self.bundle["evidence_rows"]
        )
        self.assertEqual(dict(actions), BUILDER.EXPECTED_ACTION_COUNTS)
        self.assertEqual(len(self.bundle["updated_rows"]), 270)
        self.assertEqual(len(self.bundle["evidence_rows"]), 270)
        self.assertEqual(
        ...
        self.assertEqual(len(override_rows), 47)
        self.assertTrue(
            all(
    ...
    def test_selector538_supersession_partition_is_exact(self) -> None:
        partition = BUILDER.BASE.supersession_partition(
            self.bundle["analysis"]
        )
        self.assertEqual(len(partition["potential"]), 67)
        self.assertEqual(len(partition["already_promoted"]), 0)
        self.assertEqual(len(partition["current_a19_live"]), 67)
        self.assertEqual(
            len(partition["selector538_planned_overlap"]), 5
        )
        self.assertEqual(
            len(partition["live_after_selector538_plan"]), 62
        )
        self.assertEqual(
    ...
    def test_public_reports_are_source_free_and_steam_is_unchanged(
        ...
        self.assertTrue(
            self.audit["proof"][
                "rejected_selector1096_chunk2_pending_rows_unchanged"
            ]
        )
    ...
    def test_output_path_guards_reject_escape_and_steam(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            with self.assertRaisesRegex(
                BUILDER.ClosureError, "private output"
            ):
