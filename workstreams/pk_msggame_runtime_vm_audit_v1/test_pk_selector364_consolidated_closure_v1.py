#!/usr/bin/env python3
"""Targeted checks for the frozen selector-364 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector364_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector364_closure_tested", BUILDER_PATH
)
...
class Selector364ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_union_counts_and_predecessor_states(self) -> None:
        ...
        self.assertEqual(
            Counter(
                row["selector364_consolidated_update_action"]
    ...
    def test_site_lineage_overlap_and_terminal_proofs(self) -> None:
        ...
        self.assertEqual(result["reviewed_sites"], 38)
        self.assertEqual(result["source_call_sites"], 42)
        self.assertEqual(result["source_only_sites"], 4)
        self.assertEqual(result["source_only_actions"], 0)
        self.assertEqual(
        ...
        self.assertTrue(proof["all_38_candidate_sites_reviewed"])
        self.assertTrue(proof["source_only_4_absent_from_current_and_candidate"])
    ...
    def test_owned_assignment_overlap_is_explicit_and_never_automatic(self) -> None:
        ...
        self.assertEqual(
            [row["owned_overlap_root_count"] for row in chunks], [9, 5]
        )
        self.assertEqual(
            [row["pending_row_upper_bound"] for row in chunks], [28, 20]
        )
        self.assertEqual(sum(row["site_count"] for row in chunks), 38)
