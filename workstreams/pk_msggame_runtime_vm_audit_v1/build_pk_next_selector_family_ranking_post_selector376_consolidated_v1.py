#!/usr/bin/env python3
"""Rank remaining PK selector families after the zero selector-376 closure."""
...
PRIVATE_SCHEMA = (
    ...
    "post-selector376-consolidated.private.v1"
)
PUBLIC_SCHEMA = (
    "nobu16.kr.pk-next-selector-family-ranking-"
    "post-selector376-consolidated-source-free.v1"
)
METHOD = (
    "selector376_zero_closure_pending_reachable_0143_"
    "owned_excluded_seven_way_selector_ranking"
)
OWNED_SELECTORS = (
    538, 568, 1096, 1174, 610, 550, 748, 1126, 142, 514, 628, 1198,
    178, 1090, 760, 508, 742, 322, 1162, 376,
)
...
EXPECTED_OWNED_CALL_TARGETS = 22
EXPECTED_NON_SEVEN_WAY_TARGETS = 23
EXPECTED_ELIGIBLE_FAMILIES = 105
EXPECTED_ELIGIBLE_UNION_ROWS = 927
EXPECTED_ELIGIBLE_UNION_SHA256 = (
    "CCB1FF101C8D1BB53942412D19F5D727260E3ACB6999D92A067B8F7D2548E05F"
)
EXPECTED_RECOMMENDED_SELECTOR = "0:364"
EXPECTED_RECOMMENDED_TERMINALS = tuple(
    f"0:{record_id}" for record_id in range(1699, 1706)
)
EXPECTED_RECOMMENDED_PENDING_ROWS = 48
EXPECTED_RECOMMENDED_PENDING_ROOTS = 21
EXPECTED_RECOMMENDED_PENDING_SITES = 21
EXPECTED_RECOMMENDED_CANDIDATE_SITES = 38
EXPECTED_RECOMMENDED_SOURCE_SITES = 42
EXPECTED_RECOMMENDED_SOURCE_ONLY_SITES = 4
...
PREDECESSOR_WRAPPER = load_module(
    PREDECESSOR_BUILDER,
    "pk_next_selector_post_376_predecessor_helpers_v1",
)
...
def build_outputs(*args: Any, **kwargs: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(
        "0:508" not in eligible and "0:376" not in eligible,
        "a zero-closure selector reappeared in the ranking",
    )
