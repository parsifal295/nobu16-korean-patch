#!/usr/bin/env python3
"""Targeted checks for the frozen selector-742 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector742_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector742_closure_tested", BUILDER_PATH
)
...
class Selector742ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_union_counts_and_predecessor_states(self) -> None:
        ...
        self.assertEqual(
            Counter(
                row["selector742_consolidated_update_action"]
    ...
    def test_site_lineage_overlap_and_terminal_proofs(self) -> None:
        ...
        self.assertEqual(result["reviewed_sites"], 59)
        self.assertEqual(result["source_call_sites"], 80)
        self.assertEqual(result["source_only_sites"], 21)
        ...
        self.assertTrue(proof["all_59_candidate_sites_reviewed"])
        self.assertTrue(
    ...
    def test_assignment_owned_overlap_contract_is_frozen(self) -> None:
        ...
        self.assertEqual(
            [row["owned_overlap_root_count"] for row in chunks], [1, 3]
        )
        self.assertEqual(
            [row["pending_row_upper_bound"] for row in chunks], [25, 36]
        )
        self.assertEqual(sum(row["site_count"] for row in chunks), 59)
    ...
    def test_chunk_assembly_contracts_are_complete(self) -> None:
        chunks = [
        ...
        self.assertEqual(
            sum(row["result"]["assembly_branches"] for row in chunks), 413
        )
        self.assertTrue(
            all(
