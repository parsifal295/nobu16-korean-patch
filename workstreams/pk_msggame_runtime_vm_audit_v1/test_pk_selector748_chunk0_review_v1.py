#!/usr/bin/env python3
"""Source-free tests for selector-748 chunk-0 review."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector748_chunk0_review_v1.py"
...
BUILDER = load_module(BUILDER_PATH, "pk_selector748_chunk0_test_builder")


class Selector748Chunk0ReviewTests(unittest.TestCase):
    @classmethod
    ...
    def test_blocked_rows_are_not_decided(self) -> None:
        blocked = set(
            self.evidence["blocked"]["pending_coordinates"]
        )
        decided = {row["coordinate"] for row in self.decisions}
        self.assertEqual(len(blocked), 19)
