#!/usr/bin/env python3
"""Tests for the deterministic selector-1096 assignment."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector1096_assignment_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector1096_assignment_test_builder_v1",
)
...
class Selector1096AssignmentTests(unittest.TestCase):
    @classmethod
    ...
    def test_terminal_dispatch_call_graph_and_ceilings_are_exact(self) -> None:
        graph = self.report["graph"]
        dispatch = graph["selector_dispatch"]
        self.assertTrue(dispatch["candidate_and_source_identical"])
        self.assertEqual(dispatch["root_count"], 13)
        self.assertEqual(dispatch["edge_count"], 13)
        self.assertEqual(dispatch["terminal_count"], 7)
        self.assertEqual(
            dispatch["edge_sha256"],
            BUILDER.EXPECTED_DISPATCH_SHA256,
        )
        self.assertEqual(graph["candidate_call_site_count"], 172)
        self.assertEqual(graph["candidate_call_root_count"], 171)
        self.assertEqual(graph["source_call_site_count"], 188)
        self.assertEqual(graph["source_call_root_count"], 187)
        self.assertEqual(graph["source_only_site_count"], 16)
    ...
    def test_a19_and_selector538_overlap_partitions_are_exact(self) -> None:
        ...
        self.assertEqual(overlap["current_live_pending_count"], 258)
        self.assertEqual(
    ...
    def test_chunks_and_chunk0_priority_are_deterministic(self) -> None:
        ...
        self.assertEqual(len(assigned), 172)
        self.assertEqual(len(set(assigned)), 172)
