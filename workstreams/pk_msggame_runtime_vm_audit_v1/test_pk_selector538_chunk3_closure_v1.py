#!/usr/bin/env python3
"""Tests for the independent selector-538 chunk-3 closure layer."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector538_chunk3_closure_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector538_chunk3_closure_test_builder_v1",
)
...
class Selector538Chunk3ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_reverse_ancestor_renewal_and_pending_partitions_are_exact(
        ...
        self.assertEqual(len(analysis["promotion_coordinates"]), 89)
        self.assertEqual(len(analysis["rejected_roots"]), 32)
        self.assertEqual(len(analysis["rejected_pending_roots"]), 24)
        self.assertEqual(
        ...
        self.assertEqual(len(analysis["held_unreviewed_roots"]), 188)
        self.assertEqual(
            len(analysis["held_unreviewed_coordinates"]),
            402,
        )

    def test_all_266_accepted_assemblies_and_candidate_are_bound(self) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
    ...
    def test_actions_promote_89_and_renew_every_affected_verified_row(
        self,
    ) -> None:
        actions = Counter(
        ...
        self.assertEqual(len(self.bundle["updated_rows"]), 509)
        self.assertEqual(len(self.bundle["evidence_rows"]), 509)
        self.assertEqual(
        ...
        self.assertEqual(len(override_rows), 37)
        self.assertTrue(
            all(
    ...
    def test_public_reports_are_source_free_and_steam_is_unchanged(
        ...
        self.assertEqual(self.audit["scope"]["chunk_id"], 3)
        self.assertTrue(
            self.audit["proof"]["rejected_chunk3_pending_rows_unchanged"]
        )
