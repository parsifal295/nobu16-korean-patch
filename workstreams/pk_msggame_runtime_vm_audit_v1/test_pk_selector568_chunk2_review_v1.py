#!/usr/bin/env python3
"""Tests for the selector-568 chunk-2 full caller review."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector568_chunk2_review_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector568_chunk2_review_test_builder_v1",
)


class Selector568Chunk2ReviewTests(unittest.TestCase):
    @classmethod
    ...
    def test_frozen_inputs_handoff_and_report(self) -> None:
        self.assertEqual(
    ...
    def test_all_78_sites_and_546_branches_are_recorded(self) -> None:
        self.assertEqual(
            self.private["counts"],
            {
                "accepted": 60,
                "accepted_assemblies": 420,
                "assemblies": 546,
                "keep": 9,
                "reject": 18,
                "rejected_assemblies": 126,
                "rewrite": 51,
                "sites": 78,
            },
        )
    ...
    def test_accepted_assemblies_pass_register_and_width_gates(self) -> None:
        accepted = [
        ...
        self.assertEqual(
            ...
            Counter(
                {
                    "fixed_concessive_suffix_after_terminal": 3,
                    "fixed_possessive_particle_after_terminal": 1,
                    "fixed_subject_particle_after_terminal": 4,
                    "noncaller_right_line_width_expansion": 9,
                    "required_semantic_width_expansion": 1,
                }
            ),
        )

    def test_public_report_is_source_free_and_exact(self) -> None:
        ...
        self.assertEqual(result["accepted_site_count"], 60)
        self.assertEqual(result["rewrite_coordinate_count"], 51)
        self.assertEqual(result["keep_coordinate_count"], 9)
        self.assertEqual(result["reject_coordinate_count"], 18)
        self.assertEqual(
            result["potential_promotion_coordinate_count"],
            50,
        )
        self.assertEqual(result["blocked_pending_coordinate_count"], 36)
        self.assertEqual(
