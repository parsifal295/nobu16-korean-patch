#!/usr/bin/env python3
"""Tests for the independent selector-1096 chunk-1 closure layer."""
...
BUILDER_PATH = (
    SCRIPT.parent / "build_pk_selector1096_chunk1_closure_v1.py"
)
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector1096_chunk1_closure_test_builder_v1",
)
...
class Selector1096Chunk1ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_terminal_renewal_and_pending_partitions_are_exact(self) -> None:
        ...
        self.assertEqual(len(analysis["promotion_roots"]), 36)
        self.assertEqual(len(analysis["promotion_coordinates"]), 78)
        self.assertEqual(len(analysis["rejected_roots"]), 8)
        self.assertEqual(len(analysis["rejected_pending_roots"]), 5)
        self.assertEqual(
        ...
        self.assertEqual(len(analysis["held_unreviewed_roots"]), 82)
        self.assertEqual(
            len(analysis["held_unreviewed_coordinates"]),
            180,
        )

    def test_all_350_accepted_assemblies_and_candidate_are_bound(
        self,
    ) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
    ...
    def test_actions_promote_78_renew_203_and_override_46(self) -> None:
        actions = Counter(
            str(row["action"]) for row in self.bundle["evidence_rows"]
        )
        self.assertEqual(dict(actions), BUILDER.EXPECTED_ACTION_COUNTS)
        self.assertEqual(len(self.bundle["updated_rows"]), 281)
        self.assertEqual(len(self.bundle["evidence_rows"]), 281)
        self.assertEqual(
        ...
        self.assertEqual(len(override_rows), 46)
        self.assertTrue(
            all(
    ...
    def test_selector538_planned_overlap_is_superseded_exactly(
        self,
    ) -> None:
        partition = BUILDER.supersession_partition(
            self.bundle["analysis"]
        )
        self.assertEqual(len(partition["potential"]), 78)
        self.assertEqual(len(partition["already_promoted"]), 0)
        self.assertEqual(len(partition["current_a19_live"]), 78)
        self.assertEqual(
            len(partition["selector538_planned_overlap"]),
            6,
        )
        self.assertEqual(
            len(partition["live_after_selector538_plan"]),
            72,
        )
        self.assertEqual(
        ...
        self.assertEqual(len(promotion_rows), 78)
        self.assertEqual(
            sum(
        ...
        self.assertEqual(public["bf7b_potential_promotion_rows"], 78)
        self.assertEqual(public["selector538_planned_overlap_rows"], 6)
        self.assertEqual(public["live_after_selector538_plan_rows"], 72)
    ...
    def test_public_reports_are_source_free_and_steam_is_unchanged(
        ...
        self.assertTrue(
            self.audit["proof"][
                "rejected_selector1096_chunk1_pending_rows_unchanged"
            ]
        )
