#!/usr/bin/env python3
"""Rank PK selector families after the zero-change selector-508 closure."""
...
PRIVATE_SCHEMA = (
    ...
    "post-selector508-consolidated.private.v1"
)
PUBLIC_SCHEMA = (
    "nobu16.kr.pk-next-selector-family-ranking-"
    "post-selector508-consolidated-source-free.v1"
)
METHOD = (
    "selector508_owned_on_post760_checkpoint_pending_reachable_0143_"
...
EXPECTED_OWNED_CALL_TARGETS = 17
EXPECTED_NON_SEVEN_WAY_TARGETS = 24
EXPECTED_ELIGIBLE_FAMILIES = 110
EXPECTED_ELIGIBLE_UNION_ROWS = 1_051
EXPECTED_ELIGIBLE_UNION_SHA256 = (
    "16E22107517B7F25158327BB60BD5DD0564592461246F6B17C6894CCE9BDD8F5"
)
EXPECTED_RECOMMENDED_SELECTOR = "0:742"
EXPECTED_RECOMMENDED_TERMINALS = tuple(
    f"0:{record_id}" for record_id in range(2154, 2161)
)
EXPECTED_RECOMMENDED_PENDING_ROWS = 61
EXPECTED_RECOMMENDED_PENDING_ROOTS = 27
EXPECTED_RECOMMENDED_PENDING_SITES = 27
EXPECTED_RECOMMENDED_CANDIDATE_SITES = 59
EXPECTED_RECOMMENDED_SOURCE_SITES = 80
...
EXPECTED_POINT_ESTIMATE = 44
EXPECTED_ESTIMATE_RANGE = (0, 60)
...
PREDECESSOR_WRAPPER = load_module(
    PREDECESSOR_BUILDER,
    "pk_next_selector_post_508_predecessor_helpers_v1",
)
...
CORE_BUILD_OUTPUTS = RANKING.build_outputs
...
def build_outputs(*args: Any, **kwargs: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(
        all(
            row["selector_coordinate"] != "0:508"
            for row in public["ranking"]
        ),
        "owned selector508 reappeared in the eligible ranking",
    )
