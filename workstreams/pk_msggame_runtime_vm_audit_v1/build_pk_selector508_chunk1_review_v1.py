#!/usr/bin/env python3
"""Freeze selector-508 chunk 1 as a blocked-only residual checkpoint."""
...
CORE = load_module(
    ...
    "selector508_chunk1_review_core",
)
ASSIGN_WRAPPER = load_module(
    WORKSTREAM / "build_pk_selector508_assignment_v1.py",
    "selector508_chunk1_review_assignment",
)
...
PRIVATE_DECISIONS_PATH = (
    DIALOGUE_TMP
    / "semantic_overrides"
    / "pk_selector508_chunk1_review_decisions.private.v1.jsonl"
)
PRIVATE_EVIDENCE_PATH = (
    DIALOGUE_TMP / "pk_selector508_chunk1_review_evidence.private.v1.json"
)
DEFAULT_PUBLIC_OUTPUT = (
    WORKSTREAM / "public" / "pk_selector508_chunk1_review.source_free.v1.json"
)

METHOD = (
    "post_selector760_selector508_chunk1_single_pass_"
    "heterogeneous_terminal_and_same_gap_blocked_residual_review"
)
PRIVATE_DECISION_SCHEMA = (
    "nobu16.kr.pk-selector508-chunk1-review-decision.private.v1"
)
PRIVATE_EVIDENCE_SCHEMA = (
    "nobu16.kr.pk-selector508-chunk1-review-evidence.private.v1"
)
PUBLIC_SCHEMA = "nobu16.kr.pk-selector508-chunk1-review.source-free.v1"
EXPECTED_COUNTS = {
...
EXPECTED_ASSEMBLY_SHA256: str | None = (
    "2E0D097DD747E9978A02E6F794D18D1FE00BD0F756D02841790D27617EC2CC6E"
)
...
def configure_core() -> None:
    ...
    CORE.SELECTOR = 508
    CORE.TERMINALS = tuple(range(1888, 1895))
    CORE.CHUNK_ID = 1
...
def terminal_digest(records: Any) -> tuple[int, str]:
    values = [
        ...
        for terminal in range(1888, 1895)
    ]
...
def build_private_payload() -> tuple[bytes, bytes, dict[str, Any]]:
    ...
    for assignment_row in assignment["site_assignments"]:
        ...
        site_reviews.append({
            ...
            "decision":
                "blocked_heterogeneous_terminal_or_same_gap_boundary",
        ...
        blocked.append({
            ...
            "reason":
                "heterogeneous_terminal_or_same_gap_boundary_unresolved",
            "root": f"{block_id}:{record_id}",
            "site": site,
        })
    ...
    CORE.require(
        len(site_reviews) == 36
        and len(assembly_manifest) == 252
        and same_gap_branches == 210,
