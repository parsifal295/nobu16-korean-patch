#!/usr/bin/env python3
"""Tests for the independent selector-538 chunk-1 closure layer."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector538_chunk1_closure_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector538_chunk1_closure_test_builder_v1",
)
...
class Selector538Chunk1ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_reverse_ancestor_renewal_and_pending_partitions_are_exact(
        ...
        self.assertEqual(len(analysis["promotion_roots"]), 34)
        self.assertEqual(len(analysis["promotion_coordinates"]), 79)
        self.assertEqual(len(analysis["rejected_roots"]), 26)
        ...
        self.assertEqual(len(analysis["held_unreviewed_roots"]), 199)
        self.assertEqual(
            len(analysis["held_unreviewed_coordinates"]),
            450,
        )

    def test_all_315_accepted_assemblies_and_candidate_are_bound(self) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
    ...
    def test_actions_promote_79_and_renew_every_affected_verified_row(
        self,
    ) -> None:
        actions = Counter(
        ...
        self.assertEqual(len(self.bundle["updated_rows"]), 499)
        self.assertEqual(len(self.bundle["evidence_rows"]), 499)
        self.assertEqual(
        ...
        self.assertEqual(len(override_rows), 42)
        self.assertTrue(
            all(
