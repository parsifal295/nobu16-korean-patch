#!/usr/bin/env python3
"""Tests for the selector 538 chunk-1 full caller review proposal."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector538_chunk1_review_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector538_chunk1_review_test_builder_v1",
)


class Selector538Chunk1ReviewTests(unittest.TestCase):
    @classmethod
    ...
    def test_all_71_sites_and_497_branches_are_recorded(self) -> None:
        self.assertEqual(
            self.private["counts"],
            {
                "accepted": 45,
                "accepted_assemblies": 315,
                "assemblies": 497,
                "keep": 3,
                "reject": 26,
                "rejected_assemblies": 182,
                "rewrite": 42,
                "sites": 71,
            },
        )
    ...
    def test_every_accepted_site_passes_all_seven_assembly_gates(
        ...
        self.assertEqual(len(rejected), 26)
        self.assertEqual(
    ...
    def test_chunk0_exact_map_overlap_is_zero_or_identical(self) -> None:
        overlap = self.private["chunk0_exact_map_overlap"]
        ...
        self.assertEqual(
            ...
            BUILDER.EXPECTED_CHUNK0_OVERLAP_SHA256,
        )
        self.assertEqual(
            self.report["proof"]["chunk0_exact_map_overlap"],
            overlap,
        )
    ...
    def test_exact_runtime_closure_classification_is_frozen(self) -> None:
        ...
        self.assertEqual(result["accepted_site_count"], 45)
        self.assertEqual(result["accepted_root_count"], 44)
        self.assertEqual(result["rewrite_coordinate_count"], 42)
        self.assertEqual(result["keep_coordinate_count"], 3)
        self.assertEqual(result["reject_coordinate_count"], 26)
        self.assertEqual(result["rejected_root_count"], 26)
        self.assertEqual(result["runtime_promotion_coordinate_count"], 79)
        self.assertEqual(result["runtime_promotion_root_count"], 34)
        self.assertEqual(result["rejected_pending_root_count"], 7)
        self.assertEqual(result["blocked_pending_coordinate_count"], 15)
        self.assertEqual(
            result["verification_renewal_coordinate_count"],
            420,
        )
        self.assertEqual(result["verification_renewal_root_count"], 204)
        self.assertEqual(result["exact_override_coordinate_count"], 42)
        self.assertEqual(result["decision_delta_coordinate_count"], 499)
        self.assertEqual(result["runtime_review_pending_before"], 8213)
        self.assertEqual(result["runtime_review_pending_after"], 8134)
        self.assertEqual(
