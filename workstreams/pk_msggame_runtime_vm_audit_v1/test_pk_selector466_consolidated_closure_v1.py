#!/usr/bin/env python3
"""Targeted checks for the frozen selector-466 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector466_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector466_closure_tested", BUILDER_PATH
)
...
class Selector466ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_single_coordinate_union(self) -> None:
        ...
        self.assertEqual(
            ...
            (24, 13, 6215, 6191, 24, 0, 0),
        )
        self.assertEqual(
            Counter(
                row["selector466_consolidated_update_action"]
                for row in self.decisions
            ),
            Counter(BUILDER.EXPECTED_ACTION_COUNTS),
        )
        self.assertEqual(len({row["coordinate"] for row in self.decisions}), 24)
    ...
    def test_scope_lineage_and_block_proofs(self) -> None:
        ...
        for name in (
            "all_79_candidate_sites_reviewed",
            "source_only_15_absent_from_current_and_candidate",
            "source_only_action_count_zero",
            "same_gap_atoms_blocked_as_one_unit",
    ...
    def test_terminal_and_same_gap_roots_absent(self) -> None:
        ...
        } | set(assignment["same_gap_control_atom"]["atom_roots"])
        decision_roots = {
    ...
    def test_chunk_contract_and_exact_inputs(self) -> None:
        ...
        self.assertEqual(
            tuple(row["result"]["decision_rows"] for row in chunks), (3, 21)
        )
        self.assertEqual(
            tuple(row["result"]["sites"] for row in chunks), (38, 41)
        )
