#!/usr/bin/env python3
"""Tests for the selector-1096 chunk-0 full caller review."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector1096_chunk0_review_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector1096_chunk0_review_test_builder_v1",
)


class Selector1096Chunk0ReviewTests(unittest.TestCase):
    @classmethod
    ...
    def test_all_56_sites_and_392_branches_are_recorded(self) -> None:
        self.assertEqual(
            self.private["counts"],
            {
                "accepted": 34,
                "accepted_assemblies": 238,
                "assemblies": 392,
                "keep": 0,
                "reject": 22,
                "rejected_assemblies": 154,
                "rewrite": 34,
                "sites": 56,
            },
        )
    ...
    def test_public_counts_digests_and_live_priority_match(self) -> None:
        ...
        self.assertEqual(result["accepted_site_count"], 34)
        self.assertEqual(result["rewrite_coordinate_count"], 34)
        self.assertEqual(result["keep_coordinate_count"], 0)
        self.assertEqual(result["reject_coordinate_count"], 22)
        self.assertEqual(result["accepted_root_count"], 34)
        self.assertEqual(result["rejected_root_count"], 22)
        self.assertEqual(
            result["potential_promotion_coordinate_count"],
            77,
        )
        self.assertEqual(result["blocked_pending_coordinate_count"], 16)
        self.assertEqual(
        ...
        self.assertEqual(proof["assembly_branches_recorded"], 392)
        self.assertEqual(proof["accepted_assembly_branches"], 238)
        self.assertEqual(proof["rejected_assembly_branches"], 154)
        self.assertEqual(
            proof["auxiliary_language_available_counts"],
            {"en": 22, "jp": 56, "sc": 28, "tc": 29},
        )
        self.assertEqual(
            proof["blocker_reason_counts"],
            {
                "dynamic_particle_width_constraint": 1,
                "fixed_connective_suffix_after_terminal": 2,
                "fixed_question_copula_after_terminal": 1,
                "fixed_question_suffix_after_terminal": 1,
                "noncaller_right_line_width_expansion": 17,
            },
        )
        self.assertEqual(
