#!/usr/bin/env python3
"""Validate the frozen source-free selector-748 chunk-2 review."""
...
DECISIONS = (
    ...
    / "pk_selector748_chunk2_review_decisions.private.v1.jsonl"
)
EVIDENCE = DIALOGUE_TMP / "pk_selector748_chunk2_review_evidence.private.v1.json"
PUBLIC_OUTPUT = (
    WORKSTREAM / "public" / "pk_selector748_chunk2_review.source_free.v1.json"
)
STEAM = Path(r"F:\SteamLibrary\steamapps\common\NOBU16\MSG_PK\JP\msggame.bin")

METHOD = (
    "post_selector550_selector748_chunk2_fresh_semantic_seven_branch_"
    "shared_terminal_same_gap_owned_dependency_and_current_relative_review"
)
DECISION_SCHEMA = "nobu16.kr.pk-selector748-chunk2-review-decision.private.v1"
...
def build_public():
    require(
        sha256_file(DECISIONS) == EXPECTED["decision_file"]
        and sha256_file(EVIDENCE) == EXPECTED["evidence_file"],
        "frozen private review drifted",
    )
    decisions = [
        json.loads(line)
    ...
    decided = {row["coordinate"] for row in decisions}
    require(
        len(blocked) == COUNTS["blocked_pending_rows"]
        and blocked.isdisjoint(decided),
        "blocked rows entered decisions",
    )
    ...
    public = {
        ...
        "scope": {"chunk_id": 2, "selector": 748, "terminal_count": 7},
        "status": "PASS",
