#!/usr/bin/env python3
"""Targeted checks for the frozen selector-1078 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector1078_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector1078_closure_tested", BUILDER_PATH
)
...
class Selector1078ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_single_coordinate_union(self) -> None:
        ...
        self.assertEqual(
            ...
            (17, 7, 6232, 6215, 17, 0, 0),
        )
        self.assertEqual(
            Counter(
                row["selector1078_consolidated_update_action"]
                for row in self.decisions
            ),
            Counter(BUILDER.EXPECTED_ACTION_COUNTS),
        )
        self.assertEqual(len({row["coordinate"] for row in self.decisions}), 17)
    ...
    def test_scope_lineage_and_block_proofs(self) -> None:
        ...
        for name in (
            "all_43_candidate_sites_reviewed",
    ...
    def test_chunk_contract_and_exact_inputs(self) -> None:
        ...
        self.assertEqual(
            tuple(row["result"]["decision_rows"] for row in chunks), (7, 10)
        )
        self.assertEqual(
            tuple(row["result"]["sites"] for row in chunks), (21, 22)
        )
