#!/usr/bin/env python3
"""Tests for the independent selector-538 chunk-0 closure layer."""
...
BUILDER_PATH = SCRIPT.parent / "build_pk_selector538_chunk0_closure_v1.py"
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector538_chunk0_closure_test_builder_v1",
)
...
class Selector538Chunk0ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_output_hashes_and_contents_are_frozen(self) -> None:
        expected = {
    ...
    def test_reverse_ancestor_renewal_and_pending_partitions_are_exact(
        self,
    ) -> None:
        ...
        self.assertEqual(len(analysis["promotion_coordinates"]), 65)
        self.assertEqual(len(analysis["rejected_roots"]), 30)
        self.assertEqual(len(analysis["rejected_pending_roots"]), 27)
        self.assertEqual(
    ...
    def test_all_245_accepted_assemblies_and_candidate_are_bound(self) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
        ...
        self.assertTrue(
            self.audit["proof"][
                "all_accepted_register_branches_proven"
            ]
        )
        self.assertTrue(
            self.audit["proof"][
                "all_accepted_width_branches_nonexpanding"
            ]
        )
        self.assertTrue(
            self.audit["proof"][
                "all_changed_record_control_gaps_preserved"
            ]
        )
        self.assertTrue(self.audit["proof"]["all_exact_overrides_applied"])
        self.assertTrue(self.audit["proof"]["all_keep_rows_preserved"])

    def test_actions_promote_65_and_renew_every_affected_verified_row(
        self,
    ) -> None:
        actions = Counter(
        ...
        self.assertEqual(len(self.bundle["updated_rows"]), 485)
        self.assertEqual(len(self.bundle["evidence_rows"]), 485)
        self.assertEqual(
