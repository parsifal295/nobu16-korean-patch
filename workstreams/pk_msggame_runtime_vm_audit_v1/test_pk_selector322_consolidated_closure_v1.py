#!/usr/bin/env python3
"""Targeted checks for the frozen selector-322 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector322_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector322_closure_tested", BUILDER_PATH
)
...
class Selector322ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_union_counts_and_predecessor_states(self) -> None:
        ...
        self.assertEqual(result["pending_before"], 6335)
        ...
        self.assertEqual(
            Counter(
                row["selector322_consolidated_update_action"]
    ...
    def test_site_lineage_overlap_and_terminal_proofs(self) -> None:
        ...
        self.assertEqual(result["reviewed_sites"], 85)
        self.assertEqual(result["source_call_sites"], 94)
        self.assertEqual(result["source_only_sites"], 9)
        self.assertEqual(result["source_only_actions"], 0)
        self.assertEqual(
        ...
        self.assertTrue(proof["all_85_candidate_sites_reviewed"])
        self.assertTrue(proof["source_only_9_absent_from_current_and_candidate"])
    ...
    def test_assignment_owned_overlap_contract_is_frozen(self) -> None:
        ...
        self.assertEqual(
            [row["owned_overlap_root_count"] for row in chunks], [2, 3]
        )
        self.assertEqual(
            [row["pending_row_upper_bound"] for row in chunks], [26, 31]
        )
        self.assertEqual(sum(row["site_count"] for row in chunks), 85)
    ...
    def test_chunk_assembly_empty_and_blocked_contracts(self) -> None:
        chunks = [
    ...
    def test_empty_decision_input_remains_a_supported_blocked_state(self) -> None:
        ...
        self.assertTrue(all(count > 0 for count in blocked_rows))
