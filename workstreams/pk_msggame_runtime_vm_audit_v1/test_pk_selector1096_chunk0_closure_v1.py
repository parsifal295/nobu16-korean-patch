#!/usr/bin/env python3
"""Tests for the independent selector-1096 chunk-0 closure layer."""
...
BUILDER_PATH = (
    SCRIPT.parent / "build_pk_selector1096_chunk0_closure_v1.py"
)
...
BUILDER = load_module(
    BUILDER_PATH,
    "pk_selector1096_chunk0_closure_test_builder_v1",
)
...
class Selector1096Chunk0ClosureTests(unittest.TestCase):
    @classmethod
    ...
    def test_terminal_renewal_and_pending_partitions_are_exact(self) -> None:
        ...
        self.assertEqual(len(analysis["renewal_roots"]), 98)
        self.assertEqual(len(analysis["renewal_coordinates"]), 203)
        self.assertEqual(
            BUILDER.coordinate_digest(analysis["renewal_coordinates"]),
            BUILDER.EXPECTED_RENEWAL_COORDINATE_SHA256,
        )
        self.assertEqual(len(analysis["promotion_roots"]), 28)
        self.assertEqual(len(analysis["promotion_coordinates"]), 77)
        self.assertEqual(len(analysis["rejected_roots"]), 22)
        self.assertEqual(len(analysis["rejected_pending_roots"]), 7)
        self.assertEqual(
        ...
        self.assertEqual(len(analysis["held_unreviewed_roots"]), 88)
        self.assertEqual(
            len(analysis["held_unreviewed_coordinates"]),
            178,
        )

    def test_all_238_accepted_assemblies_and_candidate_are_bound(
        self,
    ) -> None:
        analysis = self.bundle["analysis"]
        self.assertEqual(
    ...
    def test_actions_promote_77_renew_203_and_override_34(self) -> None:
        actions = Counter(
            str(row["action"]) for row in self.bundle["evidence_rows"]
        )
        self.assertEqual(dict(actions), BUILDER.EXPECTED_ACTION_COUNTS)
        self.assertEqual(len(self.bundle["updated_rows"]), 280)
        self.assertEqual(len(self.bundle["evidence_rows"]), 280)
        self.assertEqual(
    ...
    def test_bf7b_potential_and_live_supersession_are_distinct(self) -> None:
        analysis = self.bundle["analysis"]
        partition = BUILDER.supersession_partition(analysis)
        self.assertEqual(len(partition["potential"]), 77)
        self.assertEqual(len(partition["already_promoted"]), 5)
        self.assertEqual(len(partition["current_a19_live"]), 72)
        self.assertEqual(
            partition["already_promoted"],
            partition["selector538_planned_overlap"],
        )
        self.assertEqual(
            partition["current_a19_live"],
            partition["live_after_selector538_plan"],
        )
        self.assertEqual(
            partition["already_promoted"] | partition["current_a19_live"],
            partition["potential"],
        )
        self.assertFalse(
            partition["already_promoted"]
        ...
        promotion_rows = [
            row for row in self.bundle["updated_rows"]
            if BUILDER.SUPERSESSION_FIELD in row
        ]
        self.assertEqual(len(promotion_rows), 77)
        self.assertEqual(
            sum(
                row[BUILDER.SUPERSESSION_FIELD][
        ...
        public = self.audit["proof"]["selector538_evidence_supersession"]
        self.assertEqual(public["bf7b_potential_promotion_rows"], 77)
        self.assertEqual(public["already_promoted_rows"], 5)
        self.assertEqual(public["current_a19_live_pending_rows"], 72)
        self.assertTrue(public["partition_complete"])
    ...
    def test_public_reports_are_source_free_and_steam_is_unchanged(
        ...
        self.assertEqual(self.audit["scope"]["chunk_id"], 0)
        self.assertTrue(
            self.audit["proof"][
                "rejected_selector1096_chunk0_pending_rows_unchanged"
            ]
        )
