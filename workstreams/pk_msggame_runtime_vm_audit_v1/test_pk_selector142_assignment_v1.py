#!/usr/bin/env python3
"""Focused tests for the immutable selector-142 assignment."""
...
PATH = Path(__file__).with_name("build_pk_selector142_assignment_v1.py")
SPEC = importlib.util.spec_from_file_location("selector142_assignment_test", PATH)
...
class Selector142AssignmentTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_partition(self) -> None:
        ...
        self.assertEqual(
            [row["pending_root_count"]
             for row in self.public["assignment"]["chunks"]],
            [16, 14, 26],
        )
    ...
    def test_roots_and_template_atoms_are_not_split(self) -> None:
        ...
        self.assertEqual(
            ...
            list(BUILDER.EXPECTED_TEMPLATE_SIZES),
        )
        self.assertFalse(
    ...
    def test_source_only_is_separate_and_public_is_source_free(self) -> None:
        ...
        self.assertEqual(len(source_only), 6)
