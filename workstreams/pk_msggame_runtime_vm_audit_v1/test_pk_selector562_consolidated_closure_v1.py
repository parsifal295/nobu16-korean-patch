#!/usr/bin/env python3
"""Targeted checks for the frozen selector-562 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector562_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector562_closure_tested", BUILDER_PATH
)
...
class Selector562ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_single_coordinate_union(self) -> None:
        ...
        self.assertEqual(
            ...
            (10, 7, 6191, 6181, 10, 0, 0),
        )
        self.assertEqual(
            Counter(
                row["selector562_consolidated_update_action"]
                for row in self.decisions
            ),
            Counter(BUILDER.EXPECTED_ACTION_COUNTS),
        )
        self.assertEqual(len({row["coordinate"] for row in self.decisions}), 10)
    ...
    def test_scope_lineage_and_block_proofs(self) -> None:
        ...
        for name in (
            "all_54_candidate_sites_reviewed",
            "source_only_6_absent_from_current_and_candidate",
            "source_only_action_count_zero",
            "atomic_template_roots_reviewed_as_one_unit",
    ...
    def test_chunk_contract_and_exact_inputs(self) -> None:
        ...
        self.assertEqual(
            tuple(row["result"]["decision_rows"] for row in chunks), (5, 5)
        )
        self.assertEqual(
            tuple(row["result"]["sites"] for row in chunks), (28, 26)
        )
