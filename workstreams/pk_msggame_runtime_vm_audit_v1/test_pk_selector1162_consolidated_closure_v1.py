#!/usr/bin/env python3
"""Targeted checks for the frozen selector-1162 consolidated closure."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_pk_selector1162_consolidated_closure_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector1162_closure_tested", BUILDER_PATH
)
...
class Selector1162ClosureTest(unittest.TestCase):
    @classmethod
    ...
    def test_exact_union_counts_and_predecessor_states(self) -> None:
        ...
        self.assertEqual(result["pending_before"], 6310)
        ...
        self.assertEqual(
            Counter(
                row["selector1162_consolidated_update_action"]
    ...
    def test_site_lineage_overlap_and_terminal_proofs(self) -> None:
        ...
        self.assertEqual(result["reviewed_sites"], 61)
        self.assertEqual(result["source_call_sites"], 66)
        ...
        self.assertTrue(proof["all_61_candidate_sites_reviewed"])
        self.assertTrue(proof["source_only_5_absent_from_current_and_candidate"])
    ...
    def test_owned_assignment_overlap_is_explicit_and_never_automatic(self) -> None:
        ...
        self.assertEqual(
            [row["owned_overlap_root_count"] for row in chunks], [8, 10]
        )
        self.assertEqual(
            [row["pending_row_upper_bound"] for row in chunks], [21, 34]
        )
        self.assertEqual(sum(row["site_count"] for row in chunks), 61)
        ...
        for row in self.decisions:
            ...
            self.assertNotIn(
                "auto",
                row["selector1162_consolidated_update_action"].lower(),
            )
        chunk_decisions = [
            json.loads(line)
    ...
    def test_chunk_empty_and_blocked_contracts(self) -> None:
        ...
        self.assertEqual(chunks[1]["result"]["owned_overlap_roots"], 10)
