# Steam JP 현재 텍스트 품질 결합 트랜잭션 후보

NPC 표기 수정이 적용된 현재 Steam JP 11파일 프로필을 입력으로 고정하고, 검증된 인물 대사·Base 이벤트 줄바꿈 보정을 한 번에 결합하는 사설 후보입니다. 이 작업공간의 Python 빌더는 후보 파일만 만들며 Steam에 쓰거나 게임을 실행하지 않습니다.

## 결합 계약

- 입력: NPC 표기 수정 후 현재 Steam 전체 프로필 11파일
- 실제 쓰기 대상: `MSG/JP/ev_strdata.bin`, `MSG/JP/msggame.bin`, `MSG_PK/JP/msggame.bin` 세 파일만
- Base: Wave 12의 `13:143` 한 레코드
- Base 이벤트: Wave 13의 정적 이벤트 7셀
- PK: Wave 10 12개 + Wave 11 8개 + Wave 12 1개 + Wave 13 정적 대사 12개 = 33개 물리 레코드
- PK 레코드 중복: `0` 강제
- 나머지 8파일: 입력과 바이트 단위 동일
- 현재 `RES_JP` 폰트 및 HUD 자산은 이 트랜잭션의 수정·검증 범위가 아님

PK 네 Wave는 모두 같은 현재 PK 입력에서 독립적으로 검증됩니다. 따라서 단독 후보 파일을 순서대로 덮어쓰지 않고, 33개 비중첩 레코드를 원본에 한 번에 재구축합니다.
...
| `MSG/JP/ev_strdata.bin` | `CC77EE4B0587B371A901069FB3F39C2187886C3A3335D9748D275FA2881EB426` | `BF224468BFBCF3CC71DFF4609142A60D75091813281EE6F2333645413AD81B80` |
| `MSG_PK/JP/msggame.bin` | `209B96CADE84D82810A8A79CA362DFA1B6665A8C601D3DB2C3DC0F96986E9930` | `3924ADABF69C9BA72EEBA95E4CE07A3CB8FCD716A31D8F6217ECC5FFAA7B96C5` |

전체 11파일 입력·출력 해시는 빌더가 만드는 manifest에 함께 고정됩니다. 이로써 두 대상만 쓰더라도 사전·사후 Steam 프로필 전체를 검증할 수 있습니다.

## 후보 생성 및 검증

기본 입력은 현재 Steam 설치 경로입니다.

`F:\SteamLibrary\steamapps\common\NOBU16`
...
- `candidate-build-2/`: 목표 11파일 후보 트리
- `audit.v2.json`: 원문을 담지 않는 결합 레코드 감사 계약
- `build_manifest.v2.json`: writer가 검증하는 전체 프로필·3파일 쓰기 계약
...
- 적용 전 Steam 11파일 전체가 NPC 표기 수정 후 입력과 일치하는지 검증
- 후보 manifest와 후보 11파일 전체가 결합 목표와 일치하는지 검증
- 게임 및 공식 런처 프로세스 종료 확인
- 세 변경 파일만 백업하고 각 해시를 검증
...
PowerShell -NoProfile -ExecutionPolicy Bypass -File `
  ...
  -Operation APPLY -SteamRoot $root -CandidateRoot "$tmp\candidate-build-2" `
  -ManifestPath "$tmp\build_manifest.v2.json" `
