...
import build_msgdata_pk_structural_review_b12 as batch  # noqa: E402


class MsgdataPkStructuralReviewB12Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.packed_sc, cls.tables = batch.previous.previous.previous.previous.previous.load_tables(batch.GAME_ROOT)
        cls.targets = batch.previous.previous.previous.previous.previous.previous.load_target_catalog(
    ...
    def test_five_explicit_predecessors_own_2500_unique_ids(self) -> None:
        ...
        for path in batch.OWNER_PATHS:
            ...
            ids.extend(current)
        self.assertEqual(2500, len(set(ids)))
        self.assertTrue(set(ids).issubset(self.predecessors))
        self.assertEqual(batch.EXPECTED_B11_OVERLAY_SHA256, batch.sha256(self.owner_blobs[batch.previous.SELF_OVERLAY_PATH]))

    def test_scope_is_sixth_500_and_leaves_1110(self) -> None:
        self.assertEqual(500, len(self.selected))
        self.assertEqual((26887, 27493), (self.selected[0], self.selected[-1]))
        self.assertEqual(batch.EXPECTED_REVIEW_IDS_SHA256, batch.hash_json(self.selected))
        self.assertEqual(23924, len(self.predecessors))
        self.assertFalse(set(self.selected) & self.predecessors)
        self.assertEqual(1610, len(self.targets - self.predecessors))
        self.assertEqual(1110, len(self.targets - self.predecessors - set(self.selected)))

    def test_all_500_rows_are_restricted_ascii_lookup_keys(self) -> None:
        ...
        for entry_id in self.selected:
            ...
            with self.subTest(entry_id=entry_id):
                ...
                self.assertEqual(0, sum(batch.script_counts(value).values()))
    ...
    def test_self_and_successor_registration_produce_identical_artifacts(self) -> None:
        ...
        successor = "workstreams/msgdata_pk_structural_review_b13/public/msgdata_ko_pk_structural_review_b13_500.v1.json"
        ...
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b12-history-", dir=REPO_ROOT / "tmp") as directory:
            ...
            for index, tail in enumerate((list(batch.OWNER_PATHS) + [self_path], list(batch.OWNER_PATHS) + [self_path, successor])):
    ...
    def test_reproducible_build_matches_checked_in_artifacts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b12-build-", dir=REPO_ROOT / "tmp") as directory:
            ...
            self.assertEqual((500, 500, 0, 1110), (
