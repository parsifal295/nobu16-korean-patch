...
import build_msgev_pk_native_batch06 as batch  # noqa: E402


class MsgevPkNativeBatch06Tests(unittest.TestCase):
    @classmethod
    ...
    def test_selection_covers_every_remaining_safe_semantic_gap_row(self) -> None:
        ...
        lexical = [
            ...
            and not batch._looks_like_format_only(
                self.sources["SC"]["table"].texts[entry_id]
            )
        ]
        self.assertEqual(260, len(batch.SELECTED_IDS))
        self.assertEqual(batch.SELECTED_IDS, lexical)
        self.assertEqual(
    ...
    def test_candidate_selected_and_excluded_partitions_are_exact(self) -> None:
        ...
        self.assertEqual(501, len(batch.CANDIDATE_IDS))
        self.assertEqual(260, self.evidence["scope"]["translated_count"])
        self.assertEqual(241, self.evidence["scope"]["excluded_count"])
        self.assertEqual(0, self.evidence["scope"]["switch_semantic_reuse_count"])
        self.assertEqual(260, self.evidence["scope"]["pk_native_translation_count"])
    ...
    def test_overlay_shape_hash_target_membership_and_disjointness(self) -> None:
        ...
        selected_coordinates = {
            (entry["id"], entry["source_sc_utf16le_sha256"]) for entry in entries
        }
        ...
        self.assertFalse(selected_coordinates & predecessor_coordinates)
    ...
    def test_repeated_multilingual_sources_have_consistent_translations(self) -> None:
        groups: dict[tuple[str, str], list[int]] = defaultdict(list)
        sc = self.sources["SC"]["table"].texts
        jp = self.sources["JP"]["table"].texts
        for entry_id in batch.SELECTED_IDS:
            groups[(sc[entry_id], jp[entry_id])].append(entry_id)
        repeated = [ids for ids in groups.values() if len(ids) > 1]
        self.assertEqual(4, len(repeated))
        for ids in repeated:
            with self.subTest(ids=ids):
                self.assertEqual(1, len({batch.TRANSLATIONS[item] for item in ids}))
    ...
    def test_translation_provenance_partition_is_revalidated(self) -> None:
        ...
        observed_audit = batch.audit_switch_reuse_for_selected(self.sources)
        self.assertEqual(observed_audit, self.evidence["switch_reuse_audit"])
        for entry_id in batch.PK_NATIVE_IDS:
            with self.subTest(pk_native=entry_id):
                ...
                self.assertEqual(
                    bool(exact), evidence[entry_id]["switch_exact_mapping_available"]
                )
                self.assertEqual(
    ...
    def test_reproducible_build_matches_checked_in_artifacts(self) -> None:
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b06-test-", dir=REPO_ROOT / "tmp"
        ) as directory:
            ...
            self.assertEqual(501, result["candidate_count"])
            self.assertEqual(260, result["entry_count"])
            self.assertEqual(241, result["excluded_count"])
            for relative, blob in result["files"].items():
                self.assertEqual((WORKSTREAM_ROOT / relative).read_bytes(), blob)

    def test_batch05_absent_present_and_self_registration_builds_are_byte_identical(self) -> None:
        progress = json.loads(
        ...
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b06-registration-", dir=REPO_ROOT / "tmp"
        ) as directory:
            root = Path(directory)
    ...
    def test_valid_successor_registration_does_not_change_predecessor_selection(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="nobu16-msgev-b06-successor-", dir=REPO_ROOT / "tmp"
        ) as directory:
            root = Path(directory)
            ...
            successor["entries"] = [
                {
                    ...
                    "source_sc_utf16le_sha256": batch.common.text_hash(
                        self.sources["SC"]["table"].texts[17_872]
                    ),
                    "ko": "후속 등록 검증",
                }
            ]
