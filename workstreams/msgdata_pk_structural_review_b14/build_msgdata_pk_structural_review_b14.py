#!/usr/bin/env python3
"""Review the eighth PK msgdata structural page with stable registration history."""
...
B13_ROOT = REPO_ROOT / "workstreams" / "msgdata_pk_structural_review_b13"
...
previous = load_module(
    "nobu16_msgdata_pk_structural_review_b14_previous",
    B13_ROOT / "build_msgdata_pk_structural_review_b13.py",
)
common = previous.common
engine = previous.engine
StructuralReviewError = previous.StructuralReviewError

BATCH_ID = "msgdata-pk-structural-review-b14-500.v1"
RESOURCE = previous.RESOURCE
OVERLAY_NAME = "msgdata_ko_pk_structural_review_b14_500.v1.json"
EVIDENCE_NAME = "msgdata_pk_structural_review_b14_evidence.v1.json"
REVIEW_NAME = "msgdata_pk_structural_review_b14_review.v1.json"
VALIDATION_NAME = "validation.v1.json"
SELF_OVERLAY_PATH = f"workstreams/msgdata_pk_structural_review_b14/public/{OVERLAY_NAME}"
...
EXPECTED_B13_OVERLAY_SHA256 = "5881F65D11BF7ECC04B62FE98B90B1797F288BE00560737C78522A324F0F1A47"
EXPECTED_PREDECESSOR_TARGET_COUNT = 24_924
EXPECTED_PREDECESSOR_TARGET_IDS_SHA256 = "3F4907EFDAABDBC2EFD6C3D52DBCC36E3057A91A628DC79A9B52B5E6B2A79795"
EXPECTED_PRE_REVIEW_GAP_COUNT = 610
EXPECTED_PRE_REVIEW_GAP_IDS_SHA256 = previous.EXPECTED_POST_REVIEW_GAP_IDS_SHA256
EXPECTED_REVIEW_COUNT = 500
EXPECTED_FIRST_ID = 28_043
EXPECTED_LAST_ID = 28_883
EXPECTED_REVIEW_IDS_SHA256 = "D3C14CD87FD597590ADA9D70F0A95A0D41F73498941FA8CB7EA34C611C28534A"
EXPECTED_POST_REVIEW_GAP_COUNT = 110
EXPECTED_POST_REVIEW_GAP_IDS_SHA256 = "A6A47038C24CB939240311ECEFF9110BB4246089988DAE972EDC9E0E5A166012"
REASON = "romanized_or_phonetic_lookup_key"
OFFICIAL_ROWSET_SHA256 = {
    "SC": "A23F251D802120A0D5DE92E095D3FBECBE122116BD4DA05F9372124A6A18F866",
    "JP": "21555D8F2E436F1FC685A9A1DD96FB8FDC5660B1EE21BCBDEB55DC1E5DD3D6B2",
    "EN": "F57A66CDFE5CCC874D02385DF37DB8F5AD90C55EF04AE815CF4B7C44B553552D",
    "TC": "FF1478B49BFA69FDFCC1734E97762E1D37DF088EC9D9E28CC629A7C96A405961",
}
...
def load_b13_owner(repo_root: Path) -> tuple[bytes, list[int]]:
    blob = (repo_root / previous.SELF_OVERLAY_PATH).read_bytes()
    if sha256(blob) != EXPECTED_B13_OVERLAY_SHA256:
        raise StructuralReviewError("B13 direct predecessor changed")
    ...
    if resource != RESOURCE or len(ids) != 500 or hash_json(ids) != previous.EXPECTED_REVIEW_IDS_SHA256:
        raise StructuralReviewError("B13 direct predecessor ownership changed")
    return blob, ids
...
def validate_scope(
    ...
    b13_selected, reason_by_id, pre_b13_claims, owner_blobs = previous.validate_scope(tables, targets, repo_root)
    b13_blob, b13_ids = load_b13_owner(repo_root)
    if b13_ids != b13_selected:
        raise StructuralReviewError("B13 artifact and classified ownership differ")
    owner_blobs = dict(owner_blobs)
    owner_blobs[previous.SELF_OVERLAY_PATH] = b13_blob
    predecessor_claims = pre_b13_claims | set(b13_ids)
    ...
    if len(gap) != 610 or hash_json(gap) != EXPECTED_PRE_REVIEW_GAP_IDS_SHA256:
    ...
    if len(selected) != 500 or (selected[0], selected[-1]) != (EXPECTED_FIRST_ID, EXPECTED_LAST_ID) or hash_json(selected) != EXPECTED_REVIEW_IDS_SHA256:
        raise StructuralReviewError("eighth structural review page changed")
    ...
    post_gap = gap[500:]
    if len(post_gap) != 110 or hash_json(post_gap) != EXPECTED_POST_REVIEW_GAP_IDS_SHA256:
...
def audit_progress(
    ...
    prefix = list(previous.previous.previous.previous.previous.previous.previous.EXPECTED_PREDECESSOR_PATHS)
    chain = list(OWNER_PATHS) + [SELF_OVERLAY_PATH]
    historical, _successors = previous.previous.registration_helper()(
        rows[0]["overlay_globs"], prefix, chain, 14, repo_root
    )
    ...
    if SELF_OVERLAY_PATH in historical and (repo_root / SELF_OVERLAY_PATH).read_bytes() != overlay_blob:
        raise StructuralReviewError("registered B14 differs from deterministic output")
    gap = targets - predecessor_claims
    post_gap = gap - selected
    if len(gap) != 610 or hash_json(sorted(gap)) != EXPECTED_PRE_REVIEW_GAP_IDS_SHA256:
        raise StructuralReviewError("pre-review gap changed")
    if selected & predecessor_claims or selected - gap:
        raise StructuralReviewError("B14 overlaps predecessor or target scope")
    if len(post_gap) != 110 or hash_json(sorted(post_gap)) != EXPECTED_POST_REVIEW_GAP_IDS_SHA256:
        raise StructuralReviewError("post-review gap changed")
    return {
        "pre_b07_registration_count": len(prefix), "explicit_structural_predecessor_count": 3500,
...
def make_files(game_root: Path, repo_root: Path, target_catalog_path: Path, progress_path: Path) -> dict[str, bytes]:
    packed_sc, tables = previous.previous.previous.previous.previous.previous.previous.load_tables(game_root)
    targets = previous.previous.previous.previous.previous.previous.previous.previous.load_target_catalog(target_catalog_path)["ids"]
    ...
    pin = previous.previous.previous.previous.previous.previous.previous.previous.OFFICIAL_PINS["SC"]
    overlay = {
        ...
        "stock_sc": {"size": pin["size"], "packed_sha256": pin["sha256"], "raw_size": pin["raw_size"],
            "raw_sha256": pin["raw_sha256"], "string_count": previous.previous.previous.previous.previous.previous.previous.previous.STRING_COUNT},
    ...
    evidence = {
        ...
        "selection": {"first_id": selected[0], "last_id": selected[-1], "reviewed_count": 500,
            "reviewed_ids_sha256": EXPECTED_REVIEW_IDS_SHA256, "structural_before": 610, "structural_after": 110,
    ...
    validation = {
        ...
        "scope": {"reviewed_count": 500, "reviewed_ids_sha256": EXPECTED_REVIEW_IDS_SHA256, "exact_byte_preserve_count": 500,
            ...
            "predecessor_overlap_count": 0, "post_review_structural_remaining": 110},
...
def build_reproducibly(game_root: Path, repo_root: Path, target_catalog_path: Path, progress_path: Path, out_root: Path) -> dict[str, Any]:
    ...
    return {"reviewed_count": 500, "preserve_count": 500, "blocked_count": 0, "remaining_count": 110, "files": first}
