...
import build_msgdata_pk_structural_review_b14 as batch  # noqa: E402


class MsgdataPkStructuralReviewB14Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.packed_sc, cls.tables = batch.previous.previous.previous.previous.previous.previous.previous.load_tables(batch.GAME_ROOT)
        cls.targets = batch.previous.previous.previous.previous.previous.previous.previous.previous.load_target_catalog(
    ...
    def test_seven_explicit_predecessors_own_3500_unique_ids(self) -> None:
        ...
        for path in batch.OWNER_PATHS:
            ...
            ids.extend(current)
        self.assertEqual(3500, len(set(ids)))
        self.assertTrue(set(ids).issubset(self.predecessors))

    def test_b13_is_the_direct_hash_pinned_predecessor(self) -> None:
        blob, ids = batch.load_b13_owner(REPO_ROOT)
        self.assertEqual(batch.EXPECTED_B13_OVERLAY_SHA256, batch.sha256(blob))
        self.assertEqual(500, len(ids))
        self.assertEqual(batch.previous.EXPECTED_REVIEW_IDS_SHA256, batch.hash_json(ids))
        self.assertTrue(set(ids).issubset(self.predecessors))

    def test_scope_is_eighth_500_and_leaves_110(self) -> None:
        self.assertEqual((28043, 28883), (self.selected[0], self.selected[-1]))
        self.assertEqual(500, len(self.selected))
        self.assertEqual(batch.EXPECTED_REVIEW_IDS_SHA256, batch.hash_json(self.selected))
        self.assertEqual(24924, len(self.predecessors))
        self.assertFalse(set(self.selected) & self.predecessors)
        self.assertEqual(610, len(self.targets - self.predecessors))
        remaining = self.targets - self.predecessors - set(self.selected)
        self.assertEqual(110, len(remaining))
    ...
    def test_historical_self_and_b15_registration_are_byte_stable(self) -> None:
        ...
        tails = (
            list(batch.OWNER_PATHS[:-2]), list(batch.OWNER_PATHS[:-1]), list(batch.OWNER_PATHS),
            list(batch.OWNER_PATHS) + [self_path], list(batch.OWNER_PATHS) + [self_path, successor],
        )
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b14-history-", dir=REPO_ROOT / "tmp") as directory:
    ...
    def test_reproducible_build_matches_checked_in_artifacts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b14-build-", dir=REPO_ROOT / "tmp") as directory:
            ...
            self.assertEqual((500, 500, 0, 110), (
