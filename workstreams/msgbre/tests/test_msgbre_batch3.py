...
import build_msgbre_batch3 as batch  # noqa: E402
...
class MsgbreBatch3Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_100_consecutive_biographies_at_natural_boundary(self) -> None:
        expected = list(range(251, 351))
        ...
        self.assertEqual(351, batch.NEXT_START_ID)
        self.assertEqual(251, self.evidence["scope"]["start_id"])
        self.assertEqual(350, self.evidence["scope"]["end_id"])
        self.assertEqual(351, self.evidence["scope"]["next_start_id"])
    ...
    def test_overlay_entries_contain_only_id_sc_hash_and_korean(self) -> None:
        ...
        self.assertEqual(100, self.overlay["entry_count"])
        entries = self.overlay["entries"]
        self.assertEqual(list(range(251, 351)), [entry["id"] for entry in entries])
    ...
    def test_alignment_is_hash_only_for_sc_jp_en(self) -> None:
        ...
        self.assertEqual(100, len(self.evidence["entries"]))
    ...
    def test_invariants_review_state_and_safety_are_recorded(self) -> None:
        ...
        self.assertEqual(100, invariants["checked"])
        self.assertEqual(100, invariants["custom_bracket_placeholder_checks"])
        ...
        self.assertEqual(100, self.validation["translation_status"]["human_review_required"])
        ...
        for key in (
            ...
            "existing_v01_v02_artifacts_modified",
        ):
