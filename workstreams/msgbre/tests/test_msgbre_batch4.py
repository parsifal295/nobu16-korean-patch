...
import build_msgbre_batch4 as batch  # noqa: E402
...
class MsgbreBatch4Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_107_consecutive_biographies_at_ota_boundary(self) -> None:
        expected = list(range(351, 458))
        self.assertEqual(expected, batch.selected_ids())
        self.assertEqual(expected, sorted(batch.TRANSLATIONS))
        self.assertEqual(107, len(expected))
        self.assertEqual(458, batch.NEXT_START_ID)
        self.assertEqual(351, self.evidence["scope"]["start_id"])
        self.assertEqual(457, self.evidence["scope"]["end_id"])
        self.assertEqual(458, self.evidence["scope"]["next_start_id"])
        boundary = self.validation["scope"]["natural_boundary"]
        self.assertEqual("ota", boundary["last_completed_family"])
        self.assertEqual("otaka", boundary["next_family"])
        self.assertEqual(108, boundary["officer_name_ids_crosschecked"])

    def test_officer_name_overlay_confirms_natural_family_boundary(self) -> None:
        officer_path = (
            WORKSTREAM_DIR.parent
            / "ev_strdata"
            / "public"
            / "ev_strdata_ko_officer_names_0350_0549.v0.3.json"
        )
        names = {
            entry["id"]: entry["ko"]
            for entry in load_json(officer_path)["entries"]
        }
        self.assertEqual("우시오 유키키요", names[351])
        self.assertEqual(
            [
                "오타 가즈요시",
                "오타 규이치",
            ...
            [names[entry_id] for entry_id in range(447, 458)],
        )
    ...
    def test_overlay_entries_contain_only_id_sc_hash_and_korean(self) -> None:
        ...
        self.assertEqual(107, self.overlay["entry_count"])
        entries = self.overlay["entries"]
        self.assertEqual(list(range(351, 458)), [entry["id"] for entry in entries])
    ...
    def test_alignment_is_hash_only_for_sc_jp_en(self) -> None:
        self.assertEqual(107, self.evidence["entry_count"])
        self.assertEqual(107, len(self.evidence["entries"]))
    ...
    def test_invariants_review_state_and_safety_are_recorded(self) -> None:
        ...
        self.assertEqual(107, invariants["checked"])
        self.assertEqual(107, invariants["custom_bracket_placeholder_checks"])
        ...
        self.assertEqual(107, self.review["entry_count"])
        self.assertEqual(107, self.validation["translation_status"]["human_review_required"])
        self.assertEqual(0, self.validation["translation_status"]["runtime_reviewed"])
        self.assertEqual(18, self.validation["translation_status"]["specific_uncertainty_entries"])
        ...
        for key in (
            ...
            "existing_v01_v02_v03_artifacts_modified",
        ):
