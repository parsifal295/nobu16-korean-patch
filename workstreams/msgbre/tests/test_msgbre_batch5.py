...
import build_msgbre_batch5 as batch  # noqa: E402
...
EXPECTED_HASHES = {
    ...
    f"evidence/{batch.EVIDENCE_NAME}": (
        "149CCA5AC5B421870318CBA08954F1059ED0B6991C8DDE8381276279571FE3E3"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "B2371FD63BE2749B0F39D5362D167C3B13846DB93BC6B4F19639D7F183D319B8"
    ),
    batch.VALIDATION_NAME: (
        "23FAD627AB732D65F31099F5A32E5ED349CA659B6E965AACE536878E624EA0A3"
    ),
}
...
EXPECTED_PRIOR_HASHES = {
    ...
    "public/msgbre_ko_biographies_0129_0250.v0.2.json": (
        "FDB2511970F1E8CEF194E661CBCE3F8D912B372382BCFD89129FD1F8B309A4D3"
    ),
    "evidence/alignment_evidence.v0.2.json": (
        "BB7BCEE074FC3D9A42B1E57CDD0CE635DE2F1C35CC755911379220E5399F7087"
    ),
    "review/review_index.v0.2.json": (
        "B6B281B77CF981CEF76A250420DD83E7ABC9CE8A06B0FF7F5724EF674D8ABF82"
    ),
    "validation.v0.2.json": (
        "D24AB6A3B575AB25B81209C743AE121E1D197F974A086376003F1A4581DCE2C0"
    ),
    "public/msgbre_ko_biographies_0251_0350.v0.3.json": (
        "1253609BC246DB4A63D1A5658476756AB89A0660FA683262CFFA444E3678132B"
    ),
    "evidence/alignment_evidence.v0.3.json": (
        "1AEEBEB457679CC12A01310D80EB405C6B296726330833DEA36EAB5391F88985"
    ),
    "review/review_index.v0.3.json": (
        "D1DE1B7F08E6EA7AE9B12058813EEA7C65CE4A872E9F6B1257BD7D7DE83FBACD"
    ),
    "validation.v0.3.json": (
        "AD4949BAE85C21071E642234580183497FE75BED63BA818FF29B9944FDCEFE49"
    ),
    "public/msgbre_ko_biographies_0351_0457.v0.4.json": (
        "30E38BE8C8F0D0E48FAFB828A88765AFF52BF607F1D8D22985E88CD0B810109E"
    ),
    "evidence/alignment_evidence.v0.4.json": (
        "9343B6EF3978356E33ABFB17FED9C7C36F5903CDF2F31FDA197158554CAE0513"
    ),
    "review/review_index.v0.4.json": (
        "2530714A9AACBFEF30B39F2E0F83430B22D3DAC263636A52A5CF16BB1F585C54"
    ),
    "validation.v0.4.json": (
        "F5CB3B62A9E96912E80DA7A683D749ECDD8924C5554F102F27E37D89A2281DDD"
    ),
}
...
class MsgbreBatch5Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_108_consecutive_biographies_at_oda_boundary(self) -> None:
        expected = list(range(458, 566))
        ...
        self.assertEqual(566, batch.NEXT_START_ID)
        self.assertEqual(458, self.evidence["scope"]["start_id"])
        self.assertEqual(565, self.evidence["scope"]["end_id"])
        self.assertEqual(566, self.evidence["scope"]["next_start_id"])
        boundary = self.validation["scope"]["natural_boundary"]
        self.assertEqual("oda_yorinaga", boundary["last_completed_family"])
        self.assertEqual("ochi", boundary["next_family"])
        self.assertEqual(109, boundary["officer_name_ids_crosschecked"])

    def test_officer_name_overlay_confirms_natural_family_boundary(self) -> None:
        public_dir = WORKSTREAM_DIR.parent / "ev_strdata" / "public"
        officer_paths = (
            public_dir / "ev_strdata_ko_officer_names_0350_0549.v0.3.json",
            public_dir / "ev_strdata_ko_officer_names_0550_0749.v0.4.json",
        )
        names = {}
        for officer_path in officer_paths:
            names.update(
                {
                    entry["id"]: entry["ko"]
                    for entry in load_json(officer_path)["entries"]
                }
            )
        self.assertEqual("오타카 미츠타다", names[458])
        self.assertEqual("오다 요리나가", names[565])
        self.assertEqual("오치 이에마스", names[566])
        self.assertEqual(109, len([entry_id for entry_id in range(458, 567) if entry_id in names]))
    ...
    def test_overlay_entries_contain_only_id_sc_hash_and_korean(self) -> None:
        ...
        self.assertEqual(108, self.overlay["entry_count"])
        entries = self.overlay["entries"]
        self.assertEqual(list(range(458, 566)), [entry["id"] for entry in entries])
    ...
    def test_alignment_is_hash_only_for_sc_jp_en(self) -> None:
        self.assertEqual(108, self.evidence["entry_count"])
        self.assertEqual(108, len(self.evidence["entries"]))
    ...
    def test_artifacts_are_pinned_source_free_and_prior_batches_are_unchanged(self) -> None:
        for relative, expected_hash in {**EXPECTED_HASHES, **EXPECTED_PRIOR_HASHES}.items():
    ...
    def test_invariants_review_state_and_safety_are_recorded(self) -> None:
        ...
        self.assertEqual(108, invariants["checked"])
        self.assertEqual(108, invariants["custom_bracket_placeholder_checks"])
        ...
        self.assertEqual(108, self.review["entry_count"])
        self.assertEqual(108, self.validation["translation_status"]["human_review_required"])
        self.assertEqual(0, self.validation["translation_status"]["runtime_reviewed"])
        self.assertEqual(29, self.validation["translation_status"]["specific_uncertainty_entries"])
        ...
        for key in (
            ...
            "existing_v01_v02_v03_v04_artifacts_modified",
        ):
            self.assertFalse(safety[key])

    def test_isolated_a_b_and_checked_in_final_are_byte_identical(self) -> None:
        paths = {
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for relative in EXPECTED_HASHES:
                checked_in = WORKSTREAM_DIR / relative
                with self.subTest(relative=relative):
                    self.assertEqual(
