...
function Assert-JsonStringLimits($Root, [string]$Label) {
    function Test-JsonStringNode($Value, [string]$Path) {
        ...
        if ($Value -is [string]) {
            $limit = if ($Path.EndsWith('/appended_records_hex', [StringComparison]::Ordinal)) { 8192 } else { 1024 }
...
function Assert-RecipeContracts($Manifest, $MessageRecipe, $FontRecipe) {
    ...
    if ($messageShape -ne '28D2F1833CF92148146ADA24A28A41BDCBD7F7A3BCE50DF37E53DA91E09A11E1' -or
        $messageShape -ne ([string]$Manifest.message.recipe_shape_sha256).ToUpperInvariant() -or
        $fontShape -ne '5AE655B633FAC21358C5149E36AC4862B267927DADE821600BB23D1584232244' -or
    ...
    if ($MessageRecipe.schema -ne 'nobu16.file-only-msg-recipe.v1' -or
        ...
        [int64]$MessageRecipe.target.size -ne 86298 -or
        ([string]$MessageRecipe.target.sha256).ToUpperInvariant() -ne 'B3203C89A99840AEC736936268803F685A7F2575E766AFB4C9D6E052383E86E6' -or
    ...
    if ((Get-Sha256 $MessageRecipePath) -ne '49570FE246028113C19AE2DFCB12633DC9EC23401A02EDA3B95245EB611E070D' -or
        (Get-Sha256 $MessageRecipePath) -ne ([string]$Manifest.message.recipe_sha256).ToUpperInvariant()) {
        throw 'Message recipe bytes do not match the pinned P3 public artifact'
    }

    $operations = @($MessageRecipe.operations)
    if ($operations.Count -ne [int]$Manifest.message.operation_count -or
    ...
    if ($idsSha256 -ne ([string]$MessageRecipe.operation_index.ids_sha256).ToUpperInvariant() -or
    ...
    if ($FontRecipe.schema -ne 'nobu16.file-only-g1n-tail-recipe.v1' -or
        ...
        [int64]$fontLanguage.target_archive.size -ne 179937273 -or
        ([string]$fontLanguage.target_archive.sha256).ToUpperInvariant() -ne '73E3759BF1886E95C769A95EB212F7ED34B7546E9A3DFA1EB49F542A7018E6B7' -or
    ...
    if ((Get-Sha256 $FontRecipePath) -ne '1A2B882AF265D599974B5278C409CE76B4435DF9CE5BB0FDC751B6881F0BE691' -or
        (Get-Sha256 $FontRecipePath) -ne ([string]$Manifest.font.recipe_sha256).ToUpperInvariant()) {
        throw 'Font recipe bytes do not match the pinned Font-v3 public artifact'
    }
    if ($FontRecipe.release_eligible -ne $false -or
        $FontRecipe.runtime_direct_lookup_verified -ne $false) {
        throw 'Packaged Font-v3 build recipe must remain the byte-exact public build artifact'
    }

    [string[]]$allowedCodepoints = @($FontRecipe.corpus.raster_codepoints | ForEach-Object { [string]$_ })
    if ($FontRecipe.corpus.schema -ne 'nobu16.kr.font-v3-corpus-union.v1' -or
        $allowedCodepoints.Count -ne [int]$FontRecipe.corpus.hangul_syllable_count -or
        $allowedCodepoints.Count -ne [int]$Manifest.font.raster_codepoint_count -or
        (Get-CompactJsonSha256 $allowedCodepoints) -ne 'FC01E80E6865DD0578851F0C2F82973511D232FEC8F5B1B62928EA28D96BA44E' -or
    ...
    for ($index = 0; $index -lt $allowedCodepoints.Count; $index++) {
        if ($allowedCodepoints[$index] -notmatch '\AU\+[0-9A-F]{4}\z' -or
            [Convert]::ToInt32($allowedCodepoints[$index].Substring(2), 16) -lt 0xAC00 -or
            [Convert]::ToInt32($allowedCodepoints[$index].Substring(2), 16) -gt 0xD7A3 -or
    ...
    foreach ($entryNumber in @(6, 7)) {
        ...
        $recordBytes = $allowedCodepoints.Count * 12
        $pinnedPayloadSize = if ($entryNumber -eq 6) { 520704 } else { 231424 }
        $pinnedPayloadHash = if ($entryNumber -eq 6) {
            'BBBEE931A8F856C220BFC1489BC8DF5B026C687E78C64A926A750DAC3B68F96B'
        }
        else {
            'A3319739970E41A39FBDCE1FE6DB4AF7EE91A8F165C8F741DBC524653442B3B1'
        }
        if ([int]$entry.entry -ne $entryNumber -or $tables.Count -ne 2 -or
            ...
            [int]$entry.target.size -ne
                ([int]$entry.stock.size + (2 * $recordBytes) + [int]$entry.pixel_payload.size) -or
            [int]$entry.target.atlas_offset -ne ([int]$entry.stock.atlas_offset + (2 * $recordBytes))) {
        ...
        for ($tableIndex = 0; $tableIndex -lt 2; $tableIndex++) {
            ...
            if ([int]$table.table -ne $tableIndex -or
                [int]$table.target_record_count -ne ([int]$table.source_record_count + $allowedCodepoints.Count) -or
                $changes.Count -ne $allowedCodepoints.Count -or
                ([string]$table.appended_records_hex).Length -ne ($recordBytes * 2)) {
...
function Assert-EvidenceContract($Manifest, $Evidence) {
    ...
    if ($Evidence.schema -ne 'nobu16.file-only-validation-evidence.v2' -or
        $Evidence.release_id -ne 'msgui-p3-font-v3-v0.2' -or
...
function Assert-NoCommercialFullResource([string]$Path, [string]$Relative) {
    $item = Get-Item -LiteralPath $Path
    $forbiddenSizes = @(
        [int64]60829, [int64]86298,
        [int64]160318119, [int64]179937273,
        [int64]25817936, [int64]26344064,
        [int64]11771536, [int64]12008384
    )
    $forbiddenHashes = @(
        'C2C69FDF09D9BE06E14F03C4F40562ADD0CA247EE0D50FC3E06EF501524B5E82',
        'B3203C89A99840AEC736936268803F685A7F2575E766AFB4C9D6E052383E86E6',
        '916759185E9D64E487530DCA760CD36AE1FCFF021F39CEB1658837FE60AE0D99',
        '73E3759BF1886E95C769A95EB212F7ED34B7546E9A3DFA1EB49F542A7018E6B7',
        '414A8E98DCF0F52633CD039A74E97AE61A97D98A96684D450EBADD4C3C85CAEB',
        '0210DD494916A7E576DD41EE94F81D1535ED8B92EB264F25E01C4A82719C1264',
        'DADBE4EEA223FD48CEFA9A93A08EF1F2458B3BD543ADFCEBD6D888B9EE2AFBB0',
        '9B4F8A421FC3B43A143FAFAF09D83BC5DB0FE4EED55EA71A5096FF8822FC4F5A'
    )
...
function Test-Package {
    ...
    Assert-ExactProperties $manifest.font @(
        ...
        'raster_codepoints_sha256', 'stock', 'target', 'payloads'
    ...
    if ($manifest.schema -ne 'nobu16.korean-file-only-release.v2' -or
        $manifest.release_id -ne 'msgui-p3-font-v3-v0.2' -or
        $manifest.release_name -ne 'NOBU16 Korean MSGUI P3 / Font-v3 file-only v0.2' -or
        (($manifest.release_eligible -eq $true -and $manifest.version -ne '0.2') -or
            ($manifest.release_eligible -eq $false -and $manifest.version -ne '0.2-dev')) -or
        ...
        @($manifest.files).Count -ne 15 -or
        [int64]$manifest.message.recipe_size -ne 46818 -or
        ([string]$manifest.message.recipe_sha256).ToUpperInvariant() -ne '49570FE246028113C19AE2DFCB12633DC9EC23401A02EDA3B95245EB611E070D' -or
        [int64]$manifest.font.recipe_size -ne 181488 -or
        ([string]$manifest.font.recipe_sha256).ToUpperInvariant() -ne '1A2B882AF265D599974B5278C409CE76B4435DF9CE5BB0FDC751B6881F0BE691' -or
        $manifest.backup_directory_name -ne 'msgui_p3_font_v3_v0_2' -or
        [int]$manifest.message.operation_count -le 0 -or
        [string]$manifest.message.operation_ids_sha256 -notmatch '\A[0-9A-F]{64}\z') {
    ...
    $maximumSizes = @{
        ...
        'components\font\payload\glyph_pixels_entry_6.bin' = 520704
        'components\font\payload\glyph_pixels_entry_7.bin' = 231424
        'components\font\recipe.json' = 181488
        'components\message\msgui_sc.recipe.json' = 46818
...
$BackupRoot = Join-Path $BackupParent 'msgui_p3_font_v3_v0_2'
...
if ($Action -eq 'Apply') {
    ...
    $state = [ordered]@{
        ...
        release_id = 'msgui-p3-font-v3-v0.2'
