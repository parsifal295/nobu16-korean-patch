...
namespace N16KrFileOnly
{
    public static class FileRecipeCore
    {
        ...
        public static byte[] BuildG1n(
            ...
            int[] codepoints, int[] ordinalsTable0, int[] ordinalsTable1,
            byte[] appendedTable0, byte[] appendedTable1, byte[] pixelPayload)
        {
            if (targetTableOffsets == null || targetTableOffsets.Length != 2 ||
                codepoints == null || ordinalsTable0 == null || ordinalsTable1 == null ||
                codepoints.Length != ordinalsTable0.Length ||
                codepoints.Length != ordinalsTable1.Length)
            ...
            for (int table = 0; table < 2; table++)
            {
                ...
                for (int index = 0; index < codepoints.Length; index++)
                {
                    ...
                    if (codepoint < 0 || codepoint > 0xFFFF)
                        throw new InvalidDataException("codepoint is outside BMP");
