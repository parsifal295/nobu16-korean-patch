...
$ReleaseId = 'msgui-p3-font-v3-v0.2'
$ReleaseFolderName = 'msgui_p3_file_only_v0.2-dev_2026-07-13'
...
$MessageSource = Join-Path (Split-Path -Parent $PSScriptRoot) 'build_p3_core_terms\msgui_sc.recipe.json'
$FontPublicSource = Join-Path (Split-Path -Parent $PSScriptRoot) 'font_v3\build\public'
if (-not $OutputRoot) {
...
if ($message.schema -ne 'nobu16.file-only-msg-recipe.v1' -or
    ...
    $operations.Count -ne 279 -or
    $message.operation_index.count -ne 279 -or
    $operationIdsSha256 -ne ([string]$message.operation_index.ids_sha256).ToUpperInvariant()) {
    throw 'P3 message source recipe contract failed'
}
if ((Get-Sha256 $MessageSource) -ne '49570FE246028113C19AE2DFCB12633DC9EC23401A02EDA3B95245EB611E070D') {
    throw 'P3 message recipe does not match the pinned public artifact'
}
for ($index = 0; $index -lt $operationIds.Count; $index++) {
    if (($index -gt 0 -and $operationIds[$index] -le $operationIds[$index - 1]) -or
        ...
        throw "P3 operation index is not sorted/unique at $index"
    }
}
if ($font.schema -ne 'nobu16.file-only-g1n-tail-recipe.v1' -or
    ...
    $rasterCodepoints.Count -ne 226) {
    throw 'Font-v3 public recipe contract failed'
}
if ((Get-Sha256 (Join-Path $FontPublicSource 'recipe.json')) -ne
        '1A2B882AF265D599974B5278C409CE76B4435DF9CE5BB0FDC751B6881F0BE691' -or
    (Get-Sha256 (Join-Path $FontPublicSource 'payload\glyph_pixels_entry_6.bin')) -ne
        'BBBEE931A8F856C220BFC1489BC8DF5B026C687E78C64A926A750DAC3B68F96B' -or
    ...
    throw 'Font-v3 recipe or glyph payload does not match the pinned public artifacts'
}
...
if ($OfflineEvidencePath) {
    ...
    if ($offlineEvidence.schema -ne 'nobu16.file-only-offline-validation.v2' -or
        ...
        throw 'Offline validation evidence does not match the current P3 targets'
    }
...
if ($RuntimeEvidencePath) {
    ...
    if ($runtimeEvidence.schema -ne 'nobu16.file-only-runtime-validation.v2' -or
        ...
        ([string]$runtimeEvidence.artifacts.font_target_sha256).ToUpperInvariant() -ne
            ([string]$font.languages.SC.target_archive.sha256).ToUpperInvariant()) {
        throw 'Runtime validation evidence does not match the current P3 targets'
    }
...
try {
    ...
    foreach ($directory in @(
        'components\message', 'components\font\payload', 'components\font\licenses', 'tools'
    )) {
    ...
    $manifest = [ordered]@{
        ...
        release_id = $ReleaseId
        release_name = 'NOBU16 Korean MSGUI P3 / Font-v3 file-only v0.2'
        version = if ($releaseEligible) { '0.2' } else { '0.2-dev' }
        ...
        backup_directory_name = 'msgui_p3_font_v3_v0_2'
        ...
        message = [ordered]@{
            ...
            recipe_shape_sha256 = '28D2F1833CF92148146ADA24A28A41BDCBD7F7A3BCE50DF37E53DA91E09A11E1'
        ...
        font = [ordered]@{
            ...
            recipe_shape_sha256 = '5AE655B633FAC21358C5149E36AC4862B267927DADE821600BB23D1584232244'
...
$auditScript = Join-Path $PSScriptRoot 'Audit-ReleaseVNext.ps1'
