...
if (-not $PackageRoot) {
    $PackageRoot = Join-Path $KrPatchRoot 'releases\msgui_p3_file_only_v0.2-dev_2026-07-13'
}
if (-not $EvidenceOutput) {
    $EvidenceOutput = Join-Path $KrPatchRoot 'reports\release_vnext_offline_validation_2026-07-13.json'
}
...
$TargetMessageSource = Join-Path $KrPatchRoot 'workstreams\msgui_full\build_p3_core_terms\recipe_rebuilt.msgui.bin'
$PowerShellExe = 'C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe'

$StockMessageHash = 'C2C69FDF09D9BE06E14F03C4F40562ADD0CA247EE0D50FC3E06EF501524B5E82'
$TargetMessageHash = 'B3203C89A99840AEC736936268803F685A7F2575E766AFB4C9D6E052383E86E6'
$StockFontHash = '916759185E9D64E487530DCA760CD36AE1FCFF021F39CEB1658837FE60AE0D99'
$TargetFontHash = '73E3759BF1886E95C769A95EB212F7ED34B7546E9A3DFA1EB49F542A7018E6B7'
...
function Invoke-StandaloneAudit([string]$Root) {
    $previousPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $output = & $PowerShellExe -NoLogo -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $PSScriptRoot 'Audit-ReleaseVNext.ps1') -PackageRoot $Root 2>&1 | Out-String
        $exitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $previousPreference
...
foreach ($path in @($PackageRoot, $Installer, $StockMessage, $StockFont, $TargetMessageSource)) {
...
$tempParent = [IO.Path]::GetFullPath((Join-Path $KrPatchRoot 'tmp\release_vnext_tests'))
...
try {
    ...
    $backupRoot = Join-Path $fixture 'KR_PATCH_BACKUP\msgui_p3_font_v3_v0_2'
    ...
    $renamedRoot = Join-Path $testRoot 'tamper_renamed_full_resource'
    Copy-Package $renamedRoot
    $leakPath = Join-Path $renamedRoot 'components\message\recipe_rebuilt.msgui.bin'
    Copy-Item -LiteralPath $TargetMessageSource -Destination $leakPath
    Update-ManifestFile $renamedRoot 'components/message/recipe_rebuilt.msgui.bin' $true
    ...
    try {
        ...
        $auditOutput = & $PowerShellExe -NoLogo -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $PSScriptRoot 'Audit-ReleaseVNext.ps1') -PackageRoot $markerRoot 2>&1 | Out-String
    ...
    if (@($tamperResults | Where-Object { $_ -ne $true }).Count -ne 0 -or $tamperResults.Count -ne 8) {
    ...
    $builderProbeOutput = Join-Path $KrPatchRoot ('releases\.vnext_json_guard_probe_' + [Guid]::NewGuid().ToString('N'))
    $previousPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $builderOutput = & $PowerShellExe -NoLogo -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $PSScriptRoot 'Build-ReleaseVNext.ps1') -OutputRoot $builderProbeOutput `
    ...
    $evidence = [ordered]@{
        ...
        leak_regressions = @(
            'renamed complete P3 msgui plus manifest entry rejected',
            'complete P3 msgui embedded as ignored base64 JSON field rejected',
            'unlisted nested ZIP containing complete P3 msgui rejected',
