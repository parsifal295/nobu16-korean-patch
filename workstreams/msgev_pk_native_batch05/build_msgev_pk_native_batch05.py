#!/usr/bin/env python3
"""Build the fifth native PK ``msgev`` translation batch.

The batch takes the next 300 safe semantic PK/SC target rows after batch04.
Batch04 is an explicit predecessor whether or not root progress has already
registered it.  Internal keys and punctuation-only rows remain stock; output
...
sys.path[:0] = [str(BATCH04_ROOT), str(BATCH03_ROOT), str(BATCH02_ROOT), str(BATCH01_ROOT), str(TOOLS_ROOT)]

import build_msgev_pk_native_batch04 as previous  # noqa: E402


_TRANSLATIONS_SPEC = importlib.util.spec_from_file_location(
    "_nobu16_msgev_pk_native_batch05_translations",
    WORKSTREAM_ROOT / "translations.py",
)
if _TRANSLATIONS_SPEC is None or _TRANSLATIONS_SPEC.loader is None:
    raise RuntimeError("cannot load batch05 translation table")
...
BATCH_ID = "msgev-pk-native-batch05-300.v1"
RESOURCE = "MSG_PK/SC/msgev.bin"
OVERLAY_NAME = "msgev_ko_pk_native_batch05_300.v1.json"
EVIDENCE_NAME = "msgev_pk_native_batch05_alignment.v1.json"
REVIEW_NAME = "msgev_pk_native_batch05_review.v1.json"
VALIDATION_NAME = "validation.v1.json"
SELF_OVERLAY_PATH = f"workstreams/msgev_pk_native_batch05/public/{OVERLAY_NAME}"
...
EXPECTED_BASE_PREDECESSOR_PATH_COUNT = 36
...
EXPECTED_PREDECESSOR_PATH_COUNT = 37
EXPECTED_PREDECESSOR_CLAIM_COUNT = 13_703
EXPECTED_PREDECESSOR_CLAIM_IDS_SHA256 = (
    "A635E619D5F5AD8A55DB7D7E1813B68BFA1E26802CECBBEA63F98A33C0E1A6D9"
)
EXPECTED_PREDECESSOR_TARGET_COUNT = 12_105
EXPECTED_PREDECESSOR_TARGET_IDS_SHA256 = (
    "3AD4C235AAEE0A660DFDDAC5996FD1AA1A9E88AC1E9D59F62E82EC15EDA9B2DA"
)
...
EXPECTED_PRE_BATCH_GAP_COUNT = 801
EXPECTED_PRE_BATCH_GAP_IDS_SHA256 = (
    "8951003A79FEC52C35901ADCDAD8CDC15CA7316AC7F3D60DA5428D427D80A598"
)
EXPECTED_SELECTED_IDS_SHA256 = (
    "99F42EAAD16A336A384292221A40ADA1457413D550A50C4B3F7D906335E7E0BD"
)
EXPECTED_CANDIDATE_COUNT = 443
EXPECTED_CANDIDATE_IDS_SHA256 = (
    "6F2BC6A4AAA341EA3CBBDB421D04B593F22E7233850B0822DE483F5CB35AA744"
)
EXPECTED_EXCLUDED_COUNT = 143
EXPECTED_EXCLUDED_IDS_SHA256 = (
    "C690C1F4D3DE1CB4EE0CB37BAB6C2F5A2E5EB4EAB882FF47C3AD72E66152B7DB"
)
EXPECTED_NEW_EXCLUDED_IDS_SHA256 = (
    "F06EC228C8D50AD85368AFD6411643DEABFBC3B2D9336BAFECDDF3B5DCB5803F"
)
...
NEW_PUNCTUATION_ONLY_IDS = [10_703, 10_708, 10_709, 10_716, 10_764]
NEW_EXCLUDED_IDS = NEW_PUNCTUATION_ONLY_IDS
...
def audit_progress_registration(
    ...
    """Freeze base ownership and add batch04 as an explicit predecessor.

    Root progress may not have registered batch04 yet.  Its exact pinned
    ...
    if patterns.count(EXPLICIT_PREDECESSOR_PATH) > 1:
        raise NativeBatchError("batch04 overlay is registered more than once")
    ...
    for pattern in patterns:
        ...
        if pattern == SELF_OVERLAY_PATH:
            ...
            if (
                ...
                raise NativeBatchError("registered batch04 overlay identity changed")
    ...
    if (
        ...
        raise NativeBatchError("explicit batch04 predecessor identity changed")

    claims = set(base_ids) | set(explicit_ids)
    if len(claims) != len(base_ids) + len(explicit_ids):
        raise NativeBatchError("batch04 overlaps the pinned base predecessor set")
    if (
    ...
    return {
        ...
        "explicit_batch04_overlay_sha256": sha256(explicit_blob),
        ...
        "batch04_registration_states_allowed": ["absent", "one_exact_path"],
        "batch04_counted_exactly_once": True,
...
def derive_candidate_partition(
    ...
    if len(SELECTED_IDS) != 300 or hash_json(SELECTED_IDS) != EXPECTED_SELECTED_IDS_SHA256:
        raise NativeBatchError("selected ID set differs from its pin")
    if len(NEW_EXCLUDED_IDS) != 5 or hash_json(NEW_EXCLUDED_IDS) != EXPECTED_NEW_EXCLUDED_IDS_SHA256:
    ...
    if SWITCH_SEMANTIC_REUSE_IDS or len(PK_NATIVE_IDS) != 300:
    ...
    if lexical_ids[:300] != SELECTED_IDS:
        raise NativeBatchError("selection is no longer the first 300 safe semantic gap rows")
    candidate_ids = [entry_id for entry_id in gaps if entry_id <= SELECTED_IDS[-1]]
    if candidate_ids != CANDIDATE_IDS:
        raise NativeBatchError("candidate frontier through the 300th lexical row changed")
...
def actual_input_snapshot(
    ...
    paths = {
        ...
        "explicit_batch04_overlay": repo_root / EXPLICIT_PREDECESSOR_PATH,
    }
...
def build_once(
    ...
    evidence = {
        "schema": "nobu16.kr.msgev-pk-native-batch05-evidence.v1",
        "batch_id": BATCH_ID,
        "resource": RESOURCE,
        "scope": scope,
        ...
        "translation_policy": {
            "selection": "next_300_safe_semantic_gap_rows_after_batch04",
    ...
    review = {
        "schema": "nobu16.kr.msgev-pk-native-batch05-review.v1",
        "batch_id": BATCH_ID,
        "resource": RESOURCE,
    ...
    validation = {
        "schema": "nobu16.kr.msgev-pk-native-batch05-validation.v1",
        "batch_id": BATCH_ID,
        "passed": True,
        "scope": scope,
        "target_catalog": target_public,
        ...
        "reproducibility": {
            ...
            "pre_and_post_batch04_and_self_registration_must_be_byte_identical": True,
            "successor_registration_tolerant": True,
