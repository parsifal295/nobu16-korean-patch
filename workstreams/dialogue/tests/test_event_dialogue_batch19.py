...
import build_event_dialogue_batch19 as batch  # noqa: E402
...
PREVIOUS_MANIFEST_SHA256 = (
    "53EF61C01DDE8FF9E0225D6F94A22C71440165ED2BDCBE71FB22F470410BE3FE"
)
...
class EventDialogueBatch19Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_128_displayed_entries(self) -> None:
        selected = batch.selected_ids()
        self.assertEqual(128, len(selected))
        self.assertEqual(list(range(5359, 5487)), selected)
        ...
        self.assertEqual(
            128, sum(int(event["selected_count"]) for event in batch.EVENTS)
        )
        self.assertEqual(5359, batch.EVENTS[0]["start_id"])
        self.assertEqual(5486, batch.EVENTS[-1]["end_id"])
    ...
    def test_overlay_contains_only_selected_authored_entries(self) -> None:
        ...
        self.assertEqual(128, self.overlay["entry_count"])
        self.assertEqual(
            batch.TRANSLATIONS,
    ...
    def test_alignment_evidence_is_three_language_and_source_free(self) -> None:
        self.assertEqual(128, self.evidence["entry_count"])
        self.assertEqual(128, len(self.evidence["entries"]))
        self.assertEqual([], self.evidence["excluded_internal_entries"])
        self.assertEqual(6, len(self.evidence["event_groups"]))
        self.assertEqual(
            [5358, 5359, 5382, 5383, 5411, 5412, 5438,
             5439, 5447, 5448, 5464, 5465, 5486, 5487],
    ...
    def test_previous_dialogue_artifacts_and_installed_msgev_are_unchanged(self) -> None:
        ...
        self.assertEqual(72, current_previous["file_count"])
        ...
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v18_artifacts_before"]
        )
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v18_artifacts_after"]
        )
