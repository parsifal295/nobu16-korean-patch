...
import build_event_dialogue_batch23 as batch  # noqa: E402
...
PREVIOUS_MANIFEST_SHA256 = (
    "C590C2510CA1DB1A875042858432BD8F67A3B20B3BD9BC6427D877D7723B7FFE"
)
...
class EventDialogueBatch23Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_146_displayed_entries(self) -> None:
        selected = batch.selected_ids()
        self.assertEqual(146, len(selected))
        self.assertEqual(list(range(5874, 6020)), selected)
        ...
        self.assertEqual(
            146, sum(int(event["selected_count"]) for event in batch.EVENTS)
        )
        self.assertEqual(5874, batch.EVENTS[0]["start_id"])
        self.assertEqual(6019, batch.EVENTS[-1]["end_id"])
    ...
    def test_overlay_contains_only_selected_authored_entries(self) -> None:
        ...
        self.assertEqual(146, self.overlay["entry_count"])
        self.assertEqual(
            batch.TRANSLATIONS,
    ...
    def test_alignment_evidence_is_three_language_and_source_free(self) -> None:
        self.assertEqual(146, self.evidence["entry_count"])
        self.assertEqual(146, len(self.evidence["entries"]))
        self.assertEqual([], self.evidence["excluded_internal_entries"])
        self.assertEqual(8, len(self.evidence["event_groups"]))
        self.assertEqual(
            [
                5873, 5874, 5884, 5885, 5914, 5915, 5937, 5938, 5955,
                5956, 5976, 5977, 5999, 6000, 6006, 6007, 6019, 6020,
            ],
    ...
    def test_previous_dialogue_artifacts_and_installed_msgev_are_unchanged(self) -> None:
        ...
        self.assertEqual(88, current_previous["file_count"])
        ...
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v22_artifacts_before"]
        )
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v22_artifacts_after"]
        )
