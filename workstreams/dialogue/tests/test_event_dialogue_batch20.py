...
import build_event_dialogue_batch20 as batch  # noqa: E402
...
PREVIOUS_MANIFEST_SHA256 = (
    "E92600B08DFC7A576D312F76DA8728A4B21332A72094E38695C7CC3EF6D3F885"
)
...
class EventDialogueBatch20Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_142_displayed_entries(self) -> None:
        selected = batch.selected_ids()
        self.assertEqual(142, len(selected))
        self.assertEqual(list(range(5487, 5629)), selected)
        ...
        self.assertEqual(
            142, sum(int(event["selected_count"]) for event in batch.EVENTS)
        )
        self.assertEqual(5487, batch.EVENTS[0]["start_id"])
        self.assertEqual(5628, batch.EVENTS[-1]["end_id"])
    ...
    def test_overlay_contains_only_selected_authored_entries(self) -> None:
        ...
        self.assertEqual(142, self.overlay["entry_count"])
        self.assertEqual(
            batch.TRANSLATIONS,
    ...
    def test_alignment_evidence_is_three_language_and_source_free(self) -> None:
        self.assertEqual(142, self.evidence["entry_count"])
        self.assertEqual(142, len(self.evidence["entries"]))
        self.assertEqual([], self.evidence["excluded_internal_entries"])
        self.assertEqual(6, len(self.evidence["event_groups"]))
        self.assertEqual(
            [5486, 5487, 5509, 5510, 5533, 5534, 5552,
             5553, 5574, 5575, 5593, 5594, 5628, 5629],
    ...
    def test_previous_dialogue_artifacts_and_installed_msgev_are_unchanged(self) -> None:
        ...
        self.assertEqual(76, current_previous["file_count"])
        ...
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v19_artifacts_before"]
        )
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v19_artifacts_after"]
        )
