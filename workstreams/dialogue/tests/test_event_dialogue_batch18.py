...
import build_event_dialogue_batch18 as batch  # noqa: E402
...
PREVIOUS_MANIFEST_SHA256 = (
    "EF7473AE37B1E4F6BE0B95629F6CF5021D07FBC7BB09344A195FE2B7029D4CBB"
)
...
class EventDialogueBatch18Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_121_displayed_entries(self) -> None:
        selected = batch.selected_ids()
        self.assertEqual(121, len(selected))
        self.assertEqual(list(range(5238, 5359)), selected)
        ...
        self.assertEqual(
            121, sum(int(event["selected_count"]) for event in batch.EVENTS)
        )
        self.assertEqual(5238, batch.EVENTS[0]["start_id"])
        self.assertEqual(5358, batch.EVENTS[-1]["end_id"])
    ...
    def test_overlay_contains_only_selected_authored_entries(self) -> None:
        ...
        self.assertEqual(121, self.overlay["entry_count"])
        self.assertEqual(
            batch.TRANSLATIONS,
    ...
    def test_alignment_evidence_is_three_language_and_source_free(self) -> None:
        self.assertEqual(121, self.evidence["entry_count"])
        self.assertEqual(121, len(self.evidence["entries"]))
        self.assertEqual([], self.evidence["excluded_internal_entries"])
        self.assertEqual(6, len(self.evidence["event_groups"]))
        self.assertEqual(
            [5237, 5238, 5255, 5256, 5293, 5294, 5299,
             5300, 5337, 5338, 5349, 5350, 5358, 5359],
    ...
    def test_previous_dialogue_artifacts_and_installed_msgev_are_unchanged(self) -> None:
        ...
        self.assertEqual(68, current_previous["file_count"])
        ...
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v17_artifacts_before"]
        )
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v17_artifacts_after"]
        )
