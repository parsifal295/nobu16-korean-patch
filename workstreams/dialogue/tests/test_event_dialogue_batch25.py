...
import build_event_dialogue_batch25 as batch  # noqa: E402
...
PREVIOUS_MANIFEST_SHA256 = (
    "6CA1F592F375460B645456CCCB95FEA2240A5FC24CA4D1C8106735209BE00CA4"
)
...
class EventDialogueBatch25Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_127_displayed_entries(self) -> None:
        selected = batch.selected_ids()
        self.assertEqual(127, len(selected))
        self.assertEqual(list(range(6142, 6269)), selected)
        ...
        self.assertEqual(
            127, sum(int(event["selected_count"]) for event in batch.EVENTS)
        )
        self.assertEqual(6142, batch.EVENTS[0]["start_id"])
        self.assertEqual(6268, batch.EVENTS[-1]["end_id"])
    ...
    def test_overlay_contains_only_selected_authored_entries(self) -> None:
        ...
        self.assertEqual(127, self.overlay["entry_count"])
        self.assertEqual(
            batch.TRANSLATIONS,
    ...
    def test_alignment_evidence_is_three_language_and_source_free(self) -> None:
        self.assertEqual(127, self.evidence["entry_count"])
        self.assertEqual(127, len(self.evidence["entries"]))
        self.assertEqual([], self.evidence["excluded_internal_entries"])
        self.assertEqual(5, len(self.evidence["event_groups"]))
        self.assertEqual(
            [
                6141, 6142, 6163, 6164, 6193, 6194,
                6234, 6235, 6259, 6260, 6268, 6269,
            ],
    ...
    def test_previous_dialogue_artifacts_and_installed_msgev_are_unchanged(self) -> None:
        ...
        self.assertEqual(96, current_previous["file_count"])
        ...
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v24_artifacts_before"]
        )
        self.assertEqual(
            current_previous, integrity["dialogue_v01_v24_artifacts_after"]
        )
