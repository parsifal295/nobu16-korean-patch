#!/usr/bin/env python3
"""Checks for the unfrozen selector-562 targeted-checkpoint scaffold."""
...
spec = importlib.util.spec_from_file_location(
    "selector562_delta_tested", BUILDER_PATH
)
...
class Selector562TargetedDeltaScaffoldTest(unittest.TestCase):
    def test_expected_targeted_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_PREDECESSOR_PENDING, 6_191)
        ...
        self.assertEqual(BUILDER.EXPECTED_OWNER_CHUNK_COUNTS, {0: 5, 1: 5})
        self.assertEqual(
    ...
    def test_frozen_checkpoint_after_materialization(self) -> None:
        ...
        selector = report["selector562_consolidated"]
        self.assertEqual(result["affected_row_count"], 10)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_793)
        self.assertEqual(result["runtime_review_pending"], 6_181)
        self.assertEqual(result["fully_candidate_eligible"], 46_622)
        self.assertEqual(selector["promotion_count"], 10)
