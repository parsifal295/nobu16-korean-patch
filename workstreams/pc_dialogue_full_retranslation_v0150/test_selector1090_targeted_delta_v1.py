#!/usr/bin/env python3
"""Minimal immutable-checkpoint checks for selector 1090."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_runtime_vm_post_selector1090_consolidated_checkpoint_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector1090_delta_tested", BUILDER_PATH
)
...
class Selector1090TargetedDeltaTest(unittest.TestCase):
    def test_written_checkpoint(self) -> None:
        ...
        self.assertEqual(report["result"]["affected_row_count"], 89)
        self.assertEqual(
            report["result"]["unaffected_raw_line_copy_count"], 52_714
        )
        self.assertEqual(report["result"]["runtime_review_pending"], 6_368)
        self.assertEqual(
            report["result"]["fully_candidate_eligible"], 46_435
        )
        self.assertEqual(report["result"]["confirmed_non_display"], 345)
        self.assertEqual(
            report["selector1090_consolidated"]["promotion_count"], 64
        )
        self.assertEqual(
            report["selector1090_consolidated"][
                "verification_renewal_count"
            ],
            25,
        )
        self.assertEqual(
            report["selector1090_consolidated"]["semantic_override_count"],
            33,
        )
