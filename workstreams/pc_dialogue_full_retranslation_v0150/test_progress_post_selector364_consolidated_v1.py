#!/usr/bin/env python3
"""Minimal tests for the selector-364 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector364_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector364_progress_tested", BUILDER_PATH
)
...
class Selector364ProgressTest(unittest.TestCase):
    def test_frozen_delta(self) -> None:
        ...
        self.assertEqual(after_totals["runtime_review_pending"], 6302)
        self.assertEqual(after_totals["fully_candidate_eligible"], 46501)
        self.assertEqual(
            after_totals["scope_classification_counts"]["retranslated"],
            46156,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector364_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30032)
        self.assertFalse(
    ...
    def test_targeted_checkpoint_and_source_free_guards(self) -> None:
        ...
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 5)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52798)
