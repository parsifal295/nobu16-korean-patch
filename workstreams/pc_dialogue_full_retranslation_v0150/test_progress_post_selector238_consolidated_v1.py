#!/usr/bin/env python3
"""Focused tests for the selector-238 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector238_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector238_progress_tested", BUILDER_PATH
)
...
class Selector238ProgressDeltaTest(unittest.TestCase):
    def test_expected_delta_contract(self) -> None:
        self.assertEqual(BUILDER.EXPECTED_ROWS, 52_803)
        self.assertEqual(BUILDER.EXPECTED_DECISIONS, 27)
        self.assertEqual(BUILDER.EXPECTED_PROMOTIONS, 27)
        self.assertEqual(BUILDER.EXPECTED_RENEWALS, 0)
        self.assertEqual(BUILDER.EXPECTED_OVERRIDES, 22)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PENDING, 6_151)
        self.assertEqual(BUILDER.EXPECTED_FINAL_ELIGIBLE, 46_652)
        self.assertEqual(BUILDER.EXPECTED_FINAL_RETRANSLATED, 46_307)
        self.assertEqual(BUILDER.EXPECTED_CONFIRMED_NON_DISPLAY, 345)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PROMOTED_TOTAL, 30_183)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PK_PROMOTIONS, 14_532)
        self.assertEqual(BUILDER.EXPECTED_TARGETED_AFFECTED_ROWS, 27)
        self.assertEqual(BUILDER.EXPECTED_UNAFFECTED_ROWS, 52_776)
    ...
    def test_frozen_delta_after_materialization(self) -> None:
        ...
        self.assertEqual(before["totals"]["runtime_review_pending"], 6_178)
        self.assertEqual(after["totals"]["runtime_review_pending"], 6_151)
        self.assertEqual(after["totals"]["fully_candidate_eligible"], 46_652)
        self.assertEqual(
        ...
        self.assertEqual(integration["promoted_total"], 30_183)
        delta = integration["selector238_targeted_progress_delta"]
    ...
    def test_targeted_checkpoint_after_materialization(self) -> None:
        ...
        self.assertEqual(result["pk_msggame_promotion_count"], 14_532)
        self.assertEqual(result["affected_row_count"], 27)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_776)
        self.assertFalse(validation["full_integration_engine_invoked"])
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 27)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52_776)
    ...
    def test_outputs_are_progress_only(self) -> None:
        ...
        self.assertEqual(
            ...
            "progress.post_selector238_consolidated.source_free.v1.json",
        )
        self.assertNotIn(
