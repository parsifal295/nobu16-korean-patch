#!/usr/bin/env python3
"""Scaffold tests for the selector-466 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector466_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector466_progress_tested", BUILDER_PATH
)
...
class Selector466ProgressScaffoldTest(unittest.TestCase):
    def test_expected_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_FINAL_RETRANSLATED, 46_267)
        ...
        self.assertEqual(BUILDER.EXPECTED_TARGETED_AFFECTED_ROWS, 24)
    ...
    def test_frozen_delta_after_materialization(self) -> None:
        ...
        self.assertEqual(before["totals"]["runtime_review_pending"], 6_215)
        self.assertEqual(after["totals"]["runtime_review_pending"], 6_191)
        self.assertEqual(after["totals"]["fully_candidate_eligible"], 46_612)
        self.assertEqual(
            after["totals"]["scope_classification_counts"]["retranslated"],
            46_267,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector466_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30_143)
        delta = integration["selector466_targeted_progress_delta"]
    ...
    def test_targeted_checkpoint_after_materialization(self) -> None:
        ...
        self.assertEqual(result["pk_msggame_promotion_count"], 14_492)
        ...
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 24)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52_779)
