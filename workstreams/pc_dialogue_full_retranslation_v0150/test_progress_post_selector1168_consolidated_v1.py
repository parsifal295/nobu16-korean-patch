#!/usr/bin/env python3
"""Scaffold tests for the selector-1168 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector1168_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector1168_progress_tested", BUILDER_PATH
)
...
class Selector1168ProgressScaffoldTest(unittest.TestCase):
    def test_expected_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_FINAL_ELIGIBLE, 46_520)
        self.assertEqual(BUILDER.EXPECTED_FINAL_RETRANSLATED, 46_175)
        self.assertEqual(BUILDER.EXPECTED_CONFIRMED_NON_DISPLAY, 345)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PROMOTED_TOTAL, 30_051)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PK_PROMOTIONS, 14_400)
        self.assertEqual(BUILDER.EXPECTED_TARGETED_AFFECTED_ROWS, 19)
    ...
    def test_frozen_delta_after_materialization(self) -> None:
        ...
        self.assertEqual(before["totals"]["runtime_review_pending"], 6_302)
        self.assertEqual(after["totals"]["runtime_review_pending"], 6_283)
        self.assertEqual(after["totals"]["fully_candidate_eligible"], 46_520)
        self.assertEqual(
            after["totals"]["scope_classification_counts"]["retranslated"],
            46_175,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector1168_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30_051)
        delta = integration["selector1168_targeted_progress_delta"]
    ...
    def test_targeted_checkpoint_after_materialization(self) -> None:
        ...
        self.assertEqual(result["pk_msggame_promotion_count"], 14_400)
        self.assertEqual(result["affected_row_count"], 19)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_784)
        self.assertFalse(validation["full_integration_engine_invoked"])
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 19)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52_784)
