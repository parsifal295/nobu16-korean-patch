#!/usr/bin/env python3
"""Scaffold tests for the selector-1078 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector1078_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector1078_progress_tested", BUILDER_PATH
)
...
class Selector1078ProgressScaffoldTest(unittest.TestCase):
    def test_expected_delta_contract(self) -> None:
        self.assertEqual(BUILDER.EXPECTED_ROWS, 52_803)
        self.assertEqual(BUILDER.EXPECTED_DECISIONS, 17)
        self.assertEqual(BUILDER.EXPECTED_PROMOTIONS, 17)
        self.assertEqual(BUILDER.EXPECTED_RENEWALS, 0)
        self.assertEqual(BUILDER.EXPECTED_OVERRIDES, 7)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PENDING, 6_215)
        self.assertEqual(BUILDER.EXPECTED_FINAL_ELIGIBLE, 46_588)
        self.assertEqual(BUILDER.EXPECTED_FINAL_RETRANSLATED, 46_243)
        self.assertEqual(BUILDER.EXPECTED_CONFIRMED_NON_DISPLAY, 345)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PROMOTED_TOTAL, 30_119)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PK_PROMOTIONS, 14_468)
        self.assertEqual(BUILDER.EXPECTED_TARGETED_AFFECTED_ROWS, 17)
        self.assertEqual(BUILDER.EXPECTED_UNAFFECTED_ROWS, 52_786)
    ...
    def test_pin_loss_cannot_materialize(self) -> None:
        output_before = BUILDER.DEFAULT_PROGRESS_OUTPUT.read_bytes()
        ...
        self.assertEqual(
            BUILDER.DEFAULT_PROGRESS_OUTPUT.read_bytes(), output_before
        )
        self.assertEqual(
            BUILDER.IMMUTABLE_PROGRESS_OUTPUT.read_bytes(), immutable_before
        )
    ...
    def test_frozen_delta_after_materialization(self) -> None:
        ...
        self.assertEqual(before["totals"]["runtime_review_pending"], 6_232)
        self.assertEqual(after["totals"]["runtime_review_pending"], 6_215)
        self.assertEqual(after["totals"]["fully_candidate_eligible"], 46_588)
        self.assertEqual(
            after["totals"]["scope_classification_counts"]["retranslated"],
            46_243,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector1078_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30_119)
        delta = integration["selector1078_targeted_progress_delta"]
    ...
    def test_targeted_checkpoint_after_materialization(self) -> None:
        ...
        self.assertEqual(result["pk_msggame_promotion_count"], 14_468)
        self.assertEqual(result["affected_row_count"], 17)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_786)
        self.assertFalse(validation["full_integration_engine_invoked"])
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 17)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52_786)
