#!/usr/bin/env python3
"""Build the immutable runtime-VM checkpoint after PK-only promotions.

This checkpoint contains Base, PK exact, PK residual, and PK-only
exact-blocked promotions.  Later residual closure layers bind to this
non-circular predecessor instead of reading their own final integration.
"""
...
DEFAULT_PRIVATE_OUTPUT = (
    OUTPUT_ROOT / "runtime_vm_integrated.post_pk_only_checkpoint.private.v1.jsonl"
)
...
class CheckpointError(ValueError):
    """Raised when the post-PK-only checkpoint contract drifts."""
...
INTEGRATION = load_module(
    "pc_dialogue_runtime_vm_post_pk_only_checkpoint_integration",
    INTEGRATION_PATH,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require(
        ...
        == INTEGRATION.EXPECTED_PK_POST_PK_ONLY_PROMOTIONS
        and report["promotions"]["pk_msggame"]["pk_only_layer_included"] is True
        and report["result"]["runtime_review_pending"]
        == INTEGRATION.EXPECTED_POST_PK_ONLY_PENDING_AFTER
        and report["validation"][
            "pk_only_predecessor_checkpoint_rebuilt_and_matched"
        ]
        is True
        and report["steam_write_performed"] is False,
        "post-PK-only checkpoint result drifted",
    )
    if args.write:
    ...
    if args.check:
        require(
            ...
            "private post-PK-only checkpoint drifted",
        )
        require(
            args.public_output.is_file()
            and args.public_output.read_text(encoding="utf-8") == public_content,
            "source-free post-PK-only checkpoint report drifted",
        )
    print(
        "PASS "
        ...
        "pk_only=true steam_write=false"
    )
    return 0
