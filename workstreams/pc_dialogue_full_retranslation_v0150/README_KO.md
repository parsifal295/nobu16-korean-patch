...
- `base_msggame_B001_S09`~`S11`: 좌표 `2:201:0`~`2:271:2`의 표시 리터럴
  ...
  조각을 함께 검토했다.
- 생성 검토 큐 `base_msggame-B008`과 `base_msggame-B009`는 각각 표시 대상
  199개 전부가 분류되어 의미 범위 검토가 완료됐다. 작성용 segment의 `B001`
  명칭과 생성 큐 batch 번호는 서로 다른 식별자다.
- 누적 승인·분류는 398개다. 실제 재번역 70개, 런타임 조각 257개,
  비표시 시험 데이터 71개로 구성된다.
- 정적 재번역 70개와 비표시 보존 71개, 합계 141개는 후보 반영 자격을 갖췄다.
  동적 인명·세력·수치·활용 명령과 결합되는 257개는 의미 검증을 통과했으며
  실게임 조사 결합 검수가 남았다.
