...
- `base_msggame_B001_S213`~`S222`: 좌표 `6:3570:0`~`6:3675:1`의 표시 리터럴
  ...
  `신비로운 인연`으로 나눴다.
- 생성 검토 큐 `base_msggame-B008`~`base_msggame-B036`는 표시 대상 199개짜리
  여섯 배치, 200개짜리 열아홉 배치, 198개짜리 네 배치 전부가 분류되어 의미 범위
  검토가 완료됐다.
  작성용 segment의 `B001` 명칭과 생성 큐 batch 번호는 서로 다른 식별자다.
- 누적 승인·분류는 5,786개다. 실제 재번역 2,247개, 런타임 조각 3,323개,
  비표시 내부 데이터 216개로 구성된다.
- 정적 재번역 2,247개와 비표시 보존 216개, 합계 2,463개는 후보 반영 자격을 갖췄다.
  동적 인명·세력·수치·활용 명령과 결합되는 3,323개는 의미 검증을 통과했으며
  실게임 조사 결합 검수가 남았다.
