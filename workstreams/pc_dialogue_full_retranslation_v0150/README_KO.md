...
- 생성 검토 큐 `base_msggame-B008`~`base_msggame-B032`는 표시 대상 199개짜리
  다섯 배치, 200개짜리 열일곱 배치, 198개짜리 세 배치 전부가 분류되어 의미 범위
  검토가 완료됐다.
  작성용 segment의 `B001` 명칭과 생성 큐 batch 번호는 서로 다른 식별자다.
- 누적 승인·분류는 4,989개다. 실제 재번역 2,101개, 런타임 조각 2,672개,
  비표시 내부 데이터 216개로 구성된다.
- 정적 재번역 2,101개와 비표시 보존 216개, 합계 2,317개는 후보 반영 자격을 갖췄다.
  동적 인명·세력·수치·활용 명령과 결합되는 2,672개는 의미 검증을 통과했으며
  실게임 조사 결합 검수가 남았다.
