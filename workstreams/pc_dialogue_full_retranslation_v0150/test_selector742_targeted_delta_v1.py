#!/usr/bin/env python3
"""Minimal immutable-checkpoint checks for selector 742."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_runtime_vm_post_selector742_consolidated_checkpoint_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector742_delta_tested", BUILDER_PATH
)
...
class Selector742TargetedDeltaTest(unittest.TestCase):
    def test_written_checkpoint(self) -> None:
        ...
        self.assertEqual(report["result"]["affected_row_count"], 6)
        self.assertEqual(
            report["result"]["unaffected_raw_line_copy_count"], 52_797
        )
        self.assertEqual(report["result"]["runtime_review_pending"], 6_335)
        self.assertEqual(
            report["result"]["fully_candidate_eligible"], 46_468
        )
        self.assertEqual(report["result"]["confirmed_non_display"], 345)
        self.assertEqual(
            report["selector742_consolidated"]["promotion_count"], 6
        )
        self.assertEqual(
            report["selector742_consolidated"]["verification_renewal_count"],
            0,
        )
        self.assertEqual(
            report["selector742_consolidated"]["semantic_override_count"],
            3,
        )
