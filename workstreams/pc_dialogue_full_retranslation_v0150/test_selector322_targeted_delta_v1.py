#!/usr/bin/env python3
"""Minimal immutable-checkpoint checks for selector 322."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_runtime_vm_post_selector322_consolidated_checkpoint_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector322_delta_tested", BUILDER_PATH
)
...
class Selector322TargetedDeltaTest(unittest.TestCase):
    def test_written_checkpoint(self) -> None:
        ...
        self.assertEqual(report["result"]["affected_row_count"], 28)
        self.assertEqual(
            report["result"]["unaffected_raw_line_copy_count"], 52_775
        )
        self.assertEqual(report["result"]["runtime_review_pending"], 6_310)
        self.assertEqual(
            report["result"]["fully_candidate_eligible"], 46_493
        )
        self.assertEqual(report["result"]["confirmed_non_display"], 345)
        self.assertEqual(
            report["selector322_consolidated"]["promotion_count"], 25
        )
        self.assertEqual(
            report["selector322_consolidated"]["verification_renewal_count"],
            3,
        )
        self.assertEqual(
            report["selector322_consolidated"]["semantic_override_count"],
            12,
        )
