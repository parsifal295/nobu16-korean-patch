#!/usr/bin/env python3
"""Checks for the selector-238 targeted checkpoint."""
...
spec = importlib.util.spec_from_file_location(
    "selector238_delta_tested", BUILDER_PATH
)
...
class Selector238TargetedDeltaTest(unittest.TestCase):
    def test_exact_targeted_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_PREDECESSOR_PENDING, 6_178)
        ...
        self.assertEqual(
            BUILDER.EXPECTED_OWNER_CHUNK_COUNTS, {0: 11, 1: 16}
        )
        self.assertEqual(
    ...
    def test_frozen_checkpoint_after_materialization(self) -> None:
        ...
        selector = report["selector238_consolidated"]
        self.assertEqual(result["affected_row_count"], 27)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_776)
        self.assertEqual(result["runtime_review_pending"], 6_151)
        self.assertEqual(result["fully_candidate_eligible"], 46_652)
        self.assertEqual(selector["promotion_count"], 27)
        self.assertEqual(selector["semantic_override_count"], 22)
        self.assertEqual(
