#!/usr/bin/env python3
"""Checks for the frozen selector-1078 targeted checkpoint."""
...
spec = importlib.util.spec_from_file_location(
    "selector1078_delta_tested", BUILDER_PATH
)
...
class Selector1078TargetedDeltaTest(unittest.TestCase):
    def test_expected_targeted_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_PREDECESSOR_PENDING, 6_232)
        ...
        self.assertEqual(BUILDER.EXPECTED_OWNER_CHUNK_COUNTS, {0: 7, 1: 10})
    ...
    def test_frozen_checkpoint_after_materialization(self) -> None:
        ...
        selector = report["selector1078_consolidated"]
        self.assertEqual(result["affected_row_count"], 17)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_786)
        self.assertEqual(result["runtime_review_pending"], 6_215)
        self.assertEqual(result["fully_candidate_eligible"], 46_588)
        self.assertEqual(selector["promotion_count"], 17)
        self.assertEqual(selector["semantic_override_count"], 7)
        self.assertEqual(
