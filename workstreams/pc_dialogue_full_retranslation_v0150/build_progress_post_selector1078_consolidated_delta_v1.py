#!/usr/bin/env python3
"""Prepare the source-free progress delta scaffold for selector 1078."""
...
BASE_BUILDER_PATH = (
    WORKSTREAM / "build_progress_post_selector268_consolidated_delta_v1.py"
)
...
EXPECTED_BASE_BUILDER_SHA256 = (
    "311E7D401CC5FB8AEDB09233ED285AD31BE679FD9DE9386EF0CEA8DBB926A73E"
)
...
# Frozen after the selector-1078 closure and checkpoint became immutable.
...
EXPECTED_FINAL_RETRANSLATED = 46_243
EXPECTED_CONFIRMED_NON_DISPLAY = 345
EXPECTED_TARGETED_AFFECTED_ROWS = 17
...
PREVIOUS = load_module(BASE_BUILDER_PATH, "selector1078_progress_base")
BASE = PREVIOUS.BASE
...
def require_checkpoint_pins() -> None:
    ...
    if missing:
        raise RuntimeError(
            "selector1078 progress scaffold has unresolved checkpoint pins: "
            + ", ".join(missing)
        )
...
def validate_baseline_progress(progress: Mapping[str, Any]) -> None:
    ...
    BASE.require(
        ...
        and totals.get("runtime_review_pending") == 6_232
        and totals.get("fully_candidate_eligible") == 46_571
        and scope.get("retranslated") == 46_226
        and scope.get("confirmed_non_display") == 345
        and scope.get("runtime_fragment_pending") == 6_232
        and integration.get("sha256") == EXPECTED_PREDECESSOR_PUBLIC_SHA256
        and integration.get("private_integrated_decision_sha256")
        == EXPECTED_PREDECESSOR_PRIVATE_SHA256
        and integration.get("promoted_total") == 30_102
        and integration.get("runtime_review_pending_after") == 6_232
        and integration.get("selector268_consolidated_layer_included")
        is True
        ...
        == EXPECTED_ROWS,
        "post-selector268 progress predecessor drifted",
    )
...
def build_progress_delta(
    ...
    alias_report["selector268_consolidated"] = alias_report[
    ...
    integration = progress["runtime_vm_integration"]
    new_layer = copy.deepcopy(integration["selector268_consolidated"])
    new_delta = copy.deepcopy(
        integration["selector268_targeted_progress_delta"]
    )
    exact = integration["final_exact_layers"]
    new_exact = {
        key.replace("selector268_", "selector1078_", 1):
            copy.deepcopy(value)
        for key, value in exact.items()
        if key.startswith("selector268_")
    }
    for key in list(integration):
        if key.startswith("selector"):
            del integration[key]
    integration.update(preserved)
    integration["selector1078_consolidated_layer_included"] = True
    integration["selector1078_consolidated"] = new_layer
    integration["selector1078_targeted_progress_delta"] = new_delta
    exact.clear()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--check" in arguments and EXPECTED_PROGRESS_OUTPUT_SHA256 is None:
        raise RuntimeError(
            "selector1078 progress scaffold has unresolved output hash"
        )
    BASE.require(
        BASE.sha256_file(BASE_BUILDER_PATH) == EXPECTED_BASE_BUILDER_SHA256,
        "selector268 progress delta base drifted",
    )
    BASE.require(
        BASE.sha256_file(DEFAULT_PREDECESSOR_PROGRESS)
        == EXPECTED_PREDECESSOR_PROGRESS_SHA256,
        "post-selector268 progress snapshot drifted",
    )
    configure_base()
    ...
    if EXPECTED_PROGRESS_OUTPUT_SHA256 is not None:
        BASE.require(
            ...
            == EXPECTED_PROGRESS_OUTPUT_SHA256,
            "post-selector1078 progress alias drifted",
        )
    return result
