#!/usr/bin/env python3
"""Frozen checks for the selector-226 targeted checkpoint."""
...
spec = importlib.util.spec_from_file_location(
    "selector226_delta_tested", BUILDER_PATH
)
...
class Selector226TargetedDeltaTest(unittest.TestCase):
    def report(self) -> dict:
        return json.loads(BUILDER.DEFAULT_PUBLIC_OUTPUT.read_text("ascii"))

    def test_checkpoint_pins_and_predecessor_are_frozen(self) -> None:
    ...
    def test_targeted_delta_contract_is_exact(self) -> None:
    ...
    def test_written_checkpoint_and_reverse_overlay(self) -> None:
        ...
        selector = report["selector226_consolidated"]
        self.assertEqual(result["row_count"], 52_803)
        self.assertEqual(result["affected_row_count"], 37)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_766)
        self.assertEqual(result["runtime_review_pending"], 6_246)
        self.assertEqual(result["fully_candidate_eligible"], 46_557)
        self.assertEqual(selector["promotion_count"], 37)
        self.assertEqual(selector["semantic_override_count"], 3)
        self.assertEqual(
