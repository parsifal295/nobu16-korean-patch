#!/usr/bin/env python3
"""Minimal tests for the selector-322 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector322_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector322_progress_tested", BUILDER_PATH
)
...
class Selector322ProgressTest(unittest.TestCase):
    def test_frozen_delta(self) -> None:
        ...
        self.assertEqual(after_totals["runtime_review_pending"], 6310)
        self.assertEqual(after_totals["fully_candidate_eligible"], 46493)
        self.assertEqual(
        ...
        self.assertEqual(
            after_totals["scope_classification_counts"]["retranslated"],
            before_totals["scope_classification_counts"]["retranslated"] + 25,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector322_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30024)
        self.assertFalse(
            integration["selector322_targeted_progress_delta"][
    ...
    def test_targeted_checkpoint_and_source_free_guards(self) -> None:
        ...
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 28)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52775)
