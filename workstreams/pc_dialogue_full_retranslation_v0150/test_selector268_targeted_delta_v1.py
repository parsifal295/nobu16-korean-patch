#!/usr/bin/env python3
"""Scaffold checks for the selector-268 targeted checkpoint."""
...
spec = importlib.util.spec_from_file_location(
    "selector268_delta_tested", BUILDER_PATH
)
...
class Selector268TargetedDeltaScaffoldTest(unittest.TestCase):
    def test_expected_targeted_delta_contract(self) -> None:
        ...
        self.assertEqual(BUILDER.EXPECTED_PREDECESSOR_PENDING, 6_246)
    ...
    def test_frozen_checkpoint_after_materialization(self) -> None:
        ...
        selector = report["selector268_consolidated"]
        self.assertEqual(result["affected_row_count"], 14)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_789)
        self.assertEqual(result["runtime_review_pending"], 6_232)
        self.assertEqual(result["fully_candidate_eligible"], 46_571)
        self.assertEqual(selector["promotion_count"], 14)
        self.assertEqual(selector["semantic_override_count"], 4)
        self.assertEqual(
