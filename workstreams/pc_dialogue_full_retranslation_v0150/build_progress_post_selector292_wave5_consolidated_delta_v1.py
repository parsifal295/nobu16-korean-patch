#!/usr/bin/env python3
"""Prepare the source-free progress delta for post-selector292 wave 5."""
...
BASE_BUILDER_PATH = (
    WORKSTREAM
    / "build_progress_post_selector292_wave4_consolidated_delta_v1.py"
)
...
CLOSURE_DECISIONS_PATH = (
    DIALOGUE_TMP
    / "semantic_overrides"
    / "pk_dialogue_wave_post_selector292_wave5_consolidated_closure_"
...
# Frozen after the wave-5 checkpoint becomes immutable.
...
EXPECTED_PREDECESSOR_RETRANSLATED = 46_488
EXPECTED_FINAL_RETRANSLATED: int | None = 46_502
EXPECTED_CONFIRMED_NON_DISPLAY = 345
EXPECTED_TARGETED_AFFECTED_ROWS: int | None = 14
...
PREVIOUS = load_module(BASE_BUILDER_PATH, "post292_wave4_progress_base")
BASE = PREVIOUS.BASE
...
def require_checkpoint_pins() -> None:
    ...
    if missing:
        raise RuntimeError(
            "post-selector292 wave5 progress input pins unresolved: "
            + ", ".join(missing)
        )
...
def build_progress_delta(
    ...
    old_wave_exact = {
        ...
        if key.startswith("dialogue_wave_post_selector292_wave4_")
    }
    ...
    new_layer = copy.deepcopy(
        integration["dialogue_wave_post_selector292_wave4_consolidated"]
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--check" in arguments and EXPECTED_PROGRESS_OUTPUT_SHA256 is None:
        raise RuntimeError(
            "post-selector292 wave5 progress output hash unresolved"
        )
    BASE.require(
    ...
    if EXPECTED_PROGRESS_OUTPUT_SHA256 is not None:
        BASE.require(
            ...
            == EXPECTED_PROGRESS_OUTPUT_SHA256,
            "post-selector292 wave5 progress outputs drifted",
        )
    return result
