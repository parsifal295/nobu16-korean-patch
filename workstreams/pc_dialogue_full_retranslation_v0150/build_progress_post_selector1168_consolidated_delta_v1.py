#!/usr/bin/env python3
"""Prepare the source-free progress delta for selector 1168."""
...
BASE_BUILDER_PATH = (
    WORKSTREAM / "build_progress_post_selector364_consolidated_delta_v1.py"
)
...
DEFAULT_PREDECESSOR_PROGRESS = (
    WORKSTREAM / "progress.post_selector364_consolidated.source_free.v1.json"
)
DEFAULT_PROGRESS_OUTPUT = WORKSTREAM / "progress.source_free.v1.json"
IMMUTABLE_PROGRESS_OUTPUT = (
    WORKSTREAM / "progress.post_selector1168_consolidated.source_free.v1.json"
)

EXPECTED_BASE_BUILDER_SHA256 = (
    "D8DE86D1C4251BAD2A4D094737A142C21F6A478C808EC22DA612E53391EC6298"
)
EXPECTED_PREDECESSOR_PROGRESS_SHA256 = (
    "D2D334E0B5B17E28AA5CAFC096ABC29CDAAD5D1CD10672C339FEAC60F9CE8DBE"
)
...
# Frozen from the finalized closure and targeted checkpoint.
...
EXPECTED_FINAL_RETRANSLATED = 46_175
EXPECTED_CONFIRMED_NON_DISPLAY = 345
EXPECTED_TARGETED_AFFECTED_ROWS = 19
EXPECTED_UNAFFECTED_ROWS = 52_784
EXPECTED_FULL_DIALOGUE_REBUILD = False

EXPECTED_PROGRESS_OUTPUT_SHA256: str | None = (
    "8F8F1AD119D16BDCF84E0BA5A59B8853E6DB96E622B238537BA154971D93F692"
)
...
PREVIOUS = load_module(BASE_BUILDER_PATH, "selector1168_progress_base")
BASE = PREVIOUS.BASE
...
def require_checkpoint_pins() -> None:
    ...
    if missing:
        raise RuntimeError(
            "selector1168 progress scaffold has unresolved checkpoint pins: "
            + ", ".join(missing)
        )
...
def validate_baseline_progress(progress: Mapping[str, Any]) -> None:
    ...
    BASE.require(
        ...
        and totals.get("runtime_review_pending") == 6_302
        and totals.get("fully_candidate_eligible") == 46_501
        and scope.get("retranslated") == 46_156
        and scope.get("confirmed_non_display") == 345
        and scope.get("runtime_fragment_pending") == 6_302
        and integration.get("sha256") == EXPECTED_PREDECESSOR_PUBLIC_SHA256
        and integration.get("private_integrated_decision_sha256")
        == EXPECTED_PREDECESSOR_PRIVATE_SHA256
        and integration.get("promoted_total") == 30_032
        and integration.get("runtime_review_pending_after") == 6_302
        and integration.get("selector364_consolidated_layer_included") is True
        and sum(int(row["runtime_review_pending"]) for row in segments) == 6_302
        ...
        and sum(int(row["runtime_review_pending"]) for row in batches) == 6_302
        ...
        "post-selector364 progress predecessor drifted",
    )
...
def build_progress_delta(
    ...
    alias_report["selector364_consolidated"] = alias_report[
    ...
    integration = progress["runtime_vm_integration"]
    new_layer = copy.deepcopy(integration["selector364_consolidated"])
    new_delta = copy.deepcopy(
        integration["selector364_targeted_progress_delta"]
    )
    exact = integration["final_exact_layers"]
    new_exact = {
        key.replace("selector364_", "selector1168_", 1): copy.deepcopy(value)
        for key, value in exact.items()
        if key.startswith("selector364_")
    }
    for key in list(integration):
        if key.startswith("selector"):
            del integration[key]
    integration.update(preserved)
    integration["selector1168_consolidated_layer_included"] = True
    integration["selector1168_consolidated"] = new_layer
    integration["selector1168_targeted_progress_delta"] = new_delta
    exact.clear()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--check" in arguments and EXPECTED_PROGRESS_OUTPUT_SHA256 is None:
        raise RuntimeError(
            "selector1168 progress scaffold has unresolved output hash"
        )
    BASE.require(
        BASE.sha256_file(BASE_BUILDER_PATH) == EXPECTED_BASE_BUILDER_SHA256,
        "selector364 progress delta base drifted",
    )
    BASE.require(
        BASE.sha256_file(DEFAULT_PREDECESSOR_PROGRESS)
        == EXPECTED_PREDECESSOR_PROGRESS_SHA256,
        "post-selector364 progress snapshot drifted",
    )
    configure_base()
    ...
    if EXPECTED_PROGRESS_OUTPUT_SHA256 is not None:
        BASE.require(
            ...
            == EXPECTED_PROGRESS_OUTPUT_SHA256,
            "post-selector1168 progress alias drifted",
        )
    return result
