#!/usr/bin/env python3
"""Scaffold tests for the selector-268 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector268_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector268_progress_tested", BUILDER_PATH
)
...
class Selector268ProgressScaffoldTest(unittest.TestCase):
    def test_expected_delta_contract(self) -> None:
        self.assertEqual(BUILDER.EXPECTED_ROWS, 52_803)
        self.assertEqual(BUILDER.EXPECTED_DECISIONS, 14)
        self.assertEqual(BUILDER.EXPECTED_PROMOTIONS, 14)
        self.assertEqual(BUILDER.EXPECTED_RENEWALS, 0)
        self.assertEqual(BUILDER.EXPECTED_OVERRIDES, 4)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PENDING, 6_232)
        self.assertEqual(BUILDER.EXPECTED_FINAL_ELIGIBLE, 46_571)
        self.assertEqual(BUILDER.EXPECTED_FINAL_RETRANSLATED, 46_226)
        self.assertEqual(BUILDER.EXPECTED_CONFIRMED_NON_DISPLAY, 345)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PROMOTED_TOTAL, 30_102)
        self.assertEqual(BUILDER.EXPECTED_FINAL_PK_PROMOTIONS, 14_451)
        self.assertEqual(BUILDER.EXPECTED_TARGETED_AFFECTED_ROWS, 14)
        self.assertEqual(BUILDER.EXPECTED_UNAFFECTED_ROWS, 52_789)
    ...
    def test_unresolved_scaffold_cannot_materialize(self) -> None:
        ...
        self.assertFalse(BUILDER.pins_resolved())
    ...
    def test_frozen_delta_after_materialization(self) -> None:
        ...
        self.assertEqual(before["totals"]["runtime_review_pending"], 6_246)
        self.assertEqual(after["totals"]["runtime_review_pending"], 6_232)
        self.assertEqual(after["totals"]["fully_candidate_eligible"], 46_571)
        self.assertEqual(
            after["totals"]["scope_classification_counts"]["retranslated"],
            46_226,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector268_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 30_102)
        delta = integration["selector268_targeted_progress_delta"]
    ...
    def test_targeted_checkpoint_after_materialization(self) -> None:
        ...
        self.assertEqual(result["pk_msggame_promotion_count"], 14_451)
        self.assertEqual(result["affected_row_count"], 14)
        self.assertEqual(result["unaffected_raw_line_copy_count"], 52_789)
        self.assertFalse(validation["full_integration_engine_invoked"])
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 14)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52_789)
