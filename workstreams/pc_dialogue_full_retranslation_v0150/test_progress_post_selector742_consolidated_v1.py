#!/usr/bin/env python3
"""Minimal tests for the selector-742 progress delta."""
...
BUILDER_PATH = SCRIPT.with_name(
    "build_progress_post_selector742_consolidated_delta_v1.py"
)
spec = importlib.util.spec_from_file_location(
    "selector742_progress_tested", BUILDER_PATH
)
...
class Selector742ProgressTest(unittest.TestCase):
    def test_frozen_delta(self) -> None:
        ...
        self.assertEqual(after_totals["runtime_review_pending"], 6335)
        self.assertEqual(after_totals["fully_candidate_eligible"], 46468)
        self.assertEqual(
        ...
        self.assertEqual(
            after_totals["scope_classification_counts"]["retranslated"],
            before_totals["scope_classification_counts"]["retranslated"] + 6,
        )
        integration = after["runtime_vm_integration"]
        self.assertTrue(integration["selector742_consolidated_layer_included"])
        self.assertEqual(integration["promoted_total"], 29999)
        self.assertFalse(
            integration["selector742_targeted_progress_delta"][
    ...
    def test_targeted_checkpoint_and_source_free_guards(self) -> None:
        ...
        self.assertEqual(validation["targeted_affected_rows_rechecked"], 6)
        self.assertEqual(validation["unaffected_rows_byte_copied"], 52797)
