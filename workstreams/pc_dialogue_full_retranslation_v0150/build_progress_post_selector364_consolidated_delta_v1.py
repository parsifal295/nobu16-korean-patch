#!/usr/bin/env python3
"""Advance source-free progress with the targeted selector-364 delta."""
...
BASE_BUILDER_PATH = (
    WORKSTREAM / "build_progress_post_selector1162_consolidated_delta_v1.py"
)
...
EXPECTED_BASE_BUILDER_SHA256 = (
    "80D93A66DFA9ADCDB3080DF091BDA636E9952C9B22D1AF9A920FD004F9E67650"
)
...
EXPECTED_FINAL_RETRANSLATED = 46_156
...
PREVIOUS = load_module(BASE_BUILDER_PATH, "selector364_progress_base")
BASE = PREVIOUS.BASE
...
def validate_baseline_progress(progress: Mapping[str, Any]) -> None:
    ...
    BASE.require(
        ...
        and totals.get("runtime_review_pending") == 6_307
        and totals.get("fully_candidate_eligible") == 46_496
        and scope.get("retranslated") == 46_151
        and scope.get("confirmed_non_display") == 345
        and scope.get("runtime_fragment_pending") == 6_307
        and integration.get("sha256") == EXPECTED_PREDECESSOR_PUBLIC_SHA256
        and integration.get("private_integrated_decision_sha256")
        == EXPECTED_PREDECESSOR_PRIVATE_SHA256
        and integration.get("promoted_total") == 30_027
        and integration.get("runtime_review_pending_after") == 6_307
        and integration.get("selector1162_consolidated_layer_included") is True
        and sum(int(row["runtime_review_pending"]) for row in segments) == 6_307
        ...
        and sum(int(row["runtime_review_pending"]) for row in batches) == 6_307
        ...
        "post-selector1162 progress predecessor drifted",
    )
...
def build_progress_delta(
    ...
    alias_report["selector1162_consolidated"] = alias_report[
    ...
    integration = progress["runtime_vm_integration"]
    new_layer = copy.deepcopy(integration["selector1162_consolidated"])
    new_delta = copy.deepcopy(
        integration["selector1162_targeted_progress_delta"]
    )
    exact = integration["final_exact_layers"]
    new_exact = {
        key.replace("selector1162_", "selector364_", 1): copy.deepcopy(value)
        for key, value in exact.items()
        if key.startswith("selector1162_")
    }
    for key in list(integration):
        if key.startswith("selector"):
            del integration[key]
    integration.update(preserved)
    integration["selector364_consolidated_layer_included"] = True
    integration["selector364_consolidated"] = new_layer
    integration["selector364_targeted_progress_delta"] = new_delta
    exact.clear()
...
def main(argv: Sequence[str] | None = None) -> int:
    BASE.require(
        ...
        "selector1162 progress delta base drifted",
    )
    BASE.require(
        BASE.sha256_file(DEFAULT_PREDECESSOR_PROGRESS)
        == EXPECTED_PREDECESSOR_PROGRESS_SHA256,
        "post-selector1162 progress snapshot drifted",
    )
    configure_base()
    ...
    if EXPECTED_PROGRESS_OUTPUT_SHA256 is not None:
        BASE.require(
            ...
            == EXPECTED_PROGRESS_OUTPUT_SHA256,
            "post-selector364 progress alias drifted",
        )
    return result
