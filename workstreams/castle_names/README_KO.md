...
`public/castle_names_ko_9151_9542.v0.2.json`은 ID `9151..9542`의 성 이름
392개를 사람이 전수 검수한 공개 오버레이다. 각 항목의 `status`는 모두
`reviewed`이며, 여기서 `reviewed`는 **사람 검수 완료**를 뜻한다. `method`는
v0.1 자동 초안이 어떤 방식으로 만들어졌는지 남기는 출처 메타데이터다.
...
승인된 표기 정책과 예외는 다음과 같다.

- 지명·성 이름의 일본어 `つ` 계열은 표준 한글 지명 표기에 맞춰 `쓰`를 쓴다.
- 일본어 장음은 한글에서 같은 모음을 중복하지 않는다. 중간 신뢰도 후보였던
  ID 9256 `시이즈→시즈`, ID 9447 `니이타카야마→니타카야마`도 승인했다.
- ID 9361은 지역 구분어를 살려 `빗추 마쓰야마`의 공백을 유지한다.
- 공용 성 종류 접미사 ID `9936..9940`은 v0.1과 같이 이 오버레이에 넣지 않는다.

v0.2는 SHA-256으로 고정한 v0.1과 리뷰 제안 4개만 읽어 결정적으로 생성한다.
게임 설치 파일이나 상용 원문 문자열은 빌드·테스트 입력으로 사용하지 않는다.

```powershell
$Python = "$env:LOCALAPPDATA\Programs\Python\Python312\python.exe"
& $Python -B workstreams\castle_names\build_castle_names_v02.py --check
& $Python -B -m unittest workstreams\castle_names\tests\test_castle_names_v02.py
```

| v0.2 산출물 | SHA-256 |
|---|---|
| 사람 검수 완료 오버레이 | `0CEFDE11008F4503198903E1FA25ACDDB120F6B407405EF9ACE2B01B39577E5E` |
| 검증 결과 | `785BD9CBC62FD1E3492E8B08E8F5C22EB86DF7D0087A51D9F77AAF84DC253EEB` |
...
- `public/castle_names_ko_9151_9542.v0.2.json`: 392개 사람 검수 완료 오버레이
- `reviews/*.review-proposal.v0.2.json`: 원문 없는 구간별 검수 결정 4파일
- `evidence/resource_id_map.v0.1.json`: 리소스 해시와 ID 범위 증거
- `manifest.json`, `validation.json`, `validation.v0.2.json`, `verification.json`
- v0.1 생성·검증기, v0.2 리뷰 병합기, 원문 없는 단위 테스트, 이 문서
...
if (-not $env:NOBU16_GAME_ROOT) { throw 'NOBU16_GAME_ROOT에 합법적으로 보유한 게임 설치 경로를 지정하십시오.' }

$Fontless = 'workstreams\castle_names'
$SC = 'backups\officer_name_probe_v0_1\msgdata.SC.stock.bin'
$EN = Join-Path $env:NOBU16_GAME_ROOT 'MSG_PK\EN\msgdata.bin'
$JP = Join-Path $env:NOBU16_GAME_ROOT 'MSG_PK\JP\msgdata.bin'
