#!/usr/bin/env python3
"""Build a deterministic, corpus-driven SC G1N font-v5 component.
...
BASE = load_module("nobu16_font_v5_file_only_base", TOOLS_DIR / "build_file_only_font_recipe.py")
...
def raster_request(codepoints: Iterable[int]) -> dict[str, Any]:
    return {
        "schema": "nobu16.kr.font-v5-raster-request.v1",
...
def run_rasterizer(powershell: Path, request_path: Path, output_dir: Path) -> dict[str, Any]:
    helper = SCRIPT_DIR / "rasterize_font_v5.ps1"
    command = [
    ...
    if result.get("schema") != "nobu16.kr.font-v5-raster-result.v1":
...
def build_entry(
    ...
    rebuilt = BASE.build_g1n_from_recipe(
        stock, recipe, compact_pixel_bytes, f"font-v5 entry {entry} clean recipe"
    )
...
def replay_public_recipe_v2(
    ...
    try:
        ...
        raise ValueError(f"cannot read font-v5 recipe: {exc}") from exc
    if recipe.get("schema") != "nobu16.file-only-g1n-tail-recipe.v2":
        raise ValueError("font-v5 replay requires the per-table v2 recipe schema")
    if recipe.get("file_only") is not True or recipe.get("runtime_patch_features") != []:
        raise ValueError("font-v5 recipe is not file-only")
    language_recipe = recipe.get("languages", {}).get("SC")
    if not isinstance(language_recipe, dict):
        raise ValueError("font-v5 recipe is missing SC")
    ...
    if len(stock_blob) != int(stock_info["size"]) or sha256_bytes(stock_blob) != stock_info["sha256"]:
        raise ValueError("font-v5 replay stock archive gate failed")
    stock_archive = BASE.LZ4.parse_link(stock_blob)
    if BASE.LZ4.rebuild_link(stock_archive) != stock_blob:
        raise ValueError("font-v5 replay stock LINK identity failed")
    ...
    for entry in (6, 7):
        ...
        if len(pixels) != int(payload_info["size"]) or sha256_bytes(pixels) != payload_info["sha256"]:
            raise ValueError(f"entry {entry}: font-v5 public pixel payload gate failed")
        ...
        rebuilt = BASE.build_g1n_from_recipe(stock_g1n, entry_recipe, pixels, f"font-v5 replay entry {entry}")
        replacements[entry] = rebuilt
        atomic_write(output_dir / f"SC_{entry}.font-v5.recipe.g1n", rebuilt)
    ...
    if len(rebuilt_archive) != int(language_recipe["target_archive"]["size"]):
        raise ValueError("font-v5 replay target archive size mismatch")
    if sha256_bytes(rebuilt_archive) != target_hash:
        raise ValueError("font-v5 replay target archive hash mismatch")
    parsed = BASE.LZ4.parse_link(rebuilt_archive)
    if BASE.LZ4.rebuild_link(parsed) != rebuilt_archive:
        raise ValueError("font-v5 replay target LINK identity failed")
    for index, stock_entry in enumerate(stock_archive.entries):
        if index in (6, 7):
            if BASE.extract_raw_entry(parsed, index, f"SC replay target entry {index}") != replacements[index]:
                raise ValueError(f"font-v5 replay entry {index} re-extraction mismatch")
        elif parsed.entries[index].data != stock_entry.data:
            raise ValueError(f"font-v5 replay untouched LINK entry {index} changed")
    archive_path = output_dir / "res_lang.SC.font-v5.recipe.bin"
    atomic_write(archive_path, rebuilt_archive)
    report = {
        "schema": "nobu16.kr.font-v5-recipe-replay.v1",
...
def build(args: argparse.Namespace) -> int:
    ...
    for entry in (6, 7):
        ...
        atomic_write(private_root / "candidate" / f"SC_{entry}.font-v5.g1n", candidate)
    ...
    if BASE.LZ4.rebuild_link(parsed_candidate) != candidate_archive:
        raise ValueError("font-v5 candidate LINK identity roundtrip failed")
    stock_archive = BASE.LZ4.parse_link(stock_blob)
    for index, stock_entry in enumerate(stock_archive.entries):
        if index in (6, 7):
    ...
    private_archive_path = private_root / "candidate" / "res_lang.SC.font-v5.bin"
    ...
    recipe = {
        ...
        "release_blocker": "font-v5 candidate requires in-game SC runtime render/exit validation before release eligibility",
        "corpus": {
            "schema": "nobu16.kr.font-v5-corpus-union.v1",
        ...
        "languages": {
            "SC": {
                ...
                "target_archive": {
                    "relative_path": "private/candidate/res_lang.SC.font-v5.bin",
    ...
    if replay_archive != candidate_archive:
        raise ValueError("public font-v5 recipe replay is not byte-identical to the private candidate")

    stock_hash_after = sha256_file(stock_path)
    if stock_hash_after != stock_hash_before:
        raise ValueError("stock archive changed during read-only font-v5 build")
    validation = {
        "schema": "nobu16.kr.font-v5-validation.v1",
        "passed": True,
        "demand_contract": {
            "schema_valid": True,
    ...
    manifest = {
        "schema": "nobu16.kr.font-v5-build-manifest.v1",
        "target_language": "SC",
        "file_only": True,
        "corpus": {
            "sources": union["sources"],
        ...
        "inputs": {
            ...
            "rasterizer": {"path": relative_project_path(SCRIPT_DIR / "rasterize_font_v5.ps1"), "sha256": sha256_file(SCRIPT_DIR / "rasterize_font_v5.ps1")},
        ...
        "outputs": {
            ...
            "private_candidate_archive": {
                "path": "private/candidate/res_lang.SC.font-v5.bin",
                "sha256": sha256_bytes(candidate_archive),
