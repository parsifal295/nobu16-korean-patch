...
function New-RasterGlyph([int]$Codepoint, [object]$Profile) {
    ...
    if ($Codepoint -lt 0 -or $Codepoint -gt 0xFFFF) {
        throw ('G1N font-v5 supports BMP codepoints only: U+{0:X}' -f $Codepoint)
    }
    ...
    try {
        ...
        $scaledToMargin = $false
        if ($sourceWidth -gt ($cell - 2) -or $sourceHeight -gt ($cell - 2)) {
            if ($Codepoint -ge 0xAC00 -and $Codepoint -le 0xD7A3) {
                throw ('Hangul U+{0:X4} is not clipping-safe: {1}x{2} ink in {3}px cell.' -f $Codepoint, $sourceWidth, $sourceHeight, $cell)
            }
            # Some punctuation/symbol glyphs (notably box-drawing U+2500)
            # intentionally occupy the whole em box.  Preserve all source ink
            # but fit it into the same one-pixel safety margin used by Hangul.
            # This branch never runs for Hangul and the P3 226-glyph byte-exact
            # regression gate below the builder catches any accidental drift.
        ...
        for ($y = 0; $y -lt $cell; $y++) {
            for ($x = 0; $x -lt $cell; $x += 2) {
                ...
                if ($y -ge $destY -and $y -lt ($destY + $targetHeight)) {
                    $destRelativeY = $y - $destY
                    if ($scaledToMargin) {
                        $sampleRelativeY = [Math]::Min(
                            $sourceHeight - 1,
                            [int][Math]::Floor((($destRelativeY + 0.5) * $sourceHeight) / $targetHeight)
                        )
                    } else {
                        $sampleRelativeY = $destRelativeY
                    }
                    $sampleY = $sourceMinY + $sampleRelativeY
                    if ($x -ge $destX -and $x -lt ($destX + $targetWidth)) {
                        $destRelativeX = $x - $destX
                        if ($scaledToMargin) {
                            $sampleRelativeX = [Math]::Min(
                                $sourceWidth - 1,
                                [int][Math]::Floor((($destRelativeX + 0.5) * $sourceWidth) / $targetWidth)
                            )
                        } else {
                            $sampleRelativeX = $destRelativeX
                        }
                        $left = [int]($bitmap.GetPixel($sourceMinX + $sampleRelativeX, $sampleY).R -shr 4)
                    }
                    if (($x + 1) -ge $destX -and ($x + 1) -lt ($destX + $targetWidth)) {
                        $destRelativeX = $x + 1 - $destX
                        if ($scaledToMargin) {
                            $sampleRelativeX = [Math]::Min(
                                $sourceWidth - 1,
                                [int][Math]::Floor((($destRelativeX + 0.5) * $sourceWidth) / $targetWidth)
                        ...
                        $right = [int]($bitmap.GetPixel($sourceMinX + $sampleRelativeX, $sampleY).R -shr 4)
                    }
                }
...
if ($requestData.schema -ne 'nobu16.kr.font-v5-raster-request.v1') {
...
try {
    ...
    $result = [ordered]@{
        schema = 'nobu16.kr.font-v5-raster-result.v1'
        request_sha256 = Get-FileSha256 $requestPath
        rasterizer = 'System.Drawing GDI+ AntiAliasGridFit TextContrast=4 72DPI; 2x scratch full-ink extraction; centered 4bpp copy; oversize non-Hangul nearest-neighbor fit to 1px margin'
