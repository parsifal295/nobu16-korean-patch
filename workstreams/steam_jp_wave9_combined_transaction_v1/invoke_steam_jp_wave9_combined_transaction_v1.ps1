...
# The Python builder is tmp-only. This is the separate, explicitly invoked
# writer. It accepts only the exact full Wave8 profile and pinned Wave9 target.
$repoRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$transactionTmpRoot = Join-Path $repoRoot 'tmp\steam_jp_wave9_combined_transaction_v1'
...
$transactionId = 'steam-jp-wave9-combined-transaction-v1'
$manifestSchema = 'nobu16.kr.steam-jp-wave9-combined-transaction.v1'
$stateSchema = 'nobu16.kr.steam-jp-wave9-combined-transaction-apply.v1'
...
function Get-ProfileState([string]$Root, [string]$Label) {
    ...
    Fail "$Label is neither the pinned Wave8 input nor Wave9 target profile"
}
...
function Assert-CandidateManifest([string]$CandidateRootValue, [string]$ManifestPathValue) {
    ...
    if (([string]$manifest.schema -ne $manifestSchema) -or ([string]$manifest.transaction_id -ne $transactionId)) {
        Fail 'candidate manifest is not the pinned combined Wave9 transaction'
    }
    if (@(Compare-Object -ReferenceObject $profilePaths -DifferenceObject @($manifest.profile_paths)).Count -ne 0) {
        Fail 'candidate manifest profile path order differs'
    }
    if (@(Compare-Object -ReferenceObject $changedPaths -DifferenceObject @($manifest.changed_paths)).Count -ne 0) {
        Fail 'candidate manifest changed paths differ'
    }
    ...
    Assert-Profile (Get-Profile $CandidateRootValue 'candidate profile') $targetHashes 'candidate profile'
    return $manifest
}
...
if ($Operation -eq 'RESTORE') {
    ...
    if (([string]$state.schema -ne $stateSchema) -or ([string]$state.transaction_id -ne $transactionId) -or ([string]$state.status -ne 'applied')) {
        Fail 'restore state is not an applied combined Wave9 transaction'
    }
    foreach ($relative in $changedPaths) {
        $original = Join-Path $originalRoot ($relative -replace '/', '\')
        if ((Get-Sha256 $original) -ne $inputHashes[$relative]) {
    ...
    Assert-Game-Stopped
    if ($DryRun) {
        [PSCustomObject]@{ status='PASS'; result='restore_dry_run'; writes_performed=$false; changed_paths=$changedPaths } | ConvertTo-Json -Compress
        return
    }
    foreach ($relative in $changedPaths) {
