...
import build_translation_batch4 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "B3A23F25E6632BABCC177D58299F4ACCCF8BFA62012A5541A031285440EA1722"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "50000CFA2E45121E0406977129A308C7D03F5F848BBEDD7F28D74AB8A988D2AA"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "B3E3ABA295772DF850D8FCD644383098F5BB0A14907EF6C9F765198397CE8F12"
    ),
    batch.VALIDATION_NAME: (
        "EBD595A6B1184EAB16F83508613142214D20A284200D6EBE7C2719686A64E435"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "5AE84495C7AF6F2D652CE1B050FFB7D3AE3169D539CFA410E4D424D33D8A0116"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch4Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(86, len(batch.selected_record_keys()))
        self.assertEqual((2, 566, 0), coordinates[0])
        self.assertEqual((2, 665, 2), coordinates[-1])
        self.assertEqual((2, 666, 0), batch.NEXT_COORDINATE)
        self.assertEqual(
            {(2, 659, 2), (2, 660, 1)}, set(batch.SKIPPED_CANDIDATES)
        )
        self.assertTrue(set(coordinates).isdisjoint(batch.SKIPPED_CANDIDATES))
        self.assertEqual(
            152, self.validation["selection"]["scanned_visible_candidate_count"]
        )
        self.assertEqual(
            2, self.validation["selection"]["nonlinguistic_visible_candidate_skips"]
        )
    ...
    def test_alignment_records_context_and_skip_reasons_without_source_text(self) -> None:
        self.assertEqual(86, self.evidence["record_count"])
        self.assertEqual(86, len(self.evidence["records"]))
        differing_shapes = 0
        ...
        self.assertEqual(
            [
                {
                    "coordinate": [2, 659, 2],
                    "reason": "nonlinguistic_level_format_prefix",
                },
                {
                    "coordinate": [2, 660, 1],
                    "reason": "nonlinguistic_level_format_prefix",
    ...
    def test_scope_is_the_next_150_linguistic_sc_literals(self) -> None:
        ...
        for (block_id, record_id), record in sorted(records.items()):
            if block_id != 2 or record_id < 566:
                continue
            for literal in batch.previous.parse_record_literals(record):
                ...
                if coordinate == (2, 665, 2):
                    boundary_reached = True
            if following is not None:
                break
        self.assertEqual(152, len(scanned))
        self.assertEqual(
            batch.selected_coordinates(),
            [item for item in scanned if item not in batch.SKIPPED_CANDIDATES],
        )
