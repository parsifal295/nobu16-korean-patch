...
import build_translation_batch8 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "01B0E8F1D42372CF62E67F7E8C01F43001A1E79178C9B635207281F484B2F5EC"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "29EC326D344B841AFA9A317CA204C741E5F1EB9409E032DE0DA99D12437D3303"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "919B9897CAE7779C5CBF7DA1C7ABBFD8E2415B47FF5A8830ED07BE8934C5F018"
    ),
    batch.VALIDATION_NAME: (
        "47284B92D551F2C83EA93DC136618318E11D3355D1801DBC67EF996C97E41FC3"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "402C596A5B48F79B1D92E1D8F842C2DFBFAD14FA6CD858BE65B21694E0CAC315"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch8Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(132, len(batch.selected_record_keys()))
        self.assertEqual((6, 948, 0), coordinates[0])
        self.assertEqual((6, 1205, 1), coordinates[-1])
        self.assertEqual((6, 1209, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(132, self.evidence["record_count"])
        self.assertEqual(132, len(self.evidence["records"]))
        differing_shapes = 0
