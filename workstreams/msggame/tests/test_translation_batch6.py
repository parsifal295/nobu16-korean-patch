...
import build_translation_batch6 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "19AB28AF98E115885F66D483AA8A74ADABBBC8F90E96CF54910F60CFD3C39707"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "857380B1B1EF4CCAF2CFA441B8A2724C917A644F534DB177DD592520162B2EC9"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "02203730FDF0CB2B8FF3AAEC60248EAD1C2598AAACA90DC1E07335751E3B0083"
    ),
    batch.VALIDATION_NAME: (
        "70CC3DFBC356FE38EFBCF04672826003675F56C3E408766335A25C653C6977D8"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "C2F494F958EB90E7C8F885981FDF425195419BED33D7AE547EC46B4A097A17AD"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch6Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(148, len(batch.selected_record_keys()))
        self.assertEqual((4, 76, 0), coordinates[0])
        self.assertEqual((6, 559, 0), coordinates[-1])
        self.assertEqual((6, 560, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(148, self.evidence["record_count"])
        self.assertEqual(148, len(self.evidence["records"]))
        differing_shapes = 0
