...
import build_translation_batch12 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "311204F048F768B58600AB42879095D96CE12CF0613926C9E0BE67105BEDFD01"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "718B0EE76C959643D0CCD3DD70154F383D3E225B78827F880E31D1BE83F048C9"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "EB508B57ADB7D0582D9D92A7DB7E534F601B19CC14EC236F33599A45CDA03C31"
    ),
    batch.VALIDATION_NAME: (
        "B229E486CB560A04D7A347284E67978C6724DBFD67DF26ED3B083814FB4F8585"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "B3C74347332F73124D782D5D49726E8F98DCBD6DAFDAEA78848D259B1A1EEE96"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch12Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(128, len(batch.selected_record_keys()))
        self.assertEqual((6, 1677, 1), coordinates[0])
        self.assertEqual((6, 1838, 0), coordinates[-1])
        self.assertEqual((6, 1839, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(128, self.evidence["record_count"])
        self.assertEqual(128, len(self.evidence["records"]))
        differing_shapes = 0
