...
import build_translation_batch15 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "1FC393C02A36164D30B6DEDE95337D5775A1A41DA71FB5498F474F22D0C00A47"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "3BFDF6F711365795F102DAF902C216CA9077306CE990446ED1C68BD778206328"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "499F502BC88CC36478487A02D56A534E3835E9FCE29C188FB55B19B79CF78584"
    ),
    batch.VALIDATION_NAME: (
        "B9B8F8C8FBC44F53C93731762E94A2C609067FA2A00843E0FEE93299775C500C"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "C8826A69ED4290A115F9829D9D026FA882621CB23F7EEEA0110A7E28D45CEA28"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch15Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(98, len(batch.selected_record_keys()))
        self.assertEqual((6, 2139, 2), coordinates[0])
        self.assertEqual((6, 2436, 0), coordinates[-1])
        self.assertEqual((6, 2436, 1), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(98, self.evidence["record_count"])
        self.assertEqual(98, len(self.evidence["records"]))
        differing_shapes = 0
