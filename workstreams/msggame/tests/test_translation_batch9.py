...
import build_translation_batch9 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "A97C5D95653E72817F3D94EBAC70E36759A43D2C6CBAC0F60ECCA28EDF4FE729"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "150175D8EA766867D890969CECA5E2277974BB8D0087BA88B3BD2A40D05FC30E"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "214F4F4699D531CBD3B636A993DF77D803204ABB704C68659366D5F9F7CD1A6A"
    ),
    batch.VALIDATION_NAME: (
        "3945F9F411C86D43FA38CADF9D357CF0DFA27A42FAF0A60E190FDCF1557A8ADE"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "DED44ABB78BFC019556962F22A9132288255AE24582DE692610B046D453BDCEA"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch9Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(61, len(batch.selected_record_keys()))
        self.assertEqual((6, 1209, 0), coordinates[0])
        self.assertEqual((6, 1384, 0), coordinates[-1])
        self.assertEqual((6, 1385, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(61, self.evidence["record_count"])
        self.assertEqual(61, len(self.evidence["records"]))
        differing_shapes = 0
