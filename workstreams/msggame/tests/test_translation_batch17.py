...
import build_translation_batch17 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "436CB8538888496F5D6ABD989D49B5D5A8C6658C3151290FB7BC167D4A834F99"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "1D5AF587C8E92F8F5ACED2D6464F2E290F7B3E65CC8DBD49400AE66ADE989BFC"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "E9E314DD326CCB33E0D44F0BB159F49D5F0AE943AAB9DFDE3F94803B20CAC97C"
    ),
    batch.VALIDATION_NAME: (
        "DE63CCCEE547496DA65FB745C6063056C9F577BA34A29443E3ED9EDDE009C275"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "926B13A4D75D621984915D3A9F34C6A6D2FB25648437148CF2456DCB40453963"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch17Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(102, len(batch.selected_record_keys()))
        self.assertEqual((6, 2787, 1), coordinates[0])
        self.assertEqual((6, 3050, 2), coordinates[-1])
        self.assertEqual((6, 3050, 3), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(102, self.evidence["record_count"])
        self.assertEqual(102, len(self.evidence["records"]))
        differing_shapes = 0
