...
import build_translation_batch18 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "B0279F4C3958D375BD5BA99C2F9CB38E82F8AFFD5DD8DFDB7EB2828D62F92873"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "5AED61C548E54753413080B2F122870A287CE91E697AF2FC9200B916D6577209"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "F2B369C1D34626F9FA938F083908747145583176DD41A1235B4F61525E7B6AD9"
    ),
    batch.VALIDATION_NAME: (
        "D7D36ED9842B0EA14BD1959D976EBFDB113626A25C3D2CF60DD10E00976767F8"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "5A53E7598CAC4C7915E693BB76D96A67C6E15A2F0A72425DE4A9C8E7FA6B202D"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch18Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(99, len(batch.selected_record_keys()))
        self.assertEqual((6, 3050, 3), coordinates[0])
        self.assertEqual((6, 3298, 0), coordinates[-1])
        self.assertEqual((6, 3302, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(99, self.evidence["record_count"])
        self.assertEqual(99, len(self.evidence["records"]))
        differing_shapes = 0
