...
import build_translation_batch2 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "2330BAFAE23D4C3BF6E16B47CBF9EA9CE05489337FF101441964C01CD046F0DD"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "999AB9BC14792A108588FF7DE20BB6F71D350904E6F868E08677847ECDCDE319"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "5CD21EEC768361F5ABEA3884A9E2EBBC1B2FE1629B903634B3BACA5C07B42A40"
    ),
    batch.VALIDATION_NAME: (
        "B91229454F04F9B1A486A9CB622FB3121A53B5FF7C144592990690BA45DC5E53"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "D0C7A5CBF2014C035D4B620EB468F2871ED279D57078C009F7A0D5C23CBBB7D5"
)
...
class MsggameTranslationBatch2Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(100, len(batch.selected_record_keys()))
        self.assertEqual((2, 198, 0), coordinates[0])
        self.assertEqual((2, 297, 0), coordinates[-1])
        self.assertEqual((2, 298, 0), batch.NEXT_COORDINATE)
        self.assertEqual(
            {(2, 210, 2), (2, 213, 2), (2, 214, 2)},
            set(batch.SKIPPED_CANDIDATES),
        )
        self.assertTrue(set(coordinates).isdisjoint(batch.SKIPPED_CANDIDATES))
        self.assertEqual(153, self.validation["selection"]["scanned_visible_candidate_count"])
        self.assertEqual(3, self.validation["selection"]["nonlinguistic_visible_candidate_skips"])
    ...
    def test_alignment_uses_same_record_context_and_records_skip_reasons(self) -> None:
        self.assertEqual(100, self.evidence["record_count"])
        self.assertEqual(100, len(self.evidence["records"]))
        differing_shapes = 0
        ...
        self.assertEqual(
            {"nonlinguistic_numeric_delta_prefix"},
