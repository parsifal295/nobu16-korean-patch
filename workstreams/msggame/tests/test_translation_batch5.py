...
import build_translation_batch5 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "CCECA594A287733EB7EAAD08CF13488B5E857C93D076970551C9BB0CEA9806EC"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "02332D30FB29D96B7B9F91A53F7524DD44202AA55DA0143CD707399989659910"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "553C44C0435735EF483DD9FE17037D957AC14CBCE3945EA2BAA2B9F4DE68661D"
    ),
    batch.VALIDATION_NAME: (
        "8CFFDC2EF834B0CEE20671A8AB0AD1AFE72C47C16CAA81911E561C97BB38FCE3"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "E0415F847E861E6A4911230A5A89EFCAD821A820243876739FCE4C17C0A61797"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch5Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(134, len(batch.selected_record_keys()))
        self.assertEqual((2, 666, 0), coordinates[0])
        self.assertEqual((4, 75, 0), coordinates[-1])
        self.assertEqual((4, 76, 0), batch.NEXT_COORDINATE)
        self.assertEqual(
            {(2, 676, 2), (4, 57, 1), (4, 57, 2)},
            set(batch.SKIPPED_CANDIDATES),
        )
        self.assertTrue(set(coordinates).isdisjoint(batch.SKIPPED_CANDIDATES))
        self.assertEqual(
            153, self.validation["selection"]["scanned_visible_candidate_count"]
        )
        self.assertEqual(
            3,
        ...
        self.assertEqual(
            2, self.validation["selection"]["retained_nonvisible_literal_count"]
        )
    ...
    def test_alignment_records_context_and_skip_reasons_without_source_text(self) -> None:
        self.assertEqual(134, self.evidence["record_count"])
        self.assertEqual(134, len(self.evidence["records"]))
        differing_shapes = 0
        retained_nonvisible = 0
        for record in self.evidence["records"]:
            with self.subTest(record=(record["block_id"], record["record_id"])):
                ...
                retained_nonvisible += len(record["retained_nonvisible_sc_literal_ids"])
        ...
        self.assertEqual(2, retained_nonvisible)
        self.assertEqual(
            [
                {
                    "coordinate": [2, 676, 2],
                    "reason": "nonlinguistic_numeric_bonus_prefix",
                },
                {
                    "coordinate": [4, 57, 1],
                    "reason": "nonlinguistic_repeated_bullet_separator",
                },
                {
                    "coordinate": [4, 57, 2],
                    "reason": "nonlinguistic_repeated_bullet_separator",
            ...
            self.evidence["scope"]["skipped_candidates"],
        )
        forbidden_keys = {
            "text",
            "source_text",
    ...
    def test_validation_records_binary_repack_and_safety_gates(self) -> None:
        ...
        self.assertTrue(binary["retained_nonvisible_literals_unchanged"])
        ...
        for (block_id, record_id), record in sorted(records.items()):
            ...
            for literal in batch.previous.parse_record_literals(record):
                ...
                if len(selected) == 150:
                    ...
                    break
                scanned.append(coordinate)
                if coordinate not in batch.SKIPPED_CANDIDATES:
                    selected.append(coordinate)
            if following is not None:
                break
        self.assertEqual(153, len(scanned))
