...
import build_translation_batch14 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "79EB3F94DF6AC226C8FE805C8CEA7E02F519D1EE8D3B56A84FAF8EC24284FEDB"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "72CF66D43DC93894C3E8C35E6365A27F4381F2AF48AD48601ED73EC0537B8302"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "91FBD4FA90F10D9DD4B964CE15F66F8FA791853818276874C8205A15C6EBDA4A"
    ),
    batch.VALIDATION_NAME: (
        "60BBD2FAC79A512F78B83019B42BF8CC5AECDFB3D2FFA8C322BE745C034EF3F4"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "90D76A336B4CF706CADED3BA782FCC4285F2620CE0126B04110EA812BAC972FD"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch14Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(96, len(batch.selected_record_keys()))
        self.assertEqual((6, 1993, 0), coordinates[0])
        self.assertEqual((6, 2139, 1), coordinates[-1])
        self.assertEqual((6, 2139, 2), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(96, self.evidence["record_count"])
        self.assertEqual(96, len(self.evidence["records"]))
        differing_shapes = 0
