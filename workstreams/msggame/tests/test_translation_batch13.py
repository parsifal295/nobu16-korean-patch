...
import build_translation_batch13 as batch  # noqa: E402
from build_literal_overlay import apply_overlay_blob  # noqa: E402


EXPECTED_HASHES = {
    f"public/{batch.OVERLAY_NAME}": (
        "D1D5D4D51F582951F06926AD1568016D2E85C03393D9FC064EB7030BAAE1AC9F"
    ),
    f"evidence/{batch.EVIDENCE_NAME}": (
        "30099619239992867EF4B131DC11E684E931822C836A58CAE9F92DA53E1D5933"
    ),
    f"review/{batch.REVIEW_NAME}": (
        "28BD10C9FD26F3B91AAD973B29A218BAA0C50B6B74111BBB075A0A070CA4CE9C"
    ),
    batch.VALIDATION_NAME: (
        "BE36DDDF788E97B00CBAABF7E9AD9C8D74C36427D460B48FA91F7687A7369164"
    ),
}
EXPECTED_TARGET_SHA256 = (
    "BE7147B673149EA6F1912A5C5EC30A3BD2EAB2E00C97077EBB9AEC6F2295E0F1"
)
PRIOR_OVERLAYS = (
...
class MsggameTranslationBatch13Tests(unittest.TestCase):
    @classmethod
    ...
    def test_scope_is_exactly_150_linguistic_literals(self) -> None:
        ...
        self.assertEqual(147, len(batch.selected_record_keys()))
        self.assertEqual((6, 1839, 0), coordinates[0])
        self.assertEqual((6, 1991, 0), coordinates[-1])
        self.assertEqual((6, 1993, 0), batch.NEXT_COORDINATE)
    ...
    def test_alignment_records_context_without_source_text(self) -> None:
        self.assertEqual(147, self.evidence["record_count"])
        self.assertEqual(147, len(self.evidence["records"]))
        differing_shapes = 0
