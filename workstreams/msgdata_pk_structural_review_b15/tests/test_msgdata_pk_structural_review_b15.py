...
import build_msgdata_pk_structural_review_b15 as batch  # noqa: E402


class MsgdataPkStructuralReviewB15Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.packed_sc, cls.tables = batch.previous.previous.previous.previous.previous.previous.previous.previous.load_tables(batch.GAME_ROOT)
        cls.targets = batch.previous.previous.previous.previous.previous.previous.previous.previous.previous.load_target_catalog(
    ...
    def test_eight_explicit_predecessors_own_4000_unique_ids(self) -> None:
        ...
        for path in batch.OWNER_PATHS:
            ...
            ids.extend(current)
        self.assertEqual(4000, len(set(ids)))
        self.assertTrue(set(ids).issubset(self.predecessors))

    def test_b14_is_the_direct_hash_pinned_predecessor(self) -> None:
        blob, ids = batch.load_b14_owner(REPO_ROOT)
        self.assertEqual(batch.EXPECTED_B14_OVERLAY_SHA256, batch.sha256(blob))
        self.assertEqual(500, len(ids))
        self.assertEqual(batch.previous.EXPECTED_REVIEW_IDS_SHA256, batch.hash_json(ids))
        self.assertTrue(set(ids).issubset(self.predecessors))

    def test_final_scope_is_exact_110_and_leaves_zero(self) -> None:
        self.assertEqual((28884, 29209), (self.selected[0], self.selected[-1]))
        self.assertEqual(110, len(self.selected))
        self.assertEqual(batch.EXPECTED_REVIEW_IDS_SHA256, batch.hash_json(self.selected))
        self.assertEqual(25424, len(self.predecessors))
        self.assertFalse(set(self.selected) & self.predecessors)
        self.assertEqual(set(self.selected), self.targets - self.predecessors)
        remaining = self.targets - self.predecessors - set(self.selected)
        self.assertEqual(set(), remaining)
        self.assertEqual(batch.EXPECTED_POST_REVIEW_GAP_IDS_SHA256, batch.hash_json(sorted(remaining)))

    def test_completion_union_exactly_equals_the_25534_target_catalog(self) -> None:
        completed = self.predecessors | set(self.selected)
        self.assertEqual(self.targets, completed)
        self.assertEqual(batch.EXPECTED_TARGET_COUNT, len(completed))
        self.assertEqual(batch.EXPECTED_TARGET_IDS_SHA256, batch.hash_json(sorted(completed)))
        proof = self.validation["completion_proof"]
        self.assertTrue(proof["predecessor_union_final_equals_target"])
        self.assertEqual(0, proof["remaining_count"])
        self.assertTrue(self.validation["scope"]["structural_catalog_complete"])

    def test_all_final_rows_are_restricted_ascii_lookup_keys(self) -> None:
    ...
    def test_overlay_is_reviewed_unique_nonoverlapping_and_byte_preserved(self) -> None:
        ...
        self.assertEqual(110, len(set(ids)))
    ...
    def test_four_language_final_rowsets_are_pinned(self) -> None:
        actual = {
    ...
    def test_historical_self_and_b16_registration_are_byte_stable(self) -> None:
        source_progress = json.loads((REPO_ROOT / batch.PROGRESS_RELATIVE).read_text(encoding="utf-8"))
        prefix = list(batch.previous.previous.previous.previous.previous.previous.previous.previous.EXPECTED_PREDECESSOR_PATHS)
        self_path = batch.SELF_OVERLAY_PATH
        successor = "workstreams/msgdata_pk_structural_review_b16/public/msgdata_ko_pk_structural_review_b16_500.v1.json"
        ...
        tails = (
            list(batch.OWNER_PATHS[:-3]), list(batch.OWNER_PATHS[:-2]), list(batch.OWNER_PATHS[:-1]),
            list(batch.OWNER_PATHS), list(batch.OWNER_PATHS) + [self_path],
            list(batch.OWNER_PATHS) + [self_path, successor],
        )
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b15-history-", dir=REPO_ROOT / "tmp") as directory:
    ...
    def test_public_artifacts_are_source_free_safe_and_deterministic(self) -> None:
    ...
    def test_reproducible_build_matches_checked_in_artifacts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nobu16-msgdata-b15-build-", dir=REPO_ROOT / "tmp") as directory:
