#!/usr/bin/env python3
"""Review the final 110 PK msgdata structural rows and close the target catalog."""
...
B14_ROOT = REPO_ROOT / "workstreams" / "msgdata_pk_structural_review_b14"
...
previous = load_module(
    "nobu16_msgdata_pk_structural_review_b15_previous",
    B14_ROOT / "build_msgdata_pk_structural_review_b14.py",
)
common = previous.common
engine = previous.engine
StructuralReviewError = previous.StructuralReviewError

BATCH_ID = "msgdata-pk-structural-review-b15-final-110.v1"
RESOURCE = previous.RESOURCE
OVERLAY_NAME = "msgdata_ko_pk_structural_review_b15_final_110.v1.json"
EVIDENCE_NAME = "msgdata_pk_structural_review_b15_evidence.v1.json"
REVIEW_NAME = "msgdata_pk_structural_review_b15_review.v1.json"
VALIDATION_NAME = "validation.v1.json"
SELF_OVERLAY_PATH = f"workstreams/msgdata_pk_structural_review_b15/public/{OVERLAY_NAME}"
...
EXPECTED_B14_OVERLAY_SHA256 = "755ACE3784726108F7F3676B050C82A647305A680C481D42B2710C9E02A7BD3E"
EXPECTED_PREDECESSOR_TARGET_COUNT = 25_424
EXPECTED_PREDECESSOR_TARGET_IDS_SHA256 = "E823D00C81C1B8FB40F3172ADAB846A09E397888F9745BD0B603372E15068ABC"
EXPECTED_PRE_REVIEW_GAP_COUNT = 110
EXPECTED_PRE_REVIEW_GAP_IDS_SHA256 = previous.EXPECTED_POST_REVIEW_GAP_IDS_SHA256
EXPECTED_REVIEW_COUNT = 110
EXPECTED_FIRST_ID = 28_884
EXPECTED_LAST_ID = 29_209
EXPECTED_REVIEW_IDS_SHA256 = "A6A47038C24CB939240311ECEFF9110BB4246089988DAE972EDC9E0E5A166012"
EXPECTED_POST_REVIEW_GAP_COUNT = 0
EXPECTED_POST_REVIEW_GAP_IDS_SHA256 = "4F53CDA18C2BAA0C0354BB5F9A3ECBE5ED12AB4D8E11BA873C2F11161202B945"
EXPECTED_TARGET_COUNT = 25_534
EXPECTED_TARGET_IDS_SHA256 = "B541D484A26F0B6F4306D46A344A29846331CEBC7C6381F18122F0A161C59D3E"
REASON = "romanized_or_phonetic_lookup_key"
OFFICIAL_ROWSET_SHA256 = {
    "SC": "FACE0BC3936E022E5DB0F87503DB629008015B8BD49566F57B842A5E09EF1074",
    "JP": "DA5F060ABF4409A55B310A3F3B6EC45C84291C1958A6333D31C07C776ABDADE5",
    "EN": "FACE0BC3936E022E5DB0F87503DB629008015B8BD49566F57B842A5E09EF1074",
    "TC": "006E363BB9012742A05A795CBE7543C7072306066BC3EB96D7B1B69EDD30FF48",
}
...
def load_b14_owner(repo_root: Path) -> tuple[bytes, list[int]]:
    blob = (repo_root / previous.SELF_OVERLAY_PATH).read_bytes()
    if sha256(blob) != EXPECTED_B14_OVERLAY_SHA256:
        raise StructuralReviewError("B14 direct predecessor changed")
    ...
    if resource != RESOURCE or len(ids) != 500 or hash_json(ids) != previous.EXPECTED_REVIEW_IDS_SHA256:
        raise StructuralReviewError("B14 direct predecessor ownership changed")
    return blob, ids
...
def validate_scope(
    ...
    b14_selected, reason_by_id, pre_b14_claims, owner_blobs = previous.validate_scope(tables, targets, repo_root)
    b14_blob, b14_ids = load_b14_owner(repo_root)
    if b14_ids != b14_selected:
        raise StructuralReviewError("B14 artifact and classified ownership differ")
    owner_blobs = dict(owner_blobs)
    owner_blobs[previous.SELF_OVERLAY_PATH] = b14_blob
    predecessor_claims = pre_b14_claims | set(b14_ids)
    ...
    if len(targets) != EXPECTED_TARGET_COUNT or hash_json(sorted(targets)) != EXPECTED_TARGET_IDS_SHA256:
        raise StructuralReviewError("target catalog changed")
    selected = sorted(targets - predecessor_claims)
    if len(selected) != 110 or (selected[0], selected[-1]) != (EXPECTED_FIRST_ID, EXPECTED_LAST_ID) or hash_json(selected) != EXPECTED_REVIEW_IDS_SHA256:
        raise StructuralReviewError("final structural review page changed")
    if any(reason_by_id.get(entry_id) != REASON for entry_id in selected):
        raise StructuralReviewError("mixed narrative or non-key row in final page")
    post_gap = targets - predecessor_claims - set(selected)
    if len(post_gap) != 0 or hash_json(sorted(post_gap)) != EXPECTED_POST_REVIEW_GAP_IDS_SHA256:
        raise StructuralReviewError("final structural gap is not closed")
    if predecessor_claims | set(selected) != targets:
        raise StructuralReviewError("predecessor plus B15 union does not equal target catalog")
...
def audit_progress(
    ...
    prefix = list(previous.previous.previous.previous.previous.previous.previous.previous.EXPECTED_PREDECESSOR_PATHS)
    chain = list(OWNER_PATHS) + [SELF_OVERLAY_PATH]
    historical, _successors = previous.previous.previous.registration_helper()(
        rows[0]["overlay_globs"], prefix, chain, 15, repo_root
    )
    ...
    if SELF_OVERLAY_PATH in historical and (repo_root / SELF_OVERLAY_PATH).read_bytes() != overlay_blob:
        raise StructuralReviewError("registered B15 differs from deterministic output")
    gap = targets - predecessor_claims
    post_gap = gap - selected
    if len(gap) != 110 or hash_json(sorted(gap)) != EXPECTED_REVIEW_IDS_SHA256:
        raise StructuralReviewError("pre-review gap changed")
    if selected & predecessor_claims or selected != gap:
        raise StructuralReviewError("B15 does not exactly own the final gap")
    if len(post_gap) != 0 or hash_json(sorted(post_gap)) != EXPECTED_POST_REVIEW_GAP_IDS_SHA256:
        raise StructuralReviewError("post-review gap is not zero")
    return {
        "pre_b07_registration_count": len(prefix), "explicit_structural_predecessor_count": 4000,
        ...
        "pre_review_gap_count": len(gap), "pre_review_gap_ids_sha256": EXPECTED_REVIEW_IDS_SHA256,
        "post_review_gap_count": 0, "post_review_gap_ids_sha256": EXPECTED_POST_REVIEW_GAP_IDS_SHA256,
        "completed_target_count": EXPECTED_TARGET_COUNT, "completed_target_ids_sha256": EXPECTED_TARGET_IDS_SHA256,
        "structural_remaining_zero": True,
    }


def make_files(game_root: Path, repo_root: Path, target_catalog_path: Path, progress_path: Path) -> dict[str, bytes]:
    packed_sc, tables = previous.previous.previous.previous.previous.previous.previous.previous.load_tables(game_root)
    targets = previous.previous.previous.previous.previous.previous.previous.previous.previous.load_target_catalog(target_catalog_path)["ids"]
    ...
    pin = previous.previous.previous.previous.previous.previous.previous.previous.previous.OFFICIAL_PINS["SC"]
    overlay = {
        "schema": common.OVERLAY_SCHEMA, "overlay_id": BATCH_ID, "resource": RESOURCE, "base_language": "SC", "entry_count": 110,
        ...
        "stock_sc": {"size": pin["size"], "packed_sha256": pin["sha256"], "raw_size": pin["raw_size"],
            "raw_sha256": pin["raw_sha256"], "string_count": previous.previous.previous.previous.previous.previous.previous.previous.previous.STRING_COUNT},
    ...
    reason_summary = {REASON: {"count": 110, "ids_sha256": EXPECTED_REVIEW_IDS_SHA256}}
    evidence = {
        "schema": "nobu16.kr.msgdata-pk-structural-review-evidence.v1", "batch_id": BATCH_ID, "resource": RESOURCE,
        "entry_count": 110, "contains_commercial_source_text": False, "contains_complete_game_resource": False,
        "selection": {"first_id": selected[0], "last_id": selected[-1], "reviewed_count": 110,
            "reviewed_ids_sha256": EXPECTED_REVIEW_IDS_SHA256, "structural_before": 110, "structural_after": 0,
            "narrative_mixed_count": 0, "blocked_count": 0, "catalog_complete": True},
        "completion_proof": {"target_count": EXPECTED_TARGET_COUNT, "target_ids_sha256": EXPECTED_TARGET_IDS_SHA256,
            "predecessor_count": EXPECTED_PREDECESSOR_TARGET_COUNT, "predecessor_ids_sha256": EXPECTED_PREDECESSOR_TARGET_IDS_SHA256,
            "final_batch_count": 110, "final_batch_ids_sha256": EXPECTED_REVIEW_IDS_SHA256,
            "remaining_count": 0, "remaining_ids_sha256": EXPECTED_POST_REVIEW_GAP_IDS_SHA256,
            "predecessor_union_final_equals_target": True},
    ...
    review = {
        ...
        "reviewed_count": 110, "exact_byte_preserve_count": 110, "translated_narrative_count": 0, "blocked_count": 0,
        "runtime_screen_reviewed_count": 0, "structural_remaining_count": 0, "reason_summary": reason_summary,
    ...
    validation = {
        ...
        "scope": {"reviewed_count": 110, "reviewed_ids_sha256": EXPECTED_REVIEW_IDS_SHA256, "exact_byte_preserve_count": 110,
            "translated_narrative_count": 0, "narrative_mixed_count": 0, "blocked_count": 0, "duplicate_id_count": 0,
            "predecessor_overlap_count": 0, "post_review_structural_remaining": 0, "structural_catalog_complete": True},
        "completion_proof": evidence["completion_proof"], "reason_summary": reason_summary,
        "official_selected_rowset_sha256": OFFICIAL_ROWSET_SHA256, "progress_audit": audit,
        "replacement_invariants": {"checked": 110, "exact_utf16le_byte_preserve_count": 110, "printf_preserved": True,
...
def build_reproducibly(game_root: Path, repo_root: Path, target_catalog_path: Path, progress_path: Path, out_root: Path) -> dict[str, Any]:
    ...
    return {"reviewed_count": 110, "preserve_count": 110, "blocked_count": 0, "remaining_count": 0, "files": first}
