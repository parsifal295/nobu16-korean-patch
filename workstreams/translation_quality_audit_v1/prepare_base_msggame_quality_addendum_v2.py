#!/usr/bin/env python3
"""Prepare the second PC-only base msggame quality-addendum review artifact.

This small follow-up handles three fixed, source-spelled Korean particle
errors discovered by the same full-table audit.  It reuses the v1 validator
for parser, input-hash, and packed-overlay invariants.  It reads no Switch
resources and writes only a private JSONL review artifact.
"""
...
CASES: dict[str, dict[str, Any]] = {
    "6:1152:1": {
        "expected_current": "\uc744 \ubc1b\uc544\uc8fc\uc2ed\uc2dc\uc624",
        "proposal": "\ub97c \ubc1b\uc544\uc8fc\uc2ed\uc2dc\uc624",
        "expected_layouts": base.LAYOUT_TWO,
        "issue_type": "fixed_cavalry_object_particle",
        "rationale": (
            "The fixed item noun '\uad70\ub9c8' is explicitly present at the end of the "
            "preceding Korean literal and is vowel-final.  Its object particle must be "
            "'\ub97c', matching the PC Japanese, Simplified Chinese, and Traditional Chinese "
            "purchase-and-receipt contexts."
        ),
    },
    "6:1153:1": {
        "expected_current": "\uc744 \ubc1b\uc544\uc8fc\uc2ed\uc2dc\uc624",
        "proposal": "\ub97c \ubc1b\uc544\uc8fc\uc2ed\uc2dc\uc624",
        "expected_layouts": base.LAYOUT_TWO,
        "issue_type": "fixed_firearm_object_particle",
        "rationale": (
            "The fixed item noun '\ucca0\ud3ec' is explicitly present at the end of the "
            "preceding Korean literal and is vowel-final.  Its object particle must be "
            "'\ub97c', matching the PC Japanese, Simplified Chinese, and Traditional Chinese "
            "purchase-and-receipt contexts."
        ),
    ...
    "6:3943:1": {
        "expected_current": "\uc774 \uc61b \uc601\uc9c0",
        "proposal": "\uac00 \uc61b \uc601\uc9c0",
        "expected_layouts": LAYOUT_THREE,
        "issue_type": "fixed_former_lord_subject_particle",
        "rationale": (
            "The prior literal is the fixed vowel-final noun phrase '\uc804 \uc601\uc8fc'. "
            "The following subject particle is therefore '\uac00', yielding the grammatical "
            "return-to-former-territory message preserved by the PC Japanese and both PC "
            "Chinese references."
        ),
    },
}
