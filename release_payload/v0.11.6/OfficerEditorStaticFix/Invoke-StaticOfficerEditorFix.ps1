...
$OfficerOnlyPatchedSha256 = '2E098ECB5E4335DC264F865306B990B724EA7C242B1B9F87FFC5EE2E7191797C'
$PreviousPatchedSha256 = '7CA2F1D59E02650C67F343F0776F6D05517C0486B65168E63A9AE4CBCAAFDBB2'
$PatchedSha256 = 'FD7F07A29DBD76E4AB18B1D1EE85D6B1677E0A4827A79E3732075D4CACBA8BB6'
...
$ExpectedPrincessFunctionPrefix = [byte[]](0x40,0x53,0x48,0x81,0xEC,0x80,0x00,0x00,0x00,0x83,0xB9,0xE8,0x01,0x00,0x00,0x00,0x48,0x8B,0xD9)
$OfficerEditorPatchSites = @(
...
$PrincessPatchSites = @(
    @{ Name = 'fictional princess entered-name characters'; Offset = 0x00EB3BBF; Before = [byte[]](0xE8,0x4C,0x6E,0xCE,0xFF); After = [byte[]](0xB8,0x01,0x00,0x00,0x00) },
    @{ Name = 'fictional princess entered-reading characters'; Offset = 0x00EB3BE0; Before = [byte[]](0xE8,0xCB,0x6E,0xCE,0xFF); After = [byte[]](0xB8,0x01,0x00,0x00,0x00) },
    @{ Name = 'fictional princess inherited-surname characters'; Offset = 0x00EB3C4F; Before = [byte[]](0xE8,0xBC,0x6D,0xCE,0xFF); After = [byte[]](0xB8,0x01,0x00,0x00,0x00) },
    @{ Name = 'fictional princess inherited-surname-reading characters'; Offset = 0x00EB3C78; Before = [byte[]](0xE8,0x33,0x6E,0xCE,0xFF); After = [byte[]](0xB8,0x01,0x00,0x00,0x00) }
)
$HeaderLayoutPatchSites = @(
    @{ Name = 'top-header panel width'; Offset = 0x01CA7B08; Before = [byte[]](0xB8,0x02,0x00,0x00); After = [byte[]](0x0A,0x03,0x00,0x00) },
    @{ Name = 'ruler-name textbox width'; Offset = 0x01CA7C68; Before = [byte[]](0xD8,0x00,0x00,0x00); After = [byte[]](0x7A,0x01,0x00,0x00) },
    @{ Name = 'prestige icon X'; Offset = 0x01CA7C70; Before = [byte[]](0xFB,0x01,0x00,0x00); After = [byte[]](0x83,0x02,0x00,0x00) },
    @{ Name = 'prestige value X'; Offset = 0x01CA7C80; Before = [byte[]](0x1B,0x02,0x00,0x00); After = [byte[]](0xA3,0x02,0x00,0x00) },
    @{ Name = 'labor icon X'; Offset = 0x01CA7C90; Before = [byte[]](0x6B,0x02,0x00,0x00); After = [byte[]](0xE5,0x02,0x00,0x00) },
    @{ Name = 'labor value X'; Offset = 0x01CA7CA0; Before = [byte[]](0x8B,0x02,0x00,0x00); After = [byte[]](0x05,0x03,0x00,0x00) },
    @{ Name = 'labor value width'; Offset = 0x01CA7CA8; Before = [byte[]](0x62,0x00,0x00,0x00); After = [byte[]](0x40,0x00,0x00,0x00) },
    @{ Name = 'book icon X'; Offset = 0x01CA7CB0; Before = [byte[]](0xE7,0x02,0x00,0x00); After = [byte[]](0x4F,0x03,0x00,0x00) },
    @{ Name = 'book value X'; Offset = 0x01CA7CC0; Before = [byte[]](0x07,0x03,0x00,0x00); After = [byte[]](0x6F,0x03,0x00,0x00) },
    @{ Name = 'book value width'; Offset = 0x01CA7CC8; Before = [byte[]](0x38,0x00,0x00,0x00); After = [byte[]](0x2A,0x00,0x00,0x00) },
    @{ Name = 'prestige interaction anchor X'; Offset = 0x01CA7D00; Before = [byte[]](0x12,0x02,0x00,0x00); After = [byte[]](0x99,0x02,0x00,0x00) },
    @{ Name = 'book interaction anchor X'; Offset = 0x01CA7D10; Before = [byte[]](0xFD,0x02,0x00,0x00); After = [byte[]](0x5E,0x03,0x00,0x00) }
)
...
function Install-StaticFix([string]$ResolvedGameRoot, [string]$ExecutablePath, [string]$BackupPath) {
    ...
    if ($currentHash -eq $PatchedSha256) {
        Write-Host '이미 최신 정적 성명 검증 및 상단 헤더 패치가 적용되어 있습니다.' -ForegroundColor Yellow
        return
    }
    $isOfficerOnlyPatch = $currentHash -eq $OfficerOnlyPatchedSha256
    $isPreviousPatch = $currentHash -eq $PreviousPatchedSha256
    $isKnownUpgrade = $isOfficerOnlyPatch -or $isPreviousPatch
    if ($isOfficerOnlyPatch) {
        Assert-FileHash $ExecutablePath $PatchedSize $OfficerOnlyPatchedSha256 '기존 무장 에디트 패치 EXE'
    } elseif ($isPreviousPatch) {
        Assert-FileHash $ExecutablePath $PatchedSize $PreviousPatchedSha256 '기존 성명 검증 패치 EXE'
    } else {
    ...
    } elseif ($isKnownUpgrade) {
        throw '기존 정적 패치에서 업그레이드하려면 원본 백업 NOBU16PK.exe.staticfix.original_1.1.7이 필요합니다.'
    } else {
    ...
    try {
        if ($isKnownUpgrade) {
            [byte[]]$patched = [System.IO.File]::ReadAllBytes($ExecutablePath)
            $checksumOffset = Get-PeChecksumOffset $patched
            Assert-Bytes $patched 0x00BAF510 $ExpectedFunctionPrefix '성명 검증 함수'
            Assert-Bytes $patched 0x00EB3A60 $ExpectedPrincessFunctionPrefix '가공 히메 작명 함수'
            foreach ($site in $OfficerEditorPatchSites) {
                Assert-Bytes $patched $site.Offset $site.After ($site.Name + ' 기존 패치')
            }
            if ($isOfficerOnlyPatch) {
                foreach ($site in $PrincessPatchSites) {
            ...
            foreach ($site in $HeaderLayoutPatchSites) {
                Assert-Bytes $patched $site.Offset $site.Before $site.Name
                [System.Buffer]::BlockCopy($site.After, 0, $patched, $site.Offset, $site.After.Length)
            }
        } else {
        ...
        Write-Host '적용 완료: NOBU16PK.exe가 최신 영구 정적 성명 검증 및 상단 헤더 패치 EXE로 교체되었습니다.' -ForegroundColor Green
...
function Restore-Original([string]$ExecutablePath, [string]$BackupPath) {
    ...
    if ($currentHash -ne $PatchedSha256 -and $currentHash -ne $PreviousPatchedSha256 -and $currentHash -ne $OfficerOnlyPatchedSha256) {
        throw "현재 EXE가 이 패치의 원본/기존 패치본/최신 패치본과 일치하지 않습니다: $currentHash"
    }
