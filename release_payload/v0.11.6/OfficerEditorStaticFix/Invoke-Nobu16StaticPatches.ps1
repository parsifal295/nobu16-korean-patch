...
function Get-PatchRegistry {
    ...
    if ($manifest.Schema -ne 'nobu16.static-exe-patch-registry.v1') {
    ...
    foreach ($entry in $entries) {
        ...
        foreach ($rawSite in @($raw.Sites)) {
            ...
            for ($byteOffset = $site.Offset; $byteOffset -lt ($site.Offset + $before.Length); $byteOffset++) {
                if ($occupied.ContainsKey($byteOffset)) {
                    throw ('Overlapping patch sites at 0x{0:X8}.' -f $byteOffset)
                }
                $occupied[$byteOffset] = "$id/$($site.Name)"
            }
            $sites += $site
        }
        if ($sites.Count -eq 0) {
        ...
        $patches += [pscustomobject]@{ Id = $id; Name = [string]$raw.Name; Sites = $sites; File = $file.Name }
    }
...
function Get-PatchState([byte[]]$Data, [object]$Patch) {
    ...
    foreach ($site in $Patch.Sites) {
        if (Test-BytesEqual $Data $site.Offset $site.Before) {
            $beforeCount++
        } elseif (Test-BytesEqual $Data $site.Offset $site.After) {
            $afterCount++
        } else {
...
function Assert-NormalizedBase([byte[]]$Data, [object[]]$Patches) {
    if ($Data.Length -ne $UnpackedSize) {
        throw "Unpacked EXE size mismatch. expected=$UnpackedSize actual=$($Data.Length)"
    }
    [byte[]]$normalized = Copy-Bytes $Data
    foreach ($patch in $Patches) {
        [void](Get-PatchState $Data $patch)
        foreach ($site in $patch.Sites) {
...
function Install-RegisteredPatches([string]$ResolvedGameRoot, [string]$ExecutablePath, [string]$BackupPath, [object[]]$Patches, [string]$ExpectedOutputHash) {
    ...
    } elseif ((Get-Item -LiteralPath $ExecutablePath).Length -ne $UnpackedSize) {
    ...
    try {
        ...
        if (-not $isProtectedOriginal -and $pending.Count -eq 0 -and $outputHash -eq $currentHash) {
        ...
        Assert-FileHash $stagePath $UnpackedSize $outputHash 'Staged patched EXE'
        [System.IO.File]::Replace($stagePath, $ExecutablePath, $rollbackPath)
        Assert-FileHash $ExecutablePath $UnpackedSize $outputHash 'Installed patched EXE'
