...
function Get-PatchRegistry {
    ...
    if ($manifest.Schema -ne 'nobu16.static-exe-patch-registry.v2') {
    ...
    foreach ($entry in $entries) {
        ...
        if ($kind -eq 'AppendOverlay') {
            ...
            if ($baseSize -ne $UnpackedSize -or $targetSize -le $baseSize) {
...
function Get-PatchState([byte[]]$Data, [object]$Patch) {
    ...
    if ($Patch.Kind -eq 'AppendOverlay') {
        ...
        if ($Data.Length -eq $Patch.TargetSize -and $afterCount -eq $Patch.Sites.Count) {
...
function Assert-NormalizedBase([byte[]]$Data, [object[]]$Patches) {
    ...
    $appliedStructural = @(
        $Patches |
            Where-Object { $_.Kind -eq 'AppendOverlay' -and $states[$_.Id] -eq 'Applied' } |
            Sort-Object TargetSize -Descending
    )
    foreach ($patch in $appliedStructural) {
