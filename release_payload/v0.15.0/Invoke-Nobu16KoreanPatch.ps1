...
function Write-TransactionState([string]$ResolvedGameRoot, [string]$Status, [string]$Detail) {
    $directory = Join-Path $ResolvedGameRoot 'KR_PATCH_BACKUP\v0.15.0-unified-patcher'
...
function Assert-ApplyFinalState([string]$ResolvedGameRoot, [string]$Layout) {
    ...
    if ($resource.result -ne 'already_target') {
        throw 'Resource stage did not reach the complete v0.15.0 target vector.'
    }
...
try {
    ...
    if ($Apply) {
        ...
        Write-TransactionState $resolvedGameRoot 'applied' "All mandatory resources, installed DLC files, and the $($script:SelectedCastleNameLayout) static profile reached their v0.15.0 targets. Missing DLC files were skipped."
