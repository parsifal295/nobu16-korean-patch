...
param(
    ...
    [string]$CastleNameLayout = 'Horizontal'
)
...
function Get-PatchRegistry {
    ...
    foreach ($key in @('Schema', 'Release', 'AllAppliedSha256', 'Profiles', 'Patches')) {
    ...
    for ($patchIndex = 0; $patchIndex -lt $patches.Count; $patchIndex++) {
        ...
        foreach ($site in $patch.Sites) {
            [byte[]]$currentAfter = Copy-Bytes $site.After
            $acceptedAfter = @()
            $acceptedAfter += ,(Copy-Bytes $currentAfter)
            for ($laterIndex = $patchIndex + 1; $laterIndex -lt $patches.Count; $laterIndex++) {
                $changedByLaterPatch = $false
                foreach ($laterSite in $patches[$laterIndex].Sites) {
                    $overlapStart = [Math]::Max($site.Offset, $laterSite.Offset)
                    $overlapEnd = [Math]::Min(
                        $site.Offset + $site.After.Length,
                        $laterSite.Offset + $laterSite.After.Length
                    )
                    for ($absoluteOffset = $overlapStart; $absoluteOffset -lt $overlapEnd; $absoluteOffset++) {
                        $currentAfter[$absoluteOffset - $site.Offset] =
                            $laterSite.After[$absoluteOffset - $laterSite.Offset]
                        $changedByLaterPatch = $true
                    }
                }
                if ($changedByLaterPatch) {
                    $acceptedAfter += ,(Copy-Bytes $currentAfter)
                }
            }
            $site.AcceptedAfter = $acceptedAfter
            $site.FinalAfter = Copy-Bytes $currentAfter
        }
    }
    for ($patchIndex = 0; $patchIndex -lt $patches.Count; $patchIndex++) {
        $laterSites = @()
    ...
    $profileNames = @('Vertical', 'Horizontal')
    $actualProfileNames = @($manifest.Profiles.Keys | Sort-Object)
    if (($actualProfileNames -join '|') -ne (@($profileNames | Sort-Object) -join '|')) {
        throw 'Patch registry must define exactly the Vertical and Horizontal profiles.'
    }
    $knownIds = @($patches | ForEach-Object { $_.Id })
    $expectedProfileIds = @{
        Vertical = @('001', '002', '003', '007', '009', '010')
        Horizontal = $knownIds
    }
    $profiles = @{}
    foreach ($profileName in $profileNames) {
        $rawProfile = $manifest.Profiles[$profileName]
        foreach ($key in @('PatchIds', 'OutputSha256')) {
            if (-not $rawProfile.ContainsKey($key)) {
                throw "Patch profile $profileName is missing '$key'."
            }
        }
        $profileIds = @($rawProfile.PatchIds | ForEach-Object { [string]$_ })
        if (($profileIds -join '|') -ne (@($expectedProfileIds[$profileName]) -join '|')) {
            throw "Patch profile $profileName has an unexpected patch vector."
        }
        if ([string]$rawProfile.OutputSha256 -notmatch '^[0-9A-F]{64}$') {
            throw "Patch profile $profileName has an invalid OutputSha256."
        }
        $profiles[$profileName] = [pscustomobject]@{
            Name = $profileName
            PatchIds = $profileIds
            OutputSha256 = [string]$rawProfile.OutputSha256
        }
    }
    if ($profiles.Horizontal.OutputSha256 -ne [string]$manifest.AllAppliedSha256) {
        throw 'Horizontal profile output hash must equal AllAppliedSha256.'
    }
...
function Get-PatchState([byte[]]$Data, [object]$Patch) {
    ...
    foreach ($site in $Patch.Sites) {
        if ($site.Offset -lt 0 -or ($site.Offset + $site.Before.Length) -gt $Data.Length) {
            return 'Blocked'
        }
    }
    $beforeCount = 0
    $afterCount = 0
    foreach ($site in $Patch.Sites) {
        $matchesAcceptedAfter = $false
        foreach ($acceptedAfter in @($site.AcceptedAfter)) {
            if (Test-BytesEqual $Data $site.Offset $acceptedAfter) {
                $matchesAcceptedAfter = $true
                break
            }
        }
        if (Test-BytesEqual $Data $site.Offset $site.Before) {
            $beforeCount++
        } elseif ($matchesAcceptedAfter) {
            $afterCount++
        } else {
...
function Show-PatchStates([byte[]]$Data, [object[]]$Patches, [string[]]$SelectedPatchIds, [string]$ProfileName) {
    $selected = @{}
    foreach ($id in $SelectedPatchIds) {
        $selected[$id] = $true
    }
    foreach ($patch in $Patches) {
        $state = Get-PatchState $Data $patch
        if ($selected.ContainsKey($patch.Id)) {
            Write-Host ("[{0}] {1}: {2}" -f $patch.Id, $patch.Name, $state)
        } else {
            Write-Host ("[{0}] {1}: Excluded by {2} profile ({3})" -f $patch.Id, $patch.Name, $ProfileName, $state)
        }
    }
}

function Install-RegisteredPatches(
    ...
    if ($isProtectedOriginal) {
        ...
        if ((Get-Item -LiteralPath $ExecutablePath).Length -notin @($UnpackedSize) + @($AllPatches | Where-Object { $_.Kind -eq 'AppendOverlay' } | ForEach-Object { $_.TargetSize })) {
            throw "Unsupported NOBU16PK.exe. sha256=$currentHash"
        }
        [byte[]]$current = [System.IO.File]::ReadAllBytes($ExecutablePath)
        Assert-NormalizedBase $current $AllPatches
    }
    ...
    if ($currentHash -eq $ExpectedOutputHash) {
        Write-Host "$ProfileName castle-name layout profile is already applied." -ForegroundColor Yellow
        return
    }
    ...
    try {
        # Rebuild every requested profile from the protected original.  This
        # makes Horizontal -> Vertical and Vertical -> Horizontal switches
        # deterministic and removes every patch excluded by the new profile.
        [byte[]]$patched = Get-UnpackedBytes $BackupPath $workRoot
        Assert-NormalizedBase $patched $AllPatches
        ...
        foreach ($patch in $SelectedPatches) {
            ...
            if ($state -eq 'Applied') {
                throw "Patch $($patch.Id) unexpectedly began in Applied state."
            }
        ...
        foreach ($patch in $AllPatches) {
            ...
            } elseif ($state -notin @('Pending', 'Blocked')) {
                throw "Patch $($patch.Id) was not removed by the $ProfileName profile rebuild."
            }
        }
        $outputHash = Get-Sha256Bytes $patched
        if ($outputHash -ne $ExpectedOutputHash) {
            throw "$ProfileName profile output hash mismatch. expected=$ExpectedOutputHash actual=$outputHash"
        }
        ...
        Write-Host "Applied $ProfileName castle-name layout profile. output SHA-256=$outputHash" -ForegroundColor Green
...
try {
    ...
    $profile = $registry.Profiles[$CastleNameLayout]
    if ($null -eq $profile) {
        throw "Unknown castle-name layout profile: $CastleNameLayout"
    }
    $selectedIds = @($profile.PatchIds)
    $selected = @{}
    foreach ($id in $selectedIds) {
        $selected[$id] = $true
    }
    $selectedPatches = @($patches | Where-Object { $selected.ContainsKey($_.Id) })
    if ($selectedPatches.Count -ne $selectedIds.Count) {
        throw "$CastleNameLayout profile could not resolve every registered patch."
    }
        ...
        if ($hash -eq $OriginalSha256) {
            foreach ($patch in $patches) {
                if ($selected.ContainsKey($patch.Id)) {
                    ...
                    Write-Host ("[{0}] {1}: Excluded by {2} profile" -f $patch.Id, $patch.Name, $CastleNameLayout)
                }
            }
        } else {
            [byte[]]$data = [System.IO.File]::ReadAllBytes($executablePath)
            Assert-NormalizedBase $data $patches
            Show-PatchStates $data $patches $selectedIds $CastleNameLayout
        }
    } else {
        Install-RegisteredPatches `
            $resolvedGameRoot `
            $executablePath `
            $backupPath `
            $patches `
