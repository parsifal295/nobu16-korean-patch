...
$PreviousPatchedSha256 = '2E098ECB5E4335DC264F865306B990B724EA7C242B1B9F87FFC5EE2E7191797C'
$PatchedSha256 = '7CA2F1D59E02650C67F343F0776F6D05517C0486B65168E63A9AE4CBCAAFDBB2'
...
$PatchSites = @($OfficerEditorPatchSites) + @($PrincessPatchSites)
...
function Install-StaticFix([string]$ResolvedGameRoot, [string]$ExecutablePath, [string]$BackupPath) {
    ...
    if ($currentHash -eq $PatchedSha256) {
        Write-Host '이미 최신 정적 성명 검증 패치가 적용되어 있습니다.' -ForegroundColor Yellow
        return
    }
    $isPreviousPatch = $currentHash -eq $PreviousPatchedSha256
    if ($isPreviousPatch) {
        Assert-FileHash $ExecutablePath $PatchedSize $PreviousPatchedSha256 '기존 무장 에디트 패치 EXE'
    } else {
    ...
    if (Test-Path -LiteralPath $BackupPath) {
        ...
        throw '기존 무장 에디트 패치에서 업그레이드하려면 원본 백업 NOBU16PK.exe.staticfix.original_1.1.7이 필요합니다.'
    } else {
    ...
    try {
        ...
        Write-Host '적용 완료: NOBU16PK.exe가 최신 영구 정적 성명 검증 패치 EXE로 교체되었습니다.' -ForegroundColor Green
...
function Restore-Original([string]$ExecutablePath, [string]$BackupPath) {
    ...
    if ($currentHash -ne $PatchedSha256 -and $currentHash -ne $PreviousPatchedSha256) {
