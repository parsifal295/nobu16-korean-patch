...
- 파일: [NOBU16_PK_Korean_Patch_Steam_1.1.7_v0.7.1.zip](https://github.com/parsifal295/nobu16-korean-patch/releases/download/v0.7.1/NOBU16_PK_Korean_Patch_Steam_1.1.7_v0.7.1.zip)
- ZIP SHA-256: `B54E4BD95C1049C3E84800CA643F90E6A984733FE4F2E95BE063A086CB0BA1AE`

v0.7.1은 인물 이름에서 이름만 한글이고 성씨가 한자로 남던 공통 오류를 수정해 누락된 성씨 980건을 복구합니다. v0.7.0의 QHD 한글 글꼴 수정과 고해상도 일본어 PK 글꼴 4파일도 그대로 포함합니다.
...
| PK 공용 메시지 5종 | 한국어 변경 40,487 · 구조 확인 1,796 / 원천 합집합 43,169 | 검토 886 |
| PK 본문 `msggame.bin` | 적용 28,272 / 28,272 (100.0%) | 0 |
| 공용 `strdata.bin` | 안전 이식 24,524 / 24,525 | 1 |
| 일본어 경로 한글 폰트 | 4 / 4 포함 (v0.7.0 화면 검증 자원 유지) | 0 |

`msgui.bin`의 안전 이식 4,036건 중 실제 문구 변경은 3,955건이고, 81건은 원문과
동일합니다. 보류 1건은 번역 대상 문구가 아닌 비의미 공백 1자 레코드입니다.
v0.7.1에서는 인물 성씨 누락 980건을 복구했습니다. 별도 번역 묶음의 검토된 의미 간극 94건은 이번 오류수정 릴리즈에 포함하지 않았습니다.
공용 메시지 원천 합집합 43,169건은 한국어 변경 40,487건, 원문 동일 구조 확인 1,796건, 형식 계약 검토 730건, 정렬 간극 156건으로 정확히 분해됩니다.
남은 검토 886건은 형식 계약과 정렬 감사 대상이며, 정렬 간극에는 내부 더미 2건이 포함됩니다. 전역 번역 완료율이나 미번역 문장 수로 해석하면 안 됩니다.
공용 메시지의 중복 문맥 50건은 동일 대상으로 합쳐졌습니다. 수치는
구조를 보존해 현재 Steam 1.1.7 파일에 안전하게 이식된 항목
기준이며, 줄바꿈·잘림·
문맥 검수 완료율을 뜻하지는 않습니다.

v0.7.1 exact-12 파일 적용·해시 확인과 `오다 노부나가` 바이너리 재조합 검증을 마쳤습니다. 화면 검증은 PENDING이며, 기존 v0.7.0의 한글 타이틀·메인 메뉴와 4개 한글 폰트 화면 검증 자원을 그대로 유지합니다.
