...
파싱이 끝난 10개 메시지 리소스 기준 **번역 완료 15,018 / 111,951 (13.4%)**, 초벌 커버리지는 **15,018개**다.
...
| `MSG/SC/ev_strdata.bin` | 2,207 / 11,687 | 2,207 | 17,868 슬롯 | 18.9% `██░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–2206, 2,207개 완료; 다음 ID 2207부터 이벤트 제목·라벨 구간 |
...
| 본편 `ev_strdata` v0.1–v0.12 | 파일 선두 장수 전체 이름표 ID 0–2206, 2,207개 전부 완료. 기존 장수명 해시 일치 2,205개를 재사용하고 SC 표기 변형 2개를 독립 대조했으며 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; ID 2207–3200 이벤트 제목·라벨 994개가 다음 구간 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.11 1,170개, 장수 열전 566개, 본편 장수명 2,207개, `msggame` 450개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
4. 후속 대사 1,170개, 장수 열전 566개, 본편 장수명 2,207개, `msggame` 450개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
