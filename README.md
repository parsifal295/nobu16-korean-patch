...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 12,410 / 83,201 (14.9%)**, 초벌 커버리지는 **12,410개**다.
...
| `MSG_PK/SC/msgev.bin` | 3,019 / 12,906 | 3,019 | 17,910 슬롯 | 23.4% `██░░░░░░░░` | 장수명 2,207 + 대사 812 |
| `MSG_PK/SC/msgdata.bin` | 4,223 / 25,534 | 4,223 | 29,210 슬롯 | 16.5% `██░░░░░░░░` | 장수 구성요소 + 성 이름 + 옛 지방명; 검수 대기 0 |
| `MSG_PK/SC/msgbre.bin` | 251 / 2,217 | 251 | 3,000 슬롯 | 11.3% `█░░░░░░░░░` | 장수 열전 ID 0–250, 251개 초벌; 다음 ID 251 |
...
| `MSG/SC/ev_strdata.bin` | 750 / 11,687 | 750 | 17,868 슬롯 | 6.4% `█░░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–749, 750개; 다음 ID 750 |
...
| 장수 열전 v0.1–v0.2 | `msgbre` ID 0–250, 251개 초벌. SC·JP·EN 정렬, 형식 불변조건, 소스 비공개와 A/B 결정성 검증 완료 | 고유명사·문체 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 251 |
| 본편 `ev_strdata` v0.1–v0.4 | 파일 선두 장수 전체 이름표 ID 0–749, 750개. 기존 장수명과 SC 해시가 일치한 항목만 재사용하고 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 750 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.8 784개, 장수 열전 251개, 본편 장수명 750개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.8의 `msgev` 변경과 장수 열전 v0.1~v0.2의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. 후속 대사 784개, 장수 열전 251개, 본편 장수명 750개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
