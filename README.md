...
파싱이 끝난 10개 메시지 리소스 기준 **번역 완료 16,998 / 111,421 (15.3%)**, 초벌 커버리지는 **16,998개**다.
...
| `MSG_PK/SC/msggame.bin` | 1,200 / 16,482 | 1,200 | 25,598 슬롯 | 7.3% `█░░░░░░░░░` | 좌표 기반 1,200개 초벌; 다음 좌표 (6,1209,0), 블록 0 문법 조각 93개와 이전 비언어 형식 5개 문맥 검수 보류 |
...
| `msggame` 구조·번역 v0.1–v0.8 | PK 21,581·본편 19,152레코드의 18블록 바이트코드를 파싱. SC 표시 후보 16,482 + 12,268개를 확정하고 7개 언어 파일 byte-exact 재구축 및 더 긴 한글 리터럴 재패킹 성공. PK 1,200개 초벌 완료 | 코드용 문자열 사람 분류, 블록 0 문법 조각과 이전 비언어 형식 5개 문맥 검수, 실제 게임 화면 QA; 다음 좌표 `(6,1209,0)` |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.16 1,861개, 장수 열전 566개, 본편 장수명 2,207개, 본편 이벤트 라벨·서술 539개, `msggame` 1,200개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.16의 `msgev` 변경과 장수 열전 v0.1~v0.5의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. `msggame` 표시 후보 28,750개를 코드용 문자열과 실제 문장으로 계속 분류하고 PK 첫 1,200개 뒤의 번역을 진행한다.
4. 후속 대사 1,861개, 장수 열전 566개, 본편 장수명 2,207개, 본편 이벤트 라벨·서술 539개, `msggame` 1,200개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
