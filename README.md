...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 10,998 / 83,201 (13.2%)**, 초벌 커버리지는 **11,066개**다.
...
| `MSG_PK/SC/msgev.bin` | 2,806 / 12,906 | 2,806 | 17,910 슬롯 | 21.7% `██░░░░░░░░` | 장수명 2,207 + 대사 599 |
| `MSG_PK/SC/msgdata.bin` | 4,155 / 25,534 | 4,223 | 29,210 슬롯 | 16.3% `██░░░░░░░░` | 장수 구성요소 + 성 이름 + 옛 지방명; 검수 대기 68 |
| `MSG_PK/SC/msgbre.bin` | 0 / 2,217 | 0 | 3,000 슬롯 | 0.0% `░░░░░░░░░░` | 미착수 |
| `MSG_PK/SC/msgire.bin` | 0 / 122 | 0 | 122 슬롯 | 0.0% `░░░░░░░░░░` | 미착수 |
| `MSG_PK/SC/msgstf.bin` | 0 / 8 | 0 | 20 슬롯 | 0.0% `░░░░░░░░░░` | 미착수 |
...
| 옛 지방명 v0.1 | ID 13975–14046, 72개 source-free 한글 초벌과 표기 규칙 검사 완료 | 사람 검수, 장수명·성 이름 `msgdata`와 병합, 실제 지도 QA |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.6의 571개와 옛 지방명 72개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.6의 `msgev` 변경도 ID 오름차순으로 다시 병합한다.
3. 후속 대사 571개와 옛 지방명 72개의 새 한글 수요를 다음 글꼴 리비전에 넣고 네 글꼴
   표 누락 0개를 재검증한다.
