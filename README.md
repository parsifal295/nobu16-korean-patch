...
- 남은 파일 전용 후보는 성 이름 전체를 한 개의 가로 bitmap 글리프로 만드는
  `single-wide-glyph` 방식이며, 실제 지도 검증 전에는 해결로 판정하지 않는다.
...
- `data/translations/`: 검토 가능한 ID 단위 한국어 번역 배치
...
`msgui` ID 401–2000은 배치 번역과 형식 검증을 통과했다. 후속 배치는 같은 규칙으로
계속 확장하며, printf 토큰·개행·공백·PUA 코드를 원문과 동일하게 보존한다.
