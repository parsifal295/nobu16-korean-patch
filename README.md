...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 11,786 / 83,201 (14.2%)**, 초벌 커버리지는 **11,786개**다.
...
| `MSG_PK/SC/msgev.bin` | 2,917 / 12,906 | 2,917 | 17,910 슬롯 | 22.6% `██░░░░░░░░` | 장수명 2,207 + 대사 710 |
| `MSG_PK/SC/msgdata.bin` | 4,223 / 25,534 | 4,223 | 29,210 슬롯 | 16.5% `██░░░░░░░░` | 장수 구성요소 + 성 이름 + 옛 지방명; 검수 대기 0 |
| `MSG_PK/SC/msgbre.bin` | 129 / 2,217 | 129 | 3,000 슬롯 | 5.8% `█░░░░░░░░░` | 장수 열전 ID 0–128, 129개 초벌; 다음 ID 129 |
...
| `MSG_PK/SC/msggame.bin` | 0 / 조사 중 | — | 21,581 레코드 | — | 구조 레코드 수만 확정; 멀티블록 문자열 분모 조사 중 |
| `MSG/SC/msggame.bin` | 0 / 조사 중 | — | 19,152 레코드 | — | 구조 레코드 수만 확정; 본편 멀티블록 문자열 분모 조사 중 |
| `MSG/SC/strdata.bin` | 0 / 26,690 | 0 | 32,311 슬롯 | 0.0% `░░░░░░░░░░` | 5개 블록 파싱 완료; 번역 미착수 |
| `MSG/SC/ev_strdata.bin` | 350 / 11,687 | 350 | 17,868 슬롯 | 3.0% `░░░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–349, 350개; 다음 ID 350 |
...
| 장수 열전 v0.1 | `msgbre` ID 0–128, 129개 초벌. SC·JP·EN 정렬, 형식 불변조건, 소스 비공개와 A/B 결정성 검증 완료 | 고유명사·문체 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 129 |
| 본편 `ev_strdata` v0.1–v0.2 | 파일 선두 장수 전체 이름표 ID 0–349, 350개. 기존 장수명과 SC 해시가 일치한 항목만 재사용하고 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 350 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.7 682개, 장수 열전 129개, 본편 장수명 350개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
- [장수 열전 v0.1 129개](workstreams/msgbre/README_KO.md)
- [본편 `ev_strdata` v0.1–v0.2 350개](workstreams/ev_strdata/README_KO.md)
...
2. 장수명·대사 v0.1~v0.7의 `msgev` 변경과 장수 열전 v0.1의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. 후속 대사 682개, 장수 열전 129개, 본편 장수명 350개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
   리비전에 넣고 네 글꼴 표 누락 0개를 재검증한다.
4. MSGUI v0.2, 메시지 병합본, `msgire`·`msgstf`, 새 글꼴을 다중 파일 설치·복원 거래로
   조립한다.
