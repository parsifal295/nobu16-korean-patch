...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 11,586 / 83,201 (13.9%)**, 초벌 커버리지는 **11,586개**다.
...
| `MSG/SC/ev_strdata.bin` | 150 / 11,687 | 150 | 17,868 슬롯 | 1.3% `░░░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–149, 150개; 다음 ID 150 |
...
| 본편 `ev_strdata` v0.1 | 파일 선두 장수 전체 이름표 ID 0–149, 150개. 기존 장수명과 SC 해시가 일치한 항목만 재사용하고 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 150 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.7 682개, 장수 열전 129개, 본편 장수명 150개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
- [본편 `ev_strdata` v0.1 150개](workstreams/ev_strdata/README_KO.md)
...
2. 장수명·대사 v0.1~v0.7의 `msgev` 변경과 장수 열전 v0.1의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. 후속 대사 682개, 장수 열전 129개, 본편 장수명 150개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
