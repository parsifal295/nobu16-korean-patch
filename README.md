...
파싱이 끝난 10개 메시지 리소스 기준 **번역 완료 13,346 / 111,951 (11.9%)**, 초벌 커버리지는 **13,346개**다.
...
| `MSG_PK/SC/msgev.bin` | 3,148 / 12,906 | 3,148 | 17,910 슬롯 | 24.4% `██░░░░░░░░` | 장수명 2,207 + 대사 941 |
...
| `MSG/SC/ev_strdata.bin` | 1,350 / 11,687 | 1,350 | 17,868 슬롯 | 11.6% `█░░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–1349, 1,350개; 다음 ID 1350 |
...
| 본편 `ev_strdata` v0.1–v0.7 | 파일 선두 장수 전체 이름표 ID 0–1349, 1,350개. 기존 장수명 해시 일치 1,348개를 재사용하고 SC 표기 변형 2개를 독립 대조했으며 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 1350 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.9 913개, 장수 열전 458개, 본편 장수명 1,350개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.9의 `msgev` 변경과 장수 열전 v0.1~v0.4의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. `msggame` 표시 후보 28,750개를 코드용 문자열과 실제 문장으로 분류하고 번역 배치를 시작한다.
4. 후속 대사 913개, 장수 열전 458개, 본편 장수명 1,350개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
