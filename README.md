...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 13,039 / 83,201 (15.7%)**, 초벌 커버리지는 **13,039개**다.
...
| `MSG_PK/SC/msgbre.bin` | 351 / 2,217 | 351 | 3,000 슬롯 | 15.8% `██░░░░░░░░` | 장수 열전 ID 0–350, 351개 초벌; 다음 ID 351 |
...
| `MSG/SC/ev_strdata.bin` | 1,150 / 11,687 | 1,150 | 17,868 슬롯 | 9.8% `█░░░░░░░░░` | 파일 선두 장수 전체 이름표 ID 0–1149, 1,150개; 다음 ID 1150 |
...
| 장수 열전 v0.1–v0.3 | `msgbre` ID 0–350, 351개 초벌. SC·JP·EN 정렬, 형식 불변조건, 소스 비공개와 A/B 결정성 검증 완료 | 고유명사·문체 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 351 |
| 본편 `ev_strdata` v0.1–v0.6 | 파일 선두 장수 전체 이름표 ID 0–1149, 1,150개. 기존 장수명 해시 일치 1,148개를 재사용하고 SC 표기 변형 2개를 독립 대조했으며 SC·JP·TC 정렬·A/B 결정성 검증 완료 | 사람 검수 후 다음 글꼴·통합 배포본에 편입; 다음 ID 1150 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.9 913개, 장수 열전 351개, 본편 장수명 1,150개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.9의 `msgev` 변경과 장수 열전 v0.1~v0.3의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. 후속 대사 913개, 장수 열전 351개, 본편 장수명 1,150개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
