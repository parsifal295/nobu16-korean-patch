...
파싱이 끝난 10개 메시지 리소스 기준 **번역 완료 17,148 / 111,421 (15.4%)**, 초벌 커버리지는 **17,148개**다.
...
| `MSG_PK/SC/msgev.bin` | 4,096 / 12,906 | 4,096 | 17,910 슬롯 | 31.7% `███░░░░░░░` | 장수명 2,207 + 대사 1,889 |
...
| `MSG/SC/ev_strdata.bin` | 2,746 / 11,157 | 2,746 | 17,868 슬롯 | 24.6% `██░░░░░░░░` | 장수명 2,207개 + 이벤트 라벨·서술 539개 완료; 기존 code placeholder 488개와 추가 내부 참조·역할 키 42개는 대상 제외; 다음 실제 표시 후보 ID 3277 |
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.16 1,861개, 장수 열전 566개, 본편 장수명 2,207개, 본편 이벤트 라벨·서술 539개, `msggame` 1,350개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.16의 `msgev` 변경과 장수 열전 v0.1~v0.5의 `msgbre` 변경을 각 순정
   기준 레시피로 병합한다.
3. `msggame` 표시 후보 28,750개를 코드용 문자열과 실제 문장으로 계속 분류하고 PK 첫 1,350개 뒤의 번역을 진행한다.
4. 후속 대사 1,861개, 장수 열전 566개, 본편 장수명 2,207개, 본편 이벤트 라벨·서술 539개, `msggame` 1,350개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
