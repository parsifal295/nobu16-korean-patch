...
파싱이 끝난 8개 메시지 리소스 기준 **번역 완료 11,307 / 83,201 (13.6%)**, 초벌 커버리지는 **11,307개**다.
...
| Font-v6 | MSGUI v0.2 + 장수명 + 대사 v0.1 28개 + 성 이름 v0.1 392개의 700자 수요를 오프라인 A/B 빌드와 공개 레시피 재생으로 검증 | 최신 글리프 갱신과 런타임 화면·종료 QA. 성 이름 v0.2 교정분, 대사 v0.2~v0.7 682개, 옛 지방명 72개, 소형 테이블 130개는 아직 미포함 |
...
2. 장수명·대사 v0.1~v0.7의 `msgev` 변경도 ID 오름차순으로 다시 병합한다.
3. 후속 대사 682개, 옛 지방명 72개, 소형 테이블 130개의 새 한글 수요를 다음 글꼴
